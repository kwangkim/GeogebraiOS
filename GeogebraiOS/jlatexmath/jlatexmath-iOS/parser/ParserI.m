...
- (id<OrgScilabForgeJlatexmathPlatformParserDocument>)parseWithId:(id)input {
  ...
  return [new_DocumentI_initWithOrgW3cDomDocument_(document) autorelease];
}
...
void ParserI_init(ParserI *self) {
  NSObject_init(self);
  ParserI_set_factory_(self, JavaxXmlParsersDocumentBuilderFactory_newInstance());
}
...
id<OrgW3cDomDocument> ParserI_tryParseWithJavaIoInputStream_(ParserI *self, JavaIoInputStream *is) {
  ...
  @catch (JavaLangException *ex) {
    OrgScilabForgeJlatexmathExceptionResourceParseException *rpe = [new_OrgScilabForgeJlatexmathExceptionResourceParseException_initWithNSString_withJavaLangThrowable_(@"Could not parse resource", ex) autorelease];
    @throw rpe;
  }
}
