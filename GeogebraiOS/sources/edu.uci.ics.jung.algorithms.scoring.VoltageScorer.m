...
- (void)initialize__ {
  [super initialize__];
  if ([((id<JavaUtilMap>) nil_chk(source_voltages_)) isEmpty] || [((id<JavaUtilCollection>) nil_chk(sinks_)) isEmpty]) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"Both sources and sinks (grounds) must be defined") autorelease];
  if ([source_voltages_ size] + [((id<JavaUtilCollection>) nil_chk(sinks_)) size] > [((id<EduUciIcsJungGraphHypergraph>) nil_chk(graph_)) getVertexCount]) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"Source/sink sets overlap, or contain vertices not in graph") autorelease];
  for (id<JavaUtilMap_Entry> __strong entry_ in nil_chk([source_voltages_ entrySet])) {
    ...
    if ([sinks_ containsWithId:v]) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@$", @"Vertex ", v, @" is incorrectly specified as both source and sink")) autorelease];
    jdouble value = [((NSNumber *) nil_chk([entry_ getValue])) doubleValue];
    if (value <= 0) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@$", @"Source vertex ", v, @" has negative voltage")) autorelease];
  }
...
void EduUciIcsJungAlgorithmsScoringVoltageScorer_initWithEduUciIcsJungGraphHypergraph_withOrgApacheCommonsCollections15Transformer_withJavaUtilMap_withJavaUtilCollection_(EduUciIcsJungAlgorithmsScoringVoltageScorer *self, id<EduUciIcsJungGraphHypergraph> g, id<OrgApacheCommonsCollections15Transformer> edge_weights, id<JavaUtilMap> source_voltages, id<JavaUtilCollection> sinks) {
  ...
  EduUciIcsJungAlgorithmsScoringVoltageScorer_set_source_voltages_(self, source_voltages);
  EduUciIcsJungAlgorithmsScoringVoltageScorer_set_sinks_(self, sinks);
  [self initialize__];
}
...
void EduUciIcsJungAlgorithmsScoringVoltageScorer_initWithEduUciIcsJungGraphHypergraph_withOrgApacheCommonsCollections15Transformer_withJavaUtilCollection_withJavaUtilCollection_(EduUciIcsJungAlgorithmsScoringVoltageScorer *self, id<EduUciIcsJungGraphHypergraph> g, id<OrgApacheCommonsCollections15Transformer> edge_weights, id<JavaUtilCollection> sources, id<JavaUtilCollection> sinks) {
  EduUciIcsJungAlgorithmsScoringAbstractIterativeScorer_initWithEduUciIcsJungGraphHypergraph_withOrgApacheCommonsCollections15Transformer_(self, g, edge_weights);
  id<JavaUtilMap> unit_voltages = [new_JavaUtilHashMap_init() autorelease];
  for (id __strong v in nil_chk(sources)) [unit_voltages putWithId:v withId:[new_JavaLangDouble_initWithDouble_(1.0) autorelease]];
  EduUciIcsJungAlgorithmsScoringVoltageScorer_set_source_voltages_(self, unit_voltages);
  EduUciIcsJungAlgorithmsScoringVoltageScorer_set_sinks_(self, sinks);
  [self initialize__];
}
...
void EduUciIcsJungAlgorithmsScoringVoltageScorer_initWithEduUciIcsJungGraphHypergraph_withJavaUtilMap_withJavaUtilCollection_(EduUciIcsJungAlgorithmsScoringVoltageScorer *self, id<EduUciIcsJungGraphHypergraph> g, id<JavaUtilMap> source_voltages, id<JavaUtilCollection> sinks) {
  ...
  EduUciIcsJungAlgorithmsScoringVoltageScorer_set_source_voltages_(self, source_voltages);
  EduUciIcsJungAlgorithmsScoringVoltageScorer_set_sinks_(self, sinks);
  EduUciIcsJungAlgorithmsScoringAbstractIterativeScorer_setAndConsume_edge_weights_(self, new_EduUciIcsJungAlgorithmsScoringUtilUniformDegreeWeight_initWithEduUciIcsJungGraphHypergraph_(g));
  [self initialize__];
}
...
void EduUciIcsJungAlgorithmsScoringVoltageScorer_initWithEduUciIcsJungGraphHypergraph_withOrgApacheCommonsCollections15Transformer_withId_withId_(EduUciIcsJungAlgorithmsScoringVoltageScorer *self, id<EduUciIcsJungGraphHypergraph> g, id<OrgApacheCommonsCollections15Transformer> edge_weights, id source, id sink) {
  EduUciIcsJungAlgorithmsScoringVoltageScorer_initWithEduUciIcsJungGraphHypergraph_withOrgApacheCommonsCollections15Transformer_withJavaUtilMap_withJavaUtilCollection_(self, g, edge_weights, JavaUtilCollections_singletonMapWithId_withId_(source, JavaLangDouble_valueOfWithDouble_(1.0)), JavaUtilCollections_singletonListWithId_(sink));
  [self initialize__];
}
...
void EduUciIcsJungAlgorithmsScoringVoltageScorer_initWithEduUciIcsJungGraphHypergraph_withId_withId_(EduUciIcsJungAlgorithmsScoringVoltageScorer *self, id<EduUciIcsJungGraphHypergraph> g, id source, id sink) {
  EduUciIcsJungAlgorithmsScoringVoltageScorer_initWithEduUciIcsJungGraphHypergraph_withJavaUtilMap_withJavaUtilCollection_(self, g, JavaUtilCollections_singletonMapWithId_withId_(source, JavaLangDouble_valueOfWithDouble_(1.0)), JavaUtilCollections_singletonListWithId_(sink));
  [self initialize__];
}
