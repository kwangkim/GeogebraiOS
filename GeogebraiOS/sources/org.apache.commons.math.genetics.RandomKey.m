...
//  source: /Users/jacky/GSoC/geogebra_iOS/geogebra/geogebra/common/src/org/apache/commons/math/genetics/RandomKey.java
//
...
- (void)checkValidityWithJavaUtilList:(id<JavaUtilList>)chromosomeRepresentation {
  for (JavaLangDouble *boxed__ in nil_chk(chromosomeRepresentation)) {
    ...
    if (val < 0 || val > 1) {
      @throw [new_OrgApacheCommonsMathGeneticsInvalidRepresentationException_initWithNSString_(@"Values of representation must be in [0,1] interval") autorelease];
    }
  }
}
...
- (NSString *)description {
  return NSString_formatWithNSString_withNSObjectArray_(@"(f=%s pi=(%s))", [IOSObjectArray arrayWithObjects:(id[]){ JavaLangDouble_valueOfWithDouble_([self getFitness]), baseSeqPermutation_ } count:2 type:NSObject_class_()]);
}
...
- (void)dealloc {
  RELEASE_(sortedRepresentation_);
  RELEASE_(baseSeqPermutation_);
  [super dealloc];
}
...
void OrgApacheCommonsMathGeneticsRandomKey_initWithJavaUtilList_(OrgApacheCommonsMathGeneticsRandomKey *self, id<JavaUtilList> representation) {
  ...
  id<JavaUtilList> sortedRepr = [new_JavaUtilArrayList_initWithJavaUtilCollection_([self getRepresentation]) autorelease];
  JavaUtilCollections_sortWithJavaUtilList_(sortedRepr);
  OrgApacheCommonsMathGeneticsRandomKey_set_sortedRepresentation_(self, JavaUtilCollections_unmodifiableListWithJavaUtilList_(sortedRepr));
  OrgApacheCommonsMathGeneticsRandomKey_set_baseSeqPermutation_(self, JavaUtilCollections_unmodifiableListWithJavaUtilList_(OrgApacheCommonsMathGeneticsRandomKey_decodeGenericWithJavaUtilList_withJavaUtilList_withJavaUtilList_(OrgApacheCommonsMathGeneticsRandomKey_baseSequenceWithInt_([self getLength]), [self getRepresentation], self->sortedRepresentation_)));
}
...
void OrgApacheCommonsMathGeneticsRandomKey_initWithJavaLangDoubleArray_(OrgApacheCommonsMathGeneticsRandomKey *self, IOSObjectArray *representation) {
  OrgApacheCommonsMathGeneticsRandomKey_initWithJavaUtilList_(self, JavaUtilArrays_asListWithNSObjectArray_(representation));
}
...
id<JavaUtilList> OrgApacheCommonsMathGeneticsRandomKey_randomPermutationWithInt_(jint l) {
  OrgApacheCommonsMathGeneticsRandomKey_initialize();
  id<JavaUtilList> repr = [new_JavaUtilArrayList_initWithInt_(l) autorelease];
  for (jint i = 0; i < l; i++) {
...
id<JavaUtilList> OrgApacheCommonsMathGeneticsRandomKey_comparatorPermutationWithJavaUtilList_withJavaUtilComparator_(id<JavaUtilList> data, id<JavaUtilComparator> comparator) {
  OrgApacheCommonsMathGeneticsRandomKey_initialize();
  id<JavaUtilList> sortedData = [new_JavaUtilArrayList_initWithJavaUtilCollection_(data) autorelease];
...
id<JavaUtilList> OrgApacheCommonsMathGeneticsRandomKey_inducedPermutationWithJavaUtilList_withJavaUtilList_(id<JavaUtilList> originalData, id<JavaUtilList> permutedData) {
  ...
  if ([((id<JavaUtilList>) nil_chk(originalData)) size] != [((id<JavaUtilList>) nil_chk(permutedData)) size]) {
    @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"originalData and permutedData must have same length") autorelease];
  }
  jint l = [originalData size];
  id<JavaUtilList> origDataCopy = [new_JavaUtilArrayList_initWithJavaUtilCollection_(originalData) autorelease];
  IOSObjectArray *res = [IOSObjectArray arrayWithLength:l type:JavaLangDouble_class_()];
  for (jint i = 0; i < l; i++) {
    jint index = [origDataCopy indexOfWithId:[permutedData getWithInt:i]];
    if (index == -1) {
      @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"originalData and permutedData must contain the same objects.") autorelease];
    }
    IOSObjectArray_Set(res, index, JavaLangDouble_valueOfWithDouble_((jdouble) i / l));
    [origDataCopy setWithInt:index withId:nil];
  }
...
id<JavaUtilList> OrgApacheCommonsMathGeneticsRandomKey_baseSequenceWithInt_(jint l) {
  OrgApacheCommonsMathGeneticsRandomKey_initialize();
  id<JavaUtilList> baseSequence = [new_JavaUtilArrayList_initWithInt_(l) autorelease];
  for (jint i = 0; i < l; i++) {
