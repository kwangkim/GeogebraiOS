...
//  source: /Users/jacky/GSoC/geogebra_iOS/geogebra/geogebra/common/src/org/apache/commons/math/stat/Frequency.java
//
...
- (NSString *)description {
  ...
  [outBuffer appendWithNSString:@"Value \t Freq. \t Pct. \t Cum Pct. \n"];
  ...
  while ([((id<JavaUtilIterator>) nil_chk(iter)) hasNext]) {
    ...
    [outBuffer appendWithId:value];
    [outBuffer appendWithChar:0x0009];
    [outBuffer appendWithLong:[self getCountWithJavaLangComparable:value]];
    [outBuffer appendWithChar:0x0009];
    [outBuffer appendWithNSString:[((JavaTextNumberFormat *) nil_chk(nf)) formatWithDouble:[self getPctWithJavaLangComparable:value]]];
    [outBuffer appendWithChar:0x0009];
    [outBuffer appendWithNSString:[nf formatWithDouble:[self getCumPctWithJavaLangComparable:value]]];
    [outBuffer appendWithChar:0x000a];
  }
...
- (void)addValueWithJavaLangComparable:(id<JavaLangComparable>)v {
  ...
  @try {
    ...
    if (count == nil) {
      [freqTable_ putWithId:obj withId:JavaLangLong_valueOfWithLong_(1)];
    }
    else {
      [freqTable_ putWithId:obj withId:JavaLangLong_valueOfWithLong_([count longLongValue] + 1)];
    }
  }
  @catch (JavaLangClassCastException *ex) {
    @throw OrgApacheCommonsMathMathRuntimeException_createIllegalArgumentExceptionWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_INSTANCES_NOT_COMPARABLE_TO_EXISTING_VALUES(), [IOSObjectArray arrayWithObjects:(id[]){ [[((id<JavaLangComparable>) nil_chk(v)) getClass] getName] } count:1 type:NSObject_class_()]);
  }
}
...
- (jlong)getCumFreqWithJavaLangComparable:(id<JavaLangComparable>)v {
  ...
  if (c == nil) {
    c = [new_OrgApacheCommonsMathStatFrequency_NaturalComparator_init() autorelease];
  }
  jlong result = 0;
  @try {
...
void OrgApacheCommonsMathStatFrequency_init(OrgApacheCommonsMathStatFrequency *self) {
  NSObject_init(self);
  OrgApacheCommonsMathStatFrequency_setAndConsume_freqTable_(self, new_JavaUtilTreeMap_init());
}
...
void OrgApacheCommonsMathStatFrequency_initWithJavaUtilComparator_(OrgApacheCommonsMathStatFrequency *self, id<JavaUtilComparator> comparator) {
  NSObject_init(self);
  OrgApacheCommonsMathStatFrequency_setAndConsume_freqTable_(self, new_JavaUtilTreeMap_initWithJavaUtilComparator_((id<JavaUtilComparator>) check_protocol_cast(comparator, @protocol(JavaUtilComparator))));
}
