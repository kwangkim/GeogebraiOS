...
- (void)setDataWithDoubleArray:(IOSDoubleArray *)values {
  if (values == nil) {
    OrgApacheCommonsMathStatDescriptiveRankPercentile_set_cachedPivots_(self, nil);
  }
  else {
    OrgApacheCommonsMathStatDescriptiveRankPercentile_setAndConsume_cachedPivots_(self, [IOSIntArray newArrayWithLength:(LShift32((jint) 0x1, OrgApacheCommonsMathStatDescriptiveRankPercentile_MAX_CACHED_LEVELS)) - 1]);
...
- (jdouble)evaluateWithDoubleArray:(IOSDoubleArray *)values
  ...
  if (values == [self getDataRef]) {
    ...
    work = [IOSDoubleArray arrayWithLength:length];
    JavaLangSystem_arraycopyWithId_withInt_withId_withInt_withInt_(values, begin, work, 0, length);
    pivotsHeap = [IOSIntArray arrayWithLength:(LShift32((jint) 0x1, OrgApacheCommonsMathStatDescriptiveRankPercentile_MAX_CACHED_LEVELS)) - 1];
...
- (OrgApacheCommonsMathStatDescriptiveRankPercentile *)copy__ {
  OrgApacheCommonsMathStatDescriptiveRankPercentile *result = [new_OrgApacheCommonsMathStatDescriptiveRankPercentile_init() autorelease];
