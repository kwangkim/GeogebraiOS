...
- (void)checkLocked {
  if ([self isLocked]) {
    @throw [new_JavaLangUnsupportedOperationException_initWithNSString_(@"Cannot modify a FixedOrderComparator after a comparison") autorelease];
  }
}
...
- (void)setUnknownObjectBehaviorWithInt:(jint)unknownObjectBehavior {
  ...
  if (unknownObjectBehavior != OrgApacheCommonsCollections15ComparatorsFixedOrderComparator_UNKNOWN_AFTER && unknownObjectBehavior != OrgApacheCommonsCollections15ComparatorsFixedOrderComparator_UNKNOWN_BEFORE && unknownObjectBehavior != OrgApacheCommonsCollections15ComparatorsFixedOrderComparator_UNKNOWN_THROW_EXCEPTION) {
    @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"Unrecognised value for unknown behaviour flag") autorelease];
  }
...
- (jboolean)addWithId:(id)obj {
  [self checkLocked];
  JavaLangInteger *position = [((id<JavaUtilMap>) nil_chk(map_)) putWithId:obj withId:[new_JavaLangInteger_initWithInt_(counter_++) autorelease]];
  return position == nil;
}
...
- (jboolean)addAsEqualWithId:(id)existingObj
  ...
  if (position == nil) {
    @throw [new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("@$@", existingObj, @" not known to ", self)) autorelease];
  }
  ...
  if (position1 == nil || position2 == nil) {
    {
      ...
      switch (unknownObjectBehavior_) {
        ...
        @throw [new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@", @"Attempting to compare unknown object ", unknownObj)) autorelease];
        default:
        @throw [new_JavaLangUnsupportedOperationException_initWithNSString_(JreStrcat("$I", @"Unknown unknownObjectBehavior: ", unknownObjectBehavior_)) autorelease];
      }
    }
  }
  else {
...
void OrgApacheCommonsCollections15ComparatorsFixedOrderComparator_init(OrgApacheCommonsCollections15ComparatorsFixedOrderComparator *self) {
  NSObject_init(self);
  OrgApacheCommonsCollections15ComparatorsFixedOrderComparator_setAndConsume_map_(self, new_JavaUtilHashMap_init());
  self->counter_ = 0;
  self->isLocked__ = NO;
...
void OrgApacheCommonsCollections15ComparatorsFixedOrderComparator_initWithNSObjectArray_(OrgApacheCommonsCollections15ComparatorsFixedOrderComparator *self, IOSObjectArray *items) {
  ...
  self->unknownObjectBehavior_ = OrgApacheCommonsCollections15ComparatorsFixedOrderComparator_UNKNOWN_THROW_EXCEPTION;
  if (items == nil) {
    @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"The list of items must not be null") autorelease];
  }
