...
- (jboolean)addVertexWithId:(id)vertex {
  ...
  if (![self containsVertexWithId:vertex]) {
    [((id<JavaUtilMap>) nil_chk(vertices_)) putWithId:vertex withId:[new_EduUciIcsJungGraphUtilPair_initWithId_withId_([new_JavaUtilHashSet_init() autorelease], [new_JavaUtilHashSet_init() autorelease]) autorelease]];
    return YES;
  }
  else {
    return NO;
...
- (id<JavaUtilCollection>)getIncidentEdgesWithId:(id)vertex {
  if (![self containsVertexWithId:vertex]) return nil;
  id<JavaUtilCollection> incident = [new_JavaUtilHashSet_init() autorelease];
...
- (EduUciIcsJungGraphDirectedSparseMultigraph *)newInstance {
  return [new_EduUciIcsJungGraphDirectedSparseMultigraph_init() autorelease];
}
...
id<OrgApacheCommonsCollections15Factory> EduUciIcsJungGraphDirectedSparseMultigraph_getFactory() {
  ...
  return [new_EduUciIcsJungGraphDirectedSparseMultigraph_$1_init() autorelease];
}
...
void EduUciIcsJungGraphDirectedSparseMultigraph_init(EduUciIcsJungGraphDirectedSparseMultigraph *self) {
  ...
  EduUciIcsJungGraphDirectedSparseMultigraph_setAndConsume_vertices_(self, new_JavaUtilHashMap_init());
  EduUciIcsJungGraphDirectedSparseMultigraph_setAndConsume_edges_(self, new_JavaUtilHashMap_init());
}
