...
//  source: /Users/jacky/GSoC/geogebra_iOS/geogebra/geogebra/common/src/org/apache/commons/math/analysis/polynomials/PolynomialSplineFunction.java
//
...
- (jdouble)valueWithDouble:(jdouble)v {
  if (v < IOSDoubleArray_Get(nil_chk(knots_), 0) || v > IOSDoubleArray_Get(knots_, n_)) {
    @throw [new_OrgApacheCommonsMathArgumentOutsideDomainException_initWithDouble_withDouble_withDouble_(v, IOSDoubleArray_Get(knots_, 0), IOSDoubleArray_Get(knots_, n_)) autorelease];
  }
...
- (OrgApacheCommonsMathAnalysisPolynomialsPolynomialSplineFunction *)polynomialSplineDerivative {
  IOSObjectArray *derivativePolynomials = [IOSObjectArray arrayWithLength:n_ type:OrgApacheCommonsMathAnalysisPolynomialsPolynomialFunction_class_()];
  for (jint i = 0; i < n_; i++) {
    IOSObjectArray_Set(derivativePolynomials, i, [((OrgApacheCommonsMathAnalysisPolynomialsPolynomialFunction *) nil_chk(IOSObjectArray_Get(nil_chk(polynomials_), i))) polynomialDerivative]);
  }
  return [new_OrgApacheCommonsMathAnalysisPolynomialsPolynomialSplineFunction_initWithDoubleArray_withOrgApacheCommonsMathAnalysisPolynomialsPolynomialFunctionArray_(knots_, derivativePolynomials) autorelease];
}
...
- (IOSObjectArray *)getPolynomials {
  IOSObjectArray *p = [IOSObjectArray arrayWithLength:n_ type:OrgApacheCommonsMathAnalysisPolynomialsPolynomialFunction_class_()];
  JavaLangSystem_arraycopyWithId_withInt_withId_withInt_withInt_(polynomials_, 0, p, 0, n_);
  return p;
}

- (IOSDoubleArray *)getKnots {
  IOSDoubleArray *out = [IOSDoubleArray arrayWithLength:n_ + 1];
...
void OrgApacheCommonsMathAnalysisPolynomialsPolynomialSplineFunction_initWithDoubleArray_withOrgApacheCommonsMathAnalysisPolynomialsPolynomialFunctionArray_(OrgApacheCommonsMathAnalysisPolynomialsPolynomialSplineFunction *self, IOSDoubleArray *knots, IOSObjectArray *polynomials) {
  ...
  if (!OrgApacheCommonsMathAnalysisPolynomialsPolynomialSplineFunction_isStrictlyIncreasingWithDoubleArray_(knots)) {
    @throw OrgApacheCommonsMathMathRuntimeException_createIllegalArgumentExceptionWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_NOT_STRICTLY_INCREASING_KNOT_VALUES(), [IOSObjectArray arrayWithLength:0 type:NSObject_class_()]);
  }
  self->n_ = knots->size_ - 1;
  OrgApacheCommonsMathAnalysisPolynomialsPolynomialSplineFunction_setAndConsume_knots_(self, [IOSDoubleArray newArrayWithLength:self->n_ + 1]);
  JavaLangSystem_arraycopyWithId_withInt_withId_withInt_withInt_(knots, 0, self->knots_, 0, self->n_ + 1);
  OrgApacheCommonsMathAnalysisPolynomialsPolynomialSplineFunction_setAndConsume_polynomials_(self, [IOSObjectArray newArrayWithLength:self->n_ type:OrgApacheCommonsMathAnalysisPolynomialsPolynomialFunction_class_()]);
