...
#include "geogebra/common/gui/inputfield/MyTextField.h"
...
@implementation GeogebraCommonGuiInputfieldMyTextField

+ (IOSIntArray *)getBracketPositionsWithNSString:(NSString *)inputText
                                         withInt:(jint)caret {
  return GeogebraCommonGuiInputfieldMyTextField_getBracketPositionsWithNSString_withInt_(inputText, caret);
}

+ (jboolean)isCloseBracketOrWhitespaceWithChar:(jchar)c {
  return GeogebraCommonGuiInputfieldMyTextField_isCloseBracketOrWhitespaceWithChar_(c);
}
...
+ (GeogebraCommonGuiInputfieldValidateAutocompletionResult *)commonValidateAutocompletionWithInt:(jint)index
  ...
  return GeogebraCommonGuiInputfieldMyTextField_commonValidateAutocompletionWithInt_withJavaUtilList_withNSString_withInt_(index, completions, text, curWordStart);
}
...
- (instancetype)init {
  GeogebraCommonGuiInputfieldMyTextField_init(self);
  return self;
}
...
+ (const J2ObjcClassInfo *)__metadata {
  static const J2ObjcMethodInfo methods[] = {
    ...
    { "commonValidateAutocompletionWithInt:withJavaUtilList:withNSString:withInt:", "commonValidateAutocompletion", "Lgeogebra.common.gui.inputfield.ValidateAutocompletionResult;", 0x9, NULL, NULL },
    { "setShowSymbolTableIconWithBoolean:", "setShowSymbolTableIcon", "V", 0x1, NULL, NULL },
    { "init", NULL, NULL, 0x1, NULL, NULL },
  };
  static const J2ObjcClassInfo _GeogebraCommonGuiInputfieldMyTextField = { 2, "MyTextField", "geogebra.common.gui.inputfield", NULL, 0x401, 5, methods, 0, NULL, 0, NULL, 0, NULL, NULL, NULL };
  return &_GeogebraCommonGuiInputfieldMyTextField;
}

@end

IOSIntArray *GeogebraCommonGuiInputfieldMyTextField_getBracketPositionsWithNSString_withInt_(NSString *inputText, jint caret) {
  GeogebraCommonGuiInputfieldMyTextField_initialize();
  ...
  IOSIntArray *result = [IOSIntArray arrayWithInts:(jint[]){ bracketPos0, bracketPos1 } count:2];
  return result;
}

jboolean GeogebraCommonGuiInputfieldMyTextField_isCloseBracketOrWhitespaceWithChar_(jchar c) {
  GeogebraCommonGuiInputfieldMyTextField_initialize();
  return GeogebraCommonUtilStringUtil_isWhitespaceWithChar_(c) || c == ')' || c == ']' || c == '}';
}

GeogebraCommonGuiInputfieldValidateAutocompletionResult *GeogebraCommonGuiInputfieldMyTextField_commonValidateAutocompletionWithInt_withJavaUtilList_withNSString_withInt_(jint index, id<JavaUtilList> completions, NSString *text, jint curWordStart) {
  GeogebraCommonGuiInputfieldMyTextField_initialize();
  GeogebraCommonGuiInputfieldValidateAutocompletionResult *ret = [new_GeogebraCommonGuiInputfieldValidateAutocompletionResult_init() autorelease];
  ...
  JavaUtilArrayList *textTokens = [new_JavaUtilArrayList_init() autorelease];
  jint textBLPos = -1;
  jint textBRPos = -1;
  ...
  for (jint i = 0; i < ((IOSCharArray *) nil_chk(tArray))->size_; i++) {
    switch (IOSCharArray_Get(tArray, i)) {
      ...
      break;
      default:
      [sb appendWithChar:IOSCharArray_Get(tArray, i)];
    }
  ...
  JavaUtilArrayList *commandTokens = [new_JavaUtilArrayList_init() autorelease];
  sb = [new_JavaLangStringBuilder_init() autorelease];
  jint commandBLPos = -1;
  ...
  for (jint i = 0; i < ((IOSCharArray *) nil_chk(cArray))->size_; i++) {
    switch (IOSCharArray_Get(cArray, i)) {
      ...
      break;
      default:
      if (!GeogebraCommonUtilStringUtil_isWhitespaceWithChar_(IOSCharArray_Get(cArray, i))) {
  ...
  for (jint i = 0; i < startPos; i++) {
    ...
    if ([((NSString *) nil_chk(current)) isEqual:@"["]) {
      ...
      [sb appendWithNSString:current];
    }
  }
  [sb appendWithNSString:[commandTokens getWithInt:0]];
  if (textBLCount == 0) {
    if (commandParams == 0) {
      [sb appendWithNSString:@"[]"];
      carPos = [sb length];
  ...
  ret->carPos_ = carPos;
  GeogebraCommonGuiInputfieldValidateAutocompletionResult_set_sb_(ret, [sb description]);
  return ret;
}

void GeogebraCommonGuiInputfieldMyTextField_init(GeogebraCommonGuiInputfieldMyTextField *self) {
  NSObject_init(self);
}

J2OBJC_CLASS_TYPE_LITERAL_SOURCE(GeogebraCommonGuiInputfieldMyTextField)
