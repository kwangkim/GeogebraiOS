...
+ (void)initialize {
  if (self == [EduUciIcsJungAlgorithmsMetricsTriadicCensus class]) {
    JreStrongAssignAndConsume(&EduUciIcsJungAlgorithmsMetricsTriadicCensus_TRIAD_NAMES_, nil, [IOSObjectArray newArrayWithObjects:(id[]){ @"N/A", @"003", @"012", @"102", @"021D", @"021U", @"021C", @"111D", @"111U", @"030T", @"030C", @"201", @"120D", @"120U", @"120C", @"210", @"300" } count:17 type:NSString_class_()]);
    EduUciIcsJungAlgorithmsMetricsTriadicCensus_MAX_TRIADS_ = EduUciIcsJungAlgorithmsMetricsTriadicCensus_TRIAD_NAMES_->size_;
    JreStrongAssignAndConsume(&EduUciIcsJungAlgorithmsMetricsTriadicCensus_codeToType_, nil, [IOSIntArray newArrayWithInts:(jint[]){ 1, 2, 2, 3, 2, 4, 6, 8, 2, 6, 5, 7, 3, 8, 7, 11, 2, 6, 4, 8, 5, 9, 9, 13, 6, 10, 9, 14, 7, 14, 12, 15, 2, 5, 6, 7, 6, 9, 10, 14, 4, 9, 9, 12, 8, 13, 14, 15, 3, 7, 8, 11, 7, 12, 14, 15, 8, 14, 13, 15, 11, 15, 15, 16 } count:64]);
...
IOSLongArray *EduUciIcsJungAlgorithmsMetricsTriadicCensus_getCountsWithEduUciIcsJungGraphDirectedGraph_(id<EduUciIcsJungGraphDirectedGraph> g) {
  ...
  IOSLongArray *count = [IOSLongArray arrayWithLength:EduUciIcsJungAlgorithmsMetricsTriadicCensus_MAX_TRIADS_];
  id<JavaUtilList> id_ = [new_JavaUtilArrayList_initWithJavaUtilCollection_([((id<EduUciIcsJungGraphDirectedGraph>) nil_chk(g)) getVertices]) autorelease];
  for (jint i_v = 0; i_v < [g getVertexCount]; i_v++) {
    ...
    for (id __strong u in nil_chk([g getNeighborsWithId:v])) {
      ...
      id<JavaUtilSet> neighbors = [new_JavaUtilHashSet_initWithJavaUtilCollection_(OrgApacheCommonsCollections15CollectionUtils_union__WithJavaUtilCollection_withJavaUtilCollection_([g getNeighborsWithId:u], [g getNeighborsWithId:v])) autorelease];
