...
id<JavaUtilList> ComVividsolutionsJtsIndexChainMonotoneChainBuilder_getChainsWithComVividsolutionsJtsGeomCoordinateArray_withId_(IOSObjectArray *pts, id context) {
  ComVividsolutionsJtsIndexChainMonotoneChainBuilder_initialize();
  id<JavaUtilList> mcList = [new_JavaUtilArrayList_init() autorelease];
  ...
  for (jint i = 0; i < ((IOSIntArray *) nil_chk(startIndex))->size_ - 1; i++) {
    ComVividsolutionsJtsIndexChainMonotoneChain *mc = [new_ComVividsolutionsJtsIndexChainMonotoneChain_initWithComVividsolutionsJtsGeomCoordinateArray_withInt_withInt_withId_(pts, IOSIntArray_Get(startIndex, i), IOSIntArray_Get(startIndex, i + 1), context) autorelease];
    [mcList addWithId:mc];
  }
  return mcList;
}
