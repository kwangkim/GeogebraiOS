...
//  source: /Users/jacky/GSoC/geogebra_iOS/geogebra/geogebra/common/src/org/apache/commons/math/optimization/general/GaussNewtonOptimizer.java
//
...
- (OrgApacheCommonsMathOptimizationVectorialPointValuePair *)doOptimize {
  ...
  for (jboolean converged = NO; !converged; ) {
    ...
    IOSDoubleArray *b = [IOSDoubleArray arrayWithLength:cols_];
    IOSObjectArray *a = [IOSDoubleArray arrayWithDimensions:2 lengths:(jint[]){ cols_, cols_ }];
    ...
    @try {
      id<OrgApacheCommonsMathLinearRealMatrix> mA = [new_OrgApacheCommonsMathLinearBlockRealMatrix_initWithDoubleArray2_(a) autorelease];
      id<OrgApacheCommonsMathLinearDecompositionSolver> solver = useLU_ ? [((OrgApacheCommonsMathLinearLUDecompositionImpl *) [new_OrgApacheCommonsMathLinearLUDecompositionImpl_initWithOrgApacheCommonsMathLinearRealMatrix_(mA) autorelease]) getSolver] : [((OrgApacheCommonsMathLinearQRDecompositionImpl *) [new_OrgApacheCommonsMathLinearQRDecompositionImpl_initWithOrgApacheCommonsMathLinearRealMatrix_(mA) autorelease]) getSolver];
    ...
    @catch (OrgApacheCommonsMathLinearInvalidMatrixException *e) {
      @throw [new_OrgApacheCommonsMathOptimizationOptimizationException_initWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_UNABLE_TO_SOLVE_SINGULAR_PROBLEM(), [IOSObjectArray arrayWithLength:0 type:NSObject_class_()]) autorelease];
    }
    if (previous != nil) {
