...
- (id<JavaUtilList>)addAllItemsWithJavaUtilList:(id<JavaUtilList>)items {
  ...
  for (jint i = 0; i < 2; i++) {
    if (IOSObjectArray_Get(nil_chk(subnode_), i) != nil) {
      [((ComVividsolutionsJtsIndexBintreeNode *) nil_chk(IOSObjectArray_Get(subnode_, i))) addAllItemsWithJavaUtilList:items];
    }
  }
  return items;
}
...
- (id<JavaUtilList>)addAllItemsFromOverlappingWithComVividsolutionsJtsIndexBintreeInterval:(ComVividsolutionsJtsIndexBintreeInterval *)interval
  ...
  for (jint i = 0; i < 2; i++) {
    if (IOSObjectArray_Get(nil_chk(subnode_), i) != nil) {
      [((ComVividsolutionsJtsIndexBintreeNode *) nil_chk(IOSObjectArray_Get(subnode_, i))) addAllItemsFromOverlappingWithComVividsolutionsJtsIndexBintreeInterval:interval withJavaUtilCollection:resultItems];
    }
  }
  return items_;
}
...
void ComVividsolutionsJtsIndexBintreeNodeBase_init(ComVividsolutionsJtsIndexBintreeNodeBase *self) {
  NSObject_init(self);
  ComVividsolutionsJtsIndexBintreeNodeBase_setAndConsume_items_(self, new_JavaUtilArrayList_init());
  ComVividsolutionsJtsIndexBintreeNodeBase_setAndConsume_subnode_(self, [IOSObjectArray newArrayWithLength:2 type:ComVividsolutionsJtsIndexBintreeNode_class_()]);
}
