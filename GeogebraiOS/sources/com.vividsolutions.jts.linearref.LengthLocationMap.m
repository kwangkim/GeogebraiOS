...
- (jdouble)getLengthWithComVividsolutionsJtsLinearrefLinearLocation:(ComVividsolutionsJtsLinearrefLinearLocation *)loc {
  ...
  ComVividsolutionsJtsLinearrefLinearIterator *it = [new_ComVividsolutionsJtsLinearrefLinearIterator_initWithComVividsolutionsJtsGeomGeometry_(linearGeom_) autorelease];
  while ([it hasNext]) {
    if (![it isEndOfLine]) {
...
ComVividsolutionsJtsLinearrefLinearLocation *ComVividsolutionsJtsLinearrefLengthLocationMap_getLocationWithComVividsolutionsJtsGeomGeometry_withDouble_(ComVividsolutionsJtsGeomGeometry *linearGeom, jdouble length) {
  ...
  ComVividsolutionsJtsLinearrefLengthLocationMap *locater = [new_ComVividsolutionsJtsLinearrefLengthLocationMap_initWithComVividsolutionsJtsGeomGeometry_(linearGeom) autorelease];
...
void ComVividsolutionsJtsLinearrefLengthLocationMap_initWithComVividsolutionsJtsGeomGeometry_(ComVividsolutionsJtsLinearrefLengthLocationMap *self, ComVividsolutionsJtsGeomGeometry *linearGeom) {
  NSObject_init(self);
  ComVividsolutionsJtsLinearrefLengthLocationMap_set_linearGeom_(self, linearGeom);
}
...
ComVividsolutionsJtsLinearrefLinearLocation *ComVividsolutionsJtsLinearrefLengthLocationMap_getLocationForwardWithDouble_(ComVividsolutionsJtsLinearrefLengthLocationMap *self, jdouble length) {
  if (length <= 0.0) return [new_ComVividsolutionsJtsLinearrefLinearLocation_init() autorelease];
  jdouble totalLength = 0.0;
  ...
  while ([it hasNext]) {
    if (![it isEndOfLine]) {
      ...
      if (totalLength + segLen > length) {
        ...
        return [new_ComVividsolutionsJtsLinearrefLinearLocation_initWithInt_withInt_withDouble_(compIndex, segIndex, frac) autorelease];
      }
      totalLength += segLen;
    }
    [it next];
