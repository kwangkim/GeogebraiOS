...
void EduUciIcsJungAlgorithmsUtilDiscreteDistribution_checkLengthsWithDoubleArray_withDoubleArray_(IOSDoubleArray *dist, IOSDoubleArray *reference) {
  EduUciIcsJungAlgorithmsUtilDiscreteDistribution_initialize();
  if (((IOSDoubleArray *) nil_chk(dist))->size_ != ((IOSDoubleArray *) nil_chk(reference))->size_) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"Arrays must be of the same length") autorelease];
}
...
IOSDoubleArray *EduUciIcsJungAlgorithmsUtilDiscreteDistribution_meanWithJavaUtilCollection_(id<JavaUtilCollection> distributions) {
  EduUciIcsJungAlgorithmsUtilDiscreteDistribution_initialize();
  if ([((id<JavaUtilCollection>) nil_chk(distributions)) isEmpty]) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"Distribution collection must be non-empty") autorelease];
  id<JavaUtilIterator> iter = [distributions iterator];
  IOSDoubleArray *first = [((id<JavaUtilIterator>) nil_chk(iter)) next];
  IOSObjectArray *d_array = [IOSDoubleArray arrayWithDimensions:2 lengths:(jint[]){ [distributions size], ((IOSDoubleArray *) nil_chk(first))->size_ }];
  IOSObjectArray_Set(d_array, 0, first);
  for (jint i = 1; i < d_array->size_; i++) IOSObjectArray_Set(d_array, i, [iter next]);
...
IOSDoubleArray *EduUciIcsJungAlgorithmsUtilDiscreteDistribution_meanWithDoubleArray2_(IOSObjectArray *distributions) {
  EduUciIcsJungAlgorithmsUtilDiscreteDistribution_initialize();
  IOSDoubleArray *d_mean = [IOSDoubleArray arrayWithLength:((IOSDoubleArray *) nil_chk(IOSObjectArray_Get(nil_chk(distributions), 0)))->size_];
