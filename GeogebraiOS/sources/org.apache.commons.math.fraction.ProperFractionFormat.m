...
- (JavaLangStringBuffer *)formatWithOrgApacheCommonsMathFractionFraction:(OrgApacheCommonsMathFractionFraction *)fraction
  ...
  num = num % den;
  if (whole != 0) {
    [((JavaTextNumberFormat *) nil_chk([self getWholeFormat])) formatWithLong:whole withJavaLangStringBuffer:toAppendTo withJavaTextFieldPosition:pos];
    [((JavaLangStringBuffer *) nil_chk(toAppendTo)) appendWithChar:' '];
    num = JavaLangMath_absWithInt_(num);
  }
  [((JavaTextNumberFormat *) nil_chk([self getNumeratorFormat])) formatWithLong:num withJavaLangStringBuffer:toAppendTo withJavaTextFieldPosition:pos];
  [((JavaLangStringBuffer *) nil_chk(toAppendTo)) appendWithNSString:@" / "];
  [((JavaTextNumberFormat *) nil_chk([self getDenominatorFormat])) formatWithLong:den withJavaLangStringBuffer:toAppendTo withJavaTextFieldPosition:pos];
  return toAppendTo;
}
...
- (OrgApacheCommonsMathFractionFraction *)parseWithNSString:(NSString *)source
  ...
  switch (c) {
    case 0:
    return [new_OrgApacheCommonsMathFractionFraction_initWithInt_withInt_([num intValue], 1) autorelease];
    case '/':
    break;
    default:
    [pos setIndexWithInt:initialIndex];
  ...
  jint n = [num intValue];
  jint d = [den intValue];
  return [new_OrgApacheCommonsMathFractionFraction_initWithInt_withInt_(((JavaLangMath_absWithInt_(w) * d) + n) * OrgApacheCommonsMathUtilMathUtils_signWithInt_(w), d) autorelease];
}
...
- (void)setWholeFormatWithJavaTextNumberFormat:(JavaTextNumberFormat *)format {
  ...
  OrgApacheCommonsMathFractionProperFractionFormat_set_wholeFormat_(self, format);
}
...
void OrgApacheCommonsMathFractionProperFractionFormat_init(OrgApacheCommonsMathFractionProperFractionFormat *self) {
  OrgApacheCommonsMathFractionProperFractionFormat_initWithJavaTextNumberFormat_(self, OrgApacheCommonsMathFractionFractionFormat_getDefaultNumberFormat());
}
