...
- (jint)getIndexWithEduUciIcsJungGraphGraph:(id<EduUciIcsJungGraphGraph>)graph
  ...
  for (id __strong other in commonEdgeSet) {
    if ([nil_chk(e) isEqual:other] == NO) {
      [((id<JavaUtilMap>) nil_chk(edge_index_)) putWithId:EduUciIcsJungGraphUtilContext_getInstanceWithId_withId_(graph, other) withId:JavaLangInteger_valueOfWithInt_(count)];
      count++;
    }
  }
  [((id<JavaUtilMap>) nil_chk(edge_index_)) putWithId:EduUciIcsJungGraphUtilContext_getInstanceWithId_withId_(graph, e) withId:JavaLangInteger_valueOfWithInt_(count)];
  return count;
}
...
void EduUciIcsJungGraphUtilDefaultParallelEdgeIndexFunction_init(EduUciIcsJungGraphUtilDefaultParallelEdgeIndexFunction *self) {
  NSObject_init(self);
  EduUciIcsJungGraphUtilDefaultParallelEdgeIndexFunction_setAndConsume_edge_index_(self, new_JavaUtilHashMap_init());
}
...
EduUciIcsJungGraphUtilDefaultParallelEdgeIndexFunction *EduUciIcsJungGraphUtilDefaultParallelEdgeIndexFunction_getInstance() {
  ...
  return [new_EduUciIcsJungGraphUtilDefaultParallelEdgeIndexFunction_init() autorelease];
}
