...
//  source: /Users/jacky/GSoC/geogebra_iOS/geogebra/geogebra/common/src/edu/uci/ics/jung/graph/SetHypergraph.java
//
...
- (jboolean)addEdgeWithId:(id)hyperedge
  ...
  if (hyperedge == nil) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"input hyperedge may not be null") autorelease];
  if (to_attach == nil) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"endpoints may not be null") autorelease];
  if ([((id<JavaUtilCollection>) nil_chk(to_attach)) containsWithId:nil]) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"cannot add an edge with a null endpoint") autorelease];
  id<JavaUtilSet> new_endpoints = [new_JavaUtilHashSet_initWithJavaUtilCollection_(to_attach) autorelease];
  if ([((id<JavaUtilMap>) nil_chk(edges_)) containsKeyWithId:hyperedge]) {
    ...
    if (![((id<JavaUtilCollection>) nil_chk(attached)) isEqual:new_endpoints]) {
      @throw [new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@$@", @"Edge ", hyperedge, @" exists in this graph with endpoints ", attached)) autorelease];
    }
    else return NO;
  }
  [edges_ putWithId:hyperedge withId:new_endpoints];
...
withEduUciIcsJungGraphUtilEdgeTypeEnum:(EduUciIcsJungGraphUtilEdgeTypeEnum *)edge_type {
  if (edge_type != EduUciIcsJungGraphUtilEdgeTypeEnum_get_UNDIRECTED()) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@", @"Edge type for this implementation must be EdgeType.HYPER, not ", edge_type)) autorelease];
...
- (id<JavaUtilCollection>)getNeighborsWithId:(id)vertex {
  ...
  id<JavaUtilSet> neighbors = [new_JavaUtilHashSet_init() autorelease];
...
- (jboolean)addVertexWithId:(id)vertex {
  if (vertex == nil) @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"cannot add a null vertex") autorelease];
  if ([self containsVertexWithId:vertex]) return NO;
  [((id<JavaUtilMap>) nil_chk(vertices_)) putWithId:vertex withId:[new_JavaUtilHashSet_init() autorelease]];
  return YES;
}
...
- (EduUciIcsJungGraphSetHypergraph *)newInstance {
  return [new_EduUciIcsJungGraphSetHypergraph_init() autorelease];
}
...
id<OrgApacheCommonsCollections15Factory> EduUciIcsJungGraphSetHypergraph_getFactory() {
  ...
  return [new_EduUciIcsJungGraphSetHypergraph_$1_init() autorelease];
}

void EduUciIcsJungGraphSetHypergraph_init(EduUciIcsJungGraphSetHypergraph *self) {
  NSObject_init(self);
  EduUciIcsJungGraphSetHypergraph_setAndConsume_vertices_(self, new_JavaUtilHashMap_init());
  EduUciIcsJungGraphSetHypergraph_setAndConsume_edges_(self, new_JavaUtilHashMap_init());
}
