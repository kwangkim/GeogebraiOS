...
- (void)addWithJavaLangComparable:(id<JavaLangComparable>)x {
  ...
  size__ += 1;
  jint hole = size__;
  [items_ setWithInt:0 withId:x];
  for (; [((id<JavaLangComparable>) nil_chk(x)) compareToWithId:[items_ getWithInt:hole / 2]] < 0; hole /= 2) {
    [items_ setWithInt:hole withId:[items_ getWithInt:hole / 2]];
  }
  [items_ setWithInt:hole withId:x];
}
...
- (id)poll {
  ...
  [items_ setWithInt:1 withId:[items_ getWithInt:size__]];
  size__ -= 1;
...
void ComVividsolutionsJtsUtilPriorityQueue_init(ComVividsolutionsJtsUtilPriorityQueue *self) {
  NSObject_init(self);
  self->size__ = 0;
  ComVividsolutionsJtsUtilPriorityQueue_setAndConsume_items_(self, new_JavaUtilArrayList_init());
...
void ComVividsolutionsJtsUtilPriorityQueue_reorderWithInt_(ComVividsolutionsJtsUtilPriorityQueue *self, jint hole) {
  ...
  for (; hole * 2 <= self->size__; hole = child) {
    ...
    if ([((id<JavaLangComparable>) nil_chk(((id<JavaLangComparable>) check_protocol_cast([self->items_ getWithInt:child], @protocol(JavaLangComparable))))) compareToWithId:tmp] < 0) [self->items_ setWithInt:hole withId:[self->items_ getWithInt:child]];
    else break;
  }
  [self->items_ setWithInt:hole withId:tmp];
}
