...
- (void)next {
  if (prevcurve_ != nil) {
    OrgGeogebraGgbjdkJavaAwtGeomAreaIterator_set_prevcurve_(self, nil);
  }
  else {
    OrgGeogebraGgbjdkJavaAwtGeomAreaIterator_set_prevcurve_(self, thiscurve_);
    index_++;
    if (index_ < [((JavaUtilVector *) nil_chk(curves_)) size]) {
      OrgGeogebraGgbjdkJavaAwtGeomAreaIterator_set_thiscurve_(self, (OrgGeogebraGgbjdkSunAwtGeomCurve *) check_class_cast([curves_ getWithInt:index_], [OrgGeogebraGgbjdkSunAwtGeomCurve class]));
      ...
      OrgGeogebraGgbjdkJavaAwtGeomAreaIterator_set_thiscurve_(self, nil);
    }
  }
}

- (jint)currentSegmentWithFloatArray:(IOSFloatArray *)coords {
  IOSDoubleArray *dcoords = [IOSDoubleArray arrayWithLength:6];
...
- (jint)currentSegmentWithDoubleArray:(IOSDoubleArray *)coords {
  ...
  if (prevcurve_ != nil) {
    ...
    *IOSDoubleArray_GetRef(coords, 1) = [thiscurve_ getY0];
    ...
    @throw [new_JavaUtilNoSuchElementException_initWithNSString_(@"area iterator out of bounds") autorelease];
  }
  else {
...
void OrgGeogebraGgbjdkJavaAwtGeomAreaIterator_initWithJavaUtilVector_withOrgGeogebraCommonAwtGAffineTransform_(OrgGeogebraGgbjdkJavaAwtGeomAreaIterator *self, JavaUtilVector *curves, id<OrgGeogebraCommonAwtGAffineTransform> at) {
  NSObject_init(self);
  OrgGeogebraGgbjdkJavaAwtGeomAreaIterator_set_curves_(self, curves);
  OrgGeogebraGgbjdkJavaAwtGeomAreaIterator_set_transform_(self, at);
  if ([((JavaUtilVector *) nil_chk(curves)) size] >= 1) {
    OrgGeogebraGgbjdkJavaAwtGeomAreaIterator_set_thiscurve_(self, (OrgGeogebraGgbjdkSunAwtGeomCurve *) check_class_cast([curves getWithInt:0], [OrgGeogebraGgbjdkSunAwtGeomCurve class]));
  }
}
