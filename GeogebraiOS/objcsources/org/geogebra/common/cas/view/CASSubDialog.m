...
- (void)initDataWithOrgGeogebraCommonKernelGeosGeoCasCell:(OrgGeogebraCommonKernelGeosGeoCasCell *)cell {
  JavaUtilHashSet *vars = new_JavaUtilHashSet_init();
  ...
  JavaUtilVector *row;
  data_ = new_JavaUtilVector_initWithInt_([vars size] + 1);
  id<JavaUtilIterator> iter = [vars iterator];
  while ([((id<JavaUtilIterator>) nil_chk(iter)) hasNext]) {
    row = new_JavaUtilVector_initWithInt_(2);
...
- (jboolean)applyWithNSString:(NSString *)actionCommand {
  ...
  JavaLangStringBuilder *substList = new_JavaLangStringBuilder_initWithNSString_(@"{");
  JavaLangStringBuilder *substComment = new_JavaLangStringBuilder_init();
  for (jint i = 0; i < [((JavaUtilVector *) nil_chk(data_)) size]; i++) {
    ...
    if (![((NSString *) nil_chk(fromExpr)) isEqual:@""] && ![((NSString *) nil_chk(toExpr)) isEqual:@""]) {
      if ([substList length] > 1) {
        (void) [substList appendWithChar:','];
        (void) [substComment appendWithChar:','];
      }
      ...
      (void) [substList appendWithNSString:fromExpr];
      (void) [substList appendWithChar:'='];
      (void) [substList appendWithNSString:toExpr];
      (void) [substComment appendWithNSString:fromExpr];
      (void) [substComment appendWithChar:'='];
      (void) [substComment appendWithNSString:toExpr];
    }
  }
  (void) [substList appendWithChar:'}'];
