...
- (OrgGeogebraCommonCasGeoGebraCAS *)getCAS {
  @synchronized(self) {
    if (cas_ == nil) {
      OrgGeogebraCommonCasViewCASView_set_cas_(self, (OrgGeogebraCommonCasGeoGebraCAS *) check_class_cast([((OrgGeogebraCommonKernelKernel *) nil_chk(kernel_)) getGeoGebraCAS], [OrgGeogebraCommonCasGeoGebraCAS class]));
    }
    return cas_;
  }
}
...
- (void)ensureOneEmptyRow {
  ...
  if (rows == 0 || ![self isRowOutputEmptyWithInt:rows - 1] || [((OrgGeogebraCommonKernelGeosGeoCasCell *) nil_chk([((id<OrgGeogebraCommonCasViewCASTable>) nil_chk([self getConsoleTable])) getGeoCasCellWithInt:rows - 1])) isInConstructionList]) {
    OrgGeogebraCommonKernelGeosGeoCasCell *casCell = [new_OrgGeogebraCommonKernelGeosGeoCasCell_initWithOrgGeogebraCommonKernelConstruction_([((OrgGeogebraCommonKernelKernel *) nil_chk(kernel_)) getConstruction]) autorelease];
...
- (void)insertRowWithOrgGeogebraCommonKernelGeosGeoCasCell:(OrgGeogebraCommonKernelGeosGeoCasCell *)newValue
  ...
  if ([self isRowEmptyWithInt:lastRow]) {
    if (toInsert == nil) {
      toInsert = [new_OrgGeogebraCommonKernelGeosGeoCasCell_initWithOrgGeogebraCommonKernelConstruction_([((OrgGeogebraCommonKernelKernel *) nil_chk(kernel_)) getConstruction]) autorelease];
    }
