...
- (void)pinCmdWithBoolean:(jboolean)isSelected {
  ...
  for (jint i = [((JavaUtilArrayList *) nil_chk(geos2)) size] - 1; i >= 0; i--) {
    ...
    if ([OrgGeogebraCommonKernelGeosAbsoluteScreenLocateable_class_() isInstance:geo1] && ![((OrgGeogebraCommonKernelGeosGeoElement *) nil_chk(geo1)) isGeoList]) {
      ...
      OrgGeogebraCommonEuclidianEuclidianStyleBarStatic_applyFixPositionWithJavaUtilArrayList_withBoolean_withOrgGeogebraCommonEuclidianEuclidianViewInterfaceCommon_(geos2, isSelected, [((OrgGeogebraCommonMainApp *) nil_chk(app_)) getActiveEuclidianView]);
    }
  }
  [((OrgGeogebraCommonMainApp *) nil_chk(app_)) storeUndoInfo];
}
...
+ (void)initialize {
  if (self == [OrgGeogebraCommonGuiContextMenuGeoElement class]) {
    JreStrongAssignAndConsume(&OrgGeogebraCommonGuiContextMenuGeoElement_zoomFactors_, nil, [IOSDoubleArray newArrayWithDoubles:(jdouble[]){ 4.0, 2.0, 1.5, 1.25, 1.0 / 1.25, 1.0 / 1.5, 0.5, 0.25 } count:8]);
    JreStrongAssignAndConsume(&OrgGeogebraCommonGuiContextMenuGeoElement_axesRatios_, nil, [IOSDoubleArray newArrayWithDoubles:(jdouble[]){ 1.0 / 1000.0, 1.0 / 500.0, 1.0 / 200.0, 1.0 / 100.0, 1.0 / 50.0, 1.0 / 20.0, 1.0 / 10.0, 1.0 / 5.0, 1.0 / 2.0, 1, 2, 5, 10, 20, 50, 100, 200, 500, 1000 } count:19]);
