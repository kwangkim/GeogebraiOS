...
JavaUtilArrayList *OrgGeogebraCommonGuiViewDataANOVAStatTableModel_getCategoryDataWithOrgGeogebraCommonKernelGeosGeoList_(OrgGeogebraCommonKernelGeosGeoList *geoList) {
  ...
  JavaUtilArrayList *categoryData = new_JavaUtilArrayList_init();
  ...
  for (jint index = 0; index < [((OrgGeogebraCommonKernelGeosGeoList *) nil_chk(geoList)) size]; index++) {
    ...
    IOSDoubleArray *valueArray = [IOSDoubleArray newArrayWithLength:[((OrgGeogebraCommonKernelGeosGeoList *) nil_chk(list)) size]];
...
OrgGeogebraCommonGuiViewDataANOVAStatTableModel_AnovaStats *OrgGeogebraCommonGuiViewDataANOVAStatTableModel_anovaStatsWithJavaUtilCollection_(id<JavaUtilCollection> categoryData) {
  ...
  if ([((id<JavaUtilCollection>) nil_chk(categoryData)) size] < 2) {
    @throw OrgApacheCommonsMathMathRuntimeException_createIllegalArgumentExceptionWithNSString_withNSObjectArray_(@"two or more categories required, got {0}", [IOSObjectArray newArrayWithObjects:(id[]){ JavaLangInteger_valueOfWithInt_([categoryData size]) } count:1 type:NSObject_class_()]);
  }
  for (IOSDoubleArray * __strong array in categoryData) {
    if (((IOSDoubleArray *) nil_chk(array))->size_ <= 1) {
      @throw OrgApacheCommonsMathMathRuntimeException_createIllegalArgumentExceptionWithNSString_withNSObjectArray_(@"two or more values required in each category, one has {0}", [IOSObjectArray newArrayWithObjects:(id[]){ JavaLangInteger_valueOfWithInt_(array->size_) } count:1 type:NSObject_class_()]);
    }
  }
  jint dfwg = 0;
  jdouble sswg = 0;
  OrgApacheCommonsMathStatDescriptiveSummarySum *totsum = new_OrgApacheCommonsMathStatDescriptiveSummarySum_init();
  OrgApacheCommonsMathStatDescriptiveSummarySumOfSquares *totsumsq = new_OrgApacheCommonsMathStatDescriptiveSummarySumOfSquares_init();
  jint totnum = 0;
  for (IOSDoubleArray * __strong data in categoryData) {
    OrgApacheCommonsMathStatDescriptiveSummarySum *sum = new_OrgApacheCommonsMathStatDescriptiveSummarySum_init();
    OrgApacheCommonsMathStatDescriptiveSummarySumOfSquares *sumsq = new_OrgApacheCommonsMathStatDescriptiveSummarySumOfSquares_init();
    jint num = 0;
  ...
  jdouble F = msbg / mswg;
  id<OrgApacheCommonsMathDistributionFDistribution> fdist = new_OrgApacheCommonsMathDistributionFDistributionImpl_initWithDouble_withDouble_(dfbg, dfwg);
  jdouble P = 1.0 - [fdist cumulativeProbabilityWithDouble:F];
  return new_OrgGeogebraCommonGuiViewDataANOVAStatTableModel_AnovaStats_initWithInt_withInt_withDouble_withDouble_withDouble_withDouble_withDouble_withDouble_withDouble_(dfbg, dfwg, F, P, ssbg, sswg, sst, msbg, mswg);
}
