...
- (void)setColumnNames {
  OrgGeogebraCommonGuiViewProbcalculatorProbabilityTable_setAndConsume_columnNames_(self, [IOSObjectArray newArrayWithLength:2 type:NSString_class_()]);
  IOSObjectArray_Set(columnNames_, 0, @"k");
  if ([self isCumulative]) IOSObjectArray_Set(columnNames_, 1, JreStrcat("$$C$$", [((OrgGeogebraCommonMainApp *) nil_chk(app_)) getMenuWithNSString:@"ProbabilityOf"], @"X ", OrgGeogebraCommonUtilUnicode_LESS_EQUAL, @" k", [app_ getMenuWithNSString:@"EndProbabilityOf"]));
  else IOSObjectArray_Set(columnNames_, 1, JreStrcat("$$$", [((OrgGeogebraCommonMainApp *) nil_chk(app_)) getMenuWithNSString:@"ProbabilityOf"], @"X = k", [app_ getMenuWithNSString:@"EndProbabilityOf"]));
}
