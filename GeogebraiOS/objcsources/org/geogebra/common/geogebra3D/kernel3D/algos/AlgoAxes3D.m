...
- (void)createInput {
  ...
  axes_ = [IOSObjectArray newArrayWithLength:d type:OrgGeogebraCommonGeogebra3DKernel3DGeosGeoLine3D_class_()];
  for (jint i = 0; i < d; i++) {
    (void) IOSObjectArray_Set(axes_, i, new_OrgGeogebraCommonGeogebra3DKernel3DGeosGeoLine3D_initWithOrgGeogebraCommonKernelConstruction_(cons_));
  }
}

- (void)compute {
  midpoint_ = [((OrgGeogebraCommonKernelKernelNDGeoQuadricND *) nil_chk(c_)) getMidpoint3D];
  [super compute];
}
