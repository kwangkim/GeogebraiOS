...
- (void)inverseImmediate {
  @try {
    id<OrgApacheCommonsMathLinearDecompositionSolver> d = [new_OrgApacheCommonsMathLinearLUDecompositionImpl_initWithOrgApacheCommonsMathLinearRealMatrix_withDouble_(self, OrgGeogebraCommonKernelKernel_STANDARD_PRECISION) getSolver];
    id<OrgApacheCommonsMathLinearRealMatrix> ret = [((id<OrgApacheCommonsMathLinearDecompositionSolver>) nil_chk(d)) getInverse];
    data_ = [((id<OrgApacheCommonsMathLinearRealMatrix>) nil_chk(ret)) getData];
  }
...
- (jdouble)determinant {
  return [new_OrgApacheCommonsMathLinearLUDecompositionImpl_initWithOrgApacheCommonsMathLinearRealMatrix_withDouble_(self, OrgGeogebraCommonKernelKernel_STANDARD_PRECISION) getDeterminant];
}
...
- (void)reducedRowEchelonFormImmediate {
  ...
  for (jint r = 0; r < rowCount; r++) {
    if (lead >= columnCount) break;
    {
      ...
      (void) IOSObjectArray_Set(data_, r, IOSObjectArray_Get(data_, i));
      (void) IOSObjectArray_Set(data_, i, temp);
    }
    {
...
- (void)transposeImmediate {
  ...
  IOSObjectArray *C = [IOSDoubleArray newArrayWithDimensions:2 lengths:(jint[]){ n, m }];
  for (jint i = 0; i < m; i++) {
    for (jint j = 0; j < n; j++) {
...
- (void)getGeoListWithOrgGeogebraCommonKernelGeosGeoList:(OrgGeogebraCommonKernelGeosGeoList *)outputList
  ...
  for (jint r = 0; r < [self getRowDimension]; r++) {
    ...
    for (jint c = 0; c < [self getColumnDimension]; c++) {
      [columnList addWithOrgGeogebraCommonKernelGeosGeoElement:new_OrgGeogebraCommonKernelGeosGeoNumeric_initWithOrgGeogebraCommonKernelConstruction_withDouble_(cons, [self getEntryWithInt:r withInt:c])];
    }
...
- (void)getMyListWithOrgGeogebraCommonKernelArithmeticMyList:(OrgGeogebraCommonKernelArithmeticMyList *)outputList
  ...
  for (jint r = 0; r < [self getRowDimension]; r++) {
    OrgGeogebraCommonKernelArithmeticMyList *columnList = new_OrgGeogebraCommonKernelArithmeticMyList_initWithOrgGeogebraCommonKernelKernel_(kernel);
    for (jint c = 0; c < [self getColumnDimension]; c++) {
      [columnList addListElementWithOrgGeogebraCommonKernelArithmeticExpressionValue:new_OrgGeogebraCommonKernelGeosGeoNumeric_initWithOrgGeogebraCommonKernelConstruction_withDouble_([((OrgGeogebraCommonKernelKernel *) nil_chk(kernel)) getConstruction], [self getEntryWithInt:r withInt:c])];
    }
...
void OrgGeogebraCommonUtilGgbMat_initWithOrgGeogebraCommonKernelGeosGeoList_(OrgGeogebraCommonUtilGgbMat *self, OrgGeogebraCommonKernelGeosGeoList *inputList) {
  (void) OrgApacheCommonsMathLinearArray2DRowRealMatrix_init(self);
  ...
  self->data_ = [IOSDoubleArray newArrayWithDimensions:2 lengths:(jint[]){ rows, cols }];
