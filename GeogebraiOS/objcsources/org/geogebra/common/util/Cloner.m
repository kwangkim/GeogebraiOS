...
IOSDoubleArray *OrgGeogebraCommonUtilCloner_cloneWithDoubleArray_(IOSDoubleArray *array) {
  ...
  IOSDoubleArray *arrayClone = [IOSDoubleArray newArrayWithLength:((IOSDoubleArray *) nil_chk(array))->size_];
...
IOSIntArray *OrgGeogebraCommonUtilCloner_cloneWithIntArray_(IOSIntArray *array) {
  OrgGeogebraCommonUtilCloner_initialize();
  IOSIntArray *arrayClone = [IOSIntArray newArrayWithLength:((IOSIntArray *) nil_chk(array))->size_];
  for (jint i = 0; i < array->size_; i++) {
...
IOSObjectArray *OrgGeogebraCommonUtilCloner_clone2WithDoubleArray2_(IOSObjectArray *array) {
  OrgGeogebraCommonUtilCloner_initialize();
  IOSObjectArray *arrayClone = [IOSDoubleArray newArrayWithDimensions:2 lengths:(jint[]){ ((IOSObjectArray *) nil_chk(array))->size_, ((IOSDoubleArray *) nil_chk(IOSObjectArray_Get(array, 0)))->size_ }];
  for (jint i = 0; i < array->size_; i++) {
    (void) IOSObjectArray_Set(arrayClone, i, OrgGeogebraCommonUtilCloner_cloneWithDoubleArray_(IOSObjectArray_Get(array, i)));
  }
  return arrayClone;
}
