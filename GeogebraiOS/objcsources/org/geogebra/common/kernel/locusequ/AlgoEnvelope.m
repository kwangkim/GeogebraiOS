...
//  source: /Users/jacky/GSoC/GeoGebraiOSARC/GeoGebraiOSARC/javasources/org/geogebra/common/kernel/locusequ/AlgoEnvelope.java
//
...
- (void)setInputOutput {
  ...
  efficientInput_AlgoEnvelope_ = [IOSObjectArray newArrayWithLength:[inSet size] type:OrgGeogebraCommonKernelGeosGeoElement_class_()];
  efficientInput_AlgoEnvelope_ = [inSet toArrayWithNSObjectArray:efficientInput_AlgoEnvelope_];
  standardInput_ = [IOSObjectArray newArrayWithLength:2 type:OrgGeogebraCommonKernelGeosGeoElement_class_()];
  (void) IOSObjectArray_Set(standardInput_, 0, self->path_);
  (void) IOSObjectArray_Set(standardInput_, 1, self->movingPoint_);
...
void OrgGeogebraCommonKernelLocusequAlgoEnvelope_initWithOrgGeogebraCommonKernelConstruction_withNSString_withOrgGeogebraCommonKernelGeosGeoElement_withOrgGeogebraCommonKernelGeosGeoPoint_(OrgGeogebraCommonKernelLocusequAlgoEnvelope *self, OrgGeogebraCommonKernelConstruction *cons, NSString *label, OrgGeogebraCommonKernelGeosGeoElement *path, OrgGeogebraCommonKernelGeosGeoPoint *movingPoint) {
  (void) OrgGeogebraCommonKernelLocusequAlgoEnvelope_initWithOrgGeogebraCommonKernelConstruction_withOrgGeogebraCommonKernelGeosGeoElement_withOrgGeogebraCommonKernelGeosGeoPoint_(self, cons, path, movingPoint);
...
void OrgGeogebraCommonKernelLocusequAlgoEnvelope_initWithOrgGeogebraCommonKernelConstruction_withOrgGeogebraCommonKernelGeosGeoElement_withOrgGeogebraCommonKernelGeosGeoPoint_(OrgGeogebraCommonKernelLocusequAlgoEnvelope *self, OrgGeogebraCommonKernelConstruction *cons, OrgGeogebraCommonKernelGeosGeoElement *path, OrgGeogebraCommonKernelGeosGeoPoint *movingPoint) {
  ...
  self->path_ = path;
  self->geoPoly_ = new_OrgGeogebraCommonKernelImplicitGeoImplicitPoly_initWithOrgGeogebraCommonKernelConstruction_(cons);
  [self setInputOutput];
  [self compute];
}
...
NSString *OrgGeogebraCommonKernelLocusequAlgoEnvelope_getImplicitPoly(OrgGeogebraCommonKernelLocusequAlgoEnvelope *self) {
  ...
  OrgGeogebraCommonKernelLocusequEquationScope *scope = new_OrgGeogebraCommonKernelLocusequEquationScope_initWithOrgGeogebraCommonKernelGeosGeoElement_withOrgGeogebraCommonKernelGeosGeoPoint_(self->path_, self->movingPoint_);
  ...
  OrgGeogebraCommonKernelLocusequEquationList *restrictions = new_OrgGeogebraCommonKernelLocusequEquationList_init();
  OrgGeogebraCommonKernelAlgosAlgoElement *algo;
  id<JavaUtilSet> visitedAlgos = new_JavaUtilHashSet_init();
  ...
  OrgGeogebraCommonKernelGeosGeoPoint *locusPoint = new_OrgGeogebraCommonKernelGeosGeoPoint_initWithOrgGeogebraCommonKernelConstruction_(self->cons_);
  ...
  OrgGeogebraCommonKernelLocusequEquationList *last = new_OrgGeogebraCommonKernelLocusequEquationList_init();
  ...
  OrgGeogebraCommonKernelLocusequCASTranslator *ct = new_OrgGeogebraCommonKernelLocusequCASTranslator_initWithOrgGeogebraCommonKernelKernel_(self->kernel_);
  OrgGeogebraCommonKernelLocusequEquationSystem *es = new_OrgGeogebraCommonKernelLocusequEquationSystem_initWithOrgGeogebraCommonKernelLocusequEquationList_withOrgGeogebraCommonKernelLocusequEquationScope_(restrictions, scope);
  id<JavaUtilCollection> restrictionsT = [ct translateWithOrgGeogebraCommonKernelLocusequEquationSystem:es];
  es = new_OrgGeogebraCommonKernelLocusequEquationSystem_initWithOrgGeogebraCommonKernelLocusequEquationList_withOrgGeogebraCommonKernelLocusequEquationScope_(last, scope);
  ...
  IOSIntArray *scopeVarsI = [IOSIntArray newArrayWithLength:varsN];
  ...
  IOSObjectArray *lastS = [IOSObjectArray newArrayWithLength:[((id<JavaUtilCollection>) nil_chk(lastT)) size] type:NSString_class_()];
  it = [lastT iterator];
  i = 0;
  while ([((id<JavaUtilIterator>) nil_chk(it)) hasNext]) {
    ...
    (void) IOSObjectArray_Set(lastS, i++, eq);
  }
  NSString *vars = @"";
  for (i = 0; i < varsN - 2; ++i) {
  ...
  if (((jint) [((NSString *) nil_chk(locusLib)) length]) == 0) {
    (void) [script appendWithNSString:@"[["];
    (void) [((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk([script appendWithNSString:@"m:=["])) appendWithNSString:polys])) appendWithNSString:@"]],[J:=det("];
    for (i = 0; i < varsN - 2; ++i) {
      (void) [script appendWithNSString:@"["];
      for (jint j = 0; j < varsN - 2; ++j) {
        (void) [script appendWithNSString:JreStrcat("$I$IC", @"diff(m[", i, @"],x", IOSIntArray_Get(scopeVarsI, j), ')')];
        if (j != varsN - 3) {
          (void) [script appendWithNSString:@","];
        }
      }
    ...
    (void) [script appendWithNSString:@")]][1][0]"];
  ...
  (void) [script appendWithNSString:@"proc mylocusdgto(list L) {poly p=1;int i; int j; int k;for(i=1;i<=size(L);i++) { if(L[i][3]<>\"Degenerate\") { if(size(L[i][1])>1) {p=p*((L[i][1][1])^2+(L[i][1][2])^2);}else {p=p*L[i][1][1];}} } return(p); }"];
  (void) [script appendWithNSString:@"proc myenvelopeto (list GG) {list GGG;if (GG[1][2][1]<>1) { GGG=delete(GG,1); }else { GGG=GG; };string SLo=locusto(locus(GGG));if (find(SLo,\"Normal\") == 0 and find(SLo,\"Accumulation\") == 0 and find(SLo,\"Special\") == 0){ return(1); }else { return(mylocusdgto(locus(GGG))); } }"];
  (void) [((JavaLangStringBuilder *) nil_chk([script appendWithNSString:JreStrcat("$$$$", @"LIB \"", locusLib, @".lib\";ring r=(0,x,y),(", vars)])) appendWithNSString:@"),dp;short=0;ideal m="];
  (void) [script appendWithNSString:polys];
  (void) [script appendWithNSString:JreStrcat("$$$", @";poly D=det(jacob(m));ideal S=", polys, @",D;list e=myenvelopeto(grobcov(S));")];
  (void) [script appendWithNSString:@"string ex=\"poly p=\" + string(e[1]);"];
  (void) [script appendWithNSString:@"ring rr=0,(x,y),dp;"];
  (void) [script appendWithNSString:@"execute(ex);"];
  (void) [((JavaLangStringBuilder *) nil_chk([script appendWithNSString:@"printf(\"%s,%s,%s\",size(coeffs(p,x)),size(coeffs(p,y)),"])) appendWithNSString:@"coeffs(coeffs(p,x),y));"];
