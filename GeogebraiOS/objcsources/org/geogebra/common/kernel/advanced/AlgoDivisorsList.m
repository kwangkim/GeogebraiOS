...
- (void)setInputOutput {
  ...
  input_ = [IOSObjectArray newArrayWithObjects:(id[]){ [((id<OrgGeogebraCommonKernelArithmeticNumberValue>) nil_chk(number_)) toGeoElement] } count:1 type:OrgGeogebraCommonKernelGeosGeoElement_class_()];
...
- (void)compute {
  ...
  [result_ clear];
  id<JavaUtilSet> sortedSet = new_JavaUtilTreeSet_init();
  [sortedSet addAllWithJavaUtilCollection:factList_];
  id<JavaUtilIterator> iterator = [sortedSet iterator];
  while ([((id<JavaUtilIterator>) nil_chk(iterator)) hasNext]) {
    [result_ addWithOrgGeogebraCommonKernelGeosGeoElement:new_OrgGeogebraCommonKernelGeosGeoNumeric_initWithOrgGeogebraCommonKernelConstruction_withDouble_(cons_, [((JavaLangLong *) nil_chk([iterator next])) longLongValue])];
  }
}
...
void OrgGeogebraCommonKernelAdvancedAlgoDivisorsList_initWithOrgGeogebraCommonKernelConstruction_withNSString_withOrgGeogebraCommonKernelArithmeticNumberValue_(OrgGeogebraCommonKernelAdvancedAlgoDivisorsList *self, OrgGeogebraCommonKernelConstruction *cons, NSString *label, id<OrgGeogebraCommonKernelArithmeticNumberValue> number) {
  ...
  self->factList_ = new_JavaUtilArrayList_init();
  self->number_ = number;
  self->factors_ = new_OrgGeogebraCommonKernelCasAlgoPrimeFactorization_initWithOrgGeogebraCommonKernelConstruction_withOrgGeogebraCommonKernelArithmeticNumberValue_(cons, number);
  self->factorList_ = [self->factors_ getResult];
