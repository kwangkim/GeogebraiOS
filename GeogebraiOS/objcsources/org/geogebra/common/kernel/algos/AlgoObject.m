...
- (void)setInputOutput {
  ...
  inputForUpdateSetPropagation_ = [IOSObjectArray newArrayWithLength:2 type:OrgGeogebraCommonKernelGeosGeoElement_class_()];
  (void) IOSObjectArray_Set(inputForUpdateSetPropagation_, 0, text_);
...
void OrgGeogebraCommonKernelAlgosAlgoObject_updateReferencedObject(OrgGeogebraCommonKernelAlgosAlgoObject *self) {
  self->currentLabel_ = [((OrgGeogebraCommonKernelGeosGeoText *) nil_chk(self->text_)) getTextString];
  self->refObject_ = [((OrgGeogebraCommonKernelKernel *) nil_chk(self->kernel_)) lookupLabelWithNSString:self->currentLabel_];
  (void) IOSObjectArray_Set(nil_chk(self->inputForUpdateSetPropagation_), 1, self->refObject_);
