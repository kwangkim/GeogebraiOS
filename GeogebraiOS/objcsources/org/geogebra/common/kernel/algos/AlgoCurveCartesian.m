...
- (OrgGeogebraCommonKernelKernelNDGeoCurveCartesianND *)createCurveWithOrgGeogebraCommonKernelConstruction:(OrgGeogebraCommonKernelConstruction *)cons
  ...
  return [new_OrgGeogebraCommonKernelGeosGeoCurveCartesian_initWithOrgGeogebraCommonKernelConstruction_withOrgGeogebraCommonKernelArithmeticFunction_withOrgGeogebraCommonKernelArithmeticFunction_(cons, IOSObjectArray_Get(nil_chk(fun), 0), IOSObjectArray_Get(fun, 1)) autorelease];
}
...
- (void)setInputOutput {
  OrgGeogebraCommonKernelAlgosAlgoElement_setAndConsume_input_(self, [IOSObjectArray newArrayWithLength:((IOSObjectArray *) nil_chk(coords_))->size_ + 3 type:OrgGeogebraCommonKernelGeosGeoElement_class_()]);
  ...
  IOSObjectArray_Set(input_, coords_->size_ + 1, [((id<OrgGeogebraCommonKernelArithmeticNumberValue>) nil_chk(from_)) toGeoElement]);
  IOSObjectArray_Set(input_, coords_->size_ + 2, [((id<OrgGeogebraCommonKernelArithmeticNumberValue>) nil_chk(to_)) toGeoElement]);
...
void OrgGeogebraCommonKernelAlgosAlgoCurveCartesian_initWithOrgGeogebraCommonKernelConstruction_withNSString_withOrgGeogebraCommonKernelArithmeticNumberValueArray_withOrgGeogebraCommonKernelGeosGeoNumeric_withOrgGeogebraCommonKernelArithmeticNumberValue_withOrgGeogebraCommonKernelArithmeticNumberValue_(OrgGeogebraCommonKernelAlgosAlgoCurveCartesian *self, OrgGeogebraCommonKernelConstruction *cons, NSString *label, IOSObjectArray *coords, OrgGeogebraCommonKernelGeosGeoNumeric *localVar, id<OrgGeogebraCommonKernelArithmeticNumberValue> from, id<OrgGeogebraCommonKernelArithmeticNumberValue> to) {
  ...
  OrgGeogebraCommonKernelAlgosAlgoCurveCartesian_set_coords_(self, coords);
  OrgGeogebraCommonKernelAlgosAlgoCurveCartesian_set_from_(self, from);
  OrgGeogebraCommonKernelAlgosAlgoCurveCartesian_set_to_(self, to);
  OrgGeogebraCommonKernelAlgosAlgoCurveCartesian_set_localVar_(self, localVar);
  OrgGeogebraCommonKernelArithmeticFunctionVariable *funVar = [new_OrgGeogebraCommonKernelArithmeticFunctionVariable_initWithOrgGeogebraCommonKernelKernel_(self->kernel_) autorelease];
  [funVar setVarStringWithNSString:[((OrgGeogebraCommonKernelGeosGeoNumeric *) nil_chk(localVar)) getLabelSimple]];
  OrgGeogebraCommonKernelAlgosAlgoCurveCartesian_setAndConsume_exp_(self, [IOSObjectArray newArrayWithLength:((IOSObjectArray *) nil_chk(coords))->size_ type:OrgGeogebraCommonKernelArithmeticExpressionNode_class_()]);
  IOSObjectArray *fun = [IOSObjectArray arrayWithLength:coords->size_ type:OrgGeogebraCommonKernelArithmeticFunction_class_()];
  OrgGeogebraCommonKernelAlgosAlgoCurveCartesian_setAndConsume_containsFunctions_(self, [IOSBooleanArray newArrayWithLength:coords->size_]);
  for (jint i = 0; i < coords->size_; i++) {
    IOSObjectArray_Set(self->exp_, i, [((OrgGeogebraCommonKernelKernel *) nil_chk(self->kernel_)) convertNumberValueToExpressionNodeWithOrgGeogebraCommonKernelGeosGeoElement:[((id<OrgGeogebraCommonKernelArithmeticNumberValue>) nil_chk(IOSObjectArray_Get(coords, i))) toGeoElement]]);
    IOSObjectArray_Set(self->exp_, i, [((id<OrgGeogebraCommonKernelArithmeticExpressionValue>) nil_chk([((OrgGeogebraCommonKernelArithmeticExpressionNode *) nil_chk(IOSObjectArray_Get(self->exp_, i))) replaceWithOrgGeogebraCommonKernelArithmeticExpressionValue:localVar withOrgGeogebraCommonKernelArithmeticExpressionValue:funVar])) wrap]);
    IOSObjectArray_SetAndConsume(fun, i, new_OrgGeogebraCommonKernelArithmeticFunction_initWithOrgGeogebraCommonKernelArithmeticExpressionNode_withOrgGeogebraCommonKernelArithmeticFunctionVariable_(IOSObjectArray_Get(self->exp_, i), funVar));
  ...
  OrgGeogebraCommonKernelAlgosAlgoCurveCartesian_set_curve_(self, [self createCurveWithOrgGeogebraCommonKernelConstruction:cons withOrgGeogebraCommonKernelArithmeticFunctionArray:fun]);
  [self setInputOutput];
  OrgGeogebraCommonKernelAlgosAlgoCurveCartesian_compute(self);
