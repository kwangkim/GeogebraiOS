...
//  source: /Users/jacky/GSoC/GeoGebraiOSARC/GeoGebraiOSARC/javasources/org/geogebra/common/kernel/algos/AlgoDensityPlot.java
//
...
- (void)setInputOutput {
  ...
  (void) IOSObjectArray_Set(input_, 0, function_);
  (void) IOSObjectArray_Set(input_, 1, new_OrgGeogebraCommonKernelGeosGeoNumeric_initWithOrgGeogebraCommonKernelConstruction_withDouble_(cons_, minX_));
  (void) IOSObjectArray_Set(input_, 2, new_OrgGeogebraCommonKernelGeosGeoNumeric_initWithOrgGeogebraCommonKernelConstruction_withDouble_(cons_, maxX_));
  (void) IOSObjectArray_Set(input_, 3, new_OrgGeogebraCommonKernelGeosGeoNumeric_initWithOrgGeogebraCommonKernelConstruction_withDouble_(cons_, minY_));
  (void) IOSObjectArray_Set(input_, 4, new_OrgGeogebraCommonKernelGeosGeoNumeric_initWithOrgGeogebraCommonKernelConstruction_withDouble_(cons_, maxY_));
  (void) IOSObjectArray_Set(input_, 5, new_OrgGeogebraCommonKernelGeosGeoBoolean_initWithOrgGeogebraCommonKernelConstruction_withBoolean_(cons_, fixed_));
...
- (void)compute {
  ...
  for (j_ = offset_, *IOSDoubleArray_GetRef(nil_chk(vals_), 1) = maxY_; j_ < imagePlusOffset_; *IOSDoubleArray_GetRef(vals_, 1) -= incY_, j_ += grade_) {
    for (i_ = offset_, *IOSDoubleArray_GetRef(vals_, 0) = minX_; i_ < imagePlusOffset_; *IOSDoubleArray_GetRef(vals_, 0) += incX_, i_ += grade_) {
      ...
      colors_ = OrgGeogebraCommonKernelAlgosAlgoDensityPlot_rgbColorWithDouble_(value_);
      color_ = [((OrgGeogebraCommonFactoriesAwtFactory *) nil_chk(OrgGeogebraCommonFactoriesAwtFactory_get_prototype_())) newColorWithInt:IOSIntArray_Get(nil_chk(colors_), 0) withInt:IOSIntArray_Get(colors_, 1) withInt:IOSIntArray_Get(colors_, 1)];
...
void OrgGeogebraCommonKernelAlgosAlgoDensityPlot_initWithOrgGeogebraCommonKernelConstruction_withOrgGeogebraCommonKernelGeosGeoFunctionNVar_(OrgGeogebraCommonKernelAlgosAlgoDensityPlot *self, OrgGeogebraCommonKernelConstruction *c, OrgGeogebraCommonKernelGeosGeoFunctionNVar *function) {
  (void) OrgGeogebraCommonKernelAlgosAlgoDensityPlot_initWithOrgGeogebraCommonKernelConstruction_withOrgGeogebraCommonKernelGeosGeoFunctionNVar_withDouble_withDouble_withDouble_withDouble_withBoolean_(self, c, function, -2, 2, -2, 2, NO);
}
...
void OrgGeogebraCommonKernelAlgosAlgoDensityPlot_initWithOrgGeogebraCommonKernelConstruction_withOrgGeogebraCommonKernelGeosGeoFunctionNVar_withDouble_withDouble_withDouble_withDouble_withBoolean_(OrgGeogebraCommonKernelAlgosAlgoDensityPlot *self, OrgGeogebraCommonKernelConstruction *cons, OrgGeogebraCommonKernelGeosGeoFunctionNVar *geoFunctionNVar, jdouble lowX, jdouble highX, jdouble lowY, jdouble highY, jboolean fixed) {
  ...
  self->imageSize_ = 280;
  self->gridPixel_ = 14;
  self->vals_ = [IOSDoubleArray newArrayWithLength:2];
  self->font_ = [((OrgGeogebraCommonAwtGFont *) nil_chk([((OrgGeogebraCommonMainApp *) nil_chk([((OrgGeogebraCommonKernelKernel *) nil_chk(self->kernel_)) getApplication])) getFontCanDisplayWithNSString:@"-999"])) deriveFontWithInt:OrgGeogebraCommonAwtGFont_PLAIN withInt:8];
  self->grade_ = 1;
  if ([((OrgGeogebraCommonMainApp *) nil_chk([self->kernel_ getApplication])) isHTML5Applet]) {
    self->grade_ = 2;
    self->offset_ = 25;
  }
  self->function_ = geoFunctionNVar;
  self->f_ = [((OrgGeogebraCommonKernelGeosGeoFunctionNVar *) nil_chk(self->function_)) getFunction];
  self->view_ = [((OrgGeogebraCommonMainApp *) nil_chk([self->kernel_ getApplication])) getActiveEuclidianView];
  self->fixed_ = fixed;
  self->minX_ = -2;
  self->minY_ = -2;
  self->maxX_ = 2;
  ...
  self->outputImage_ = new_OrgGeogebraCommonKernelGeosGeoCanvasImage_initWithOrgGeogebraCommonKernelConstruction_withInt_withInt_(cons, self->imageSize_ + 2 * self->offset_, self->imageSize_ + 2 * self->offset_);
  self->g_ = [self->outputImage_ getGraphics];
  ...
  self->df_ = new_JavaTextDecimalFormat_initWithNSString_(@"0.##");
...
IOSIntArray *OrgGeogebraCommonKernelAlgosAlgoDensityPlot_hlsToRgbWithDouble_withDouble_withDouble_(jdouble h, jdouble l, jdouble s) {
  OrgGeogebraCommonKernelAlgosAlgoDensityPlot_initialize();
  IOSIntArray *rgb = [IOSIntArray newArrayWithLength:2];
  jdouble m2;
  if (l < 0.5) {
    m2 = l * (1 + s);
  }
