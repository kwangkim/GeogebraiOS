...
void OrgGeogebraCommonKernelStatisticsAlgoANOVA_compute(OrgGeogebraCommonKernelStatisticsAlgoANOVA *self) {
  ...
  if (self->categoryData_ == nil) {
    self->categoryData_ = new_JavaUtilArrayList_init();
  }
  else {
  ...
  for (jint index = 0; index < size; index++) {
    ...
    IOSDoubleArray *val = [IOSDoubleArray newArrayWithLength:[((OrgGeogebraCommonKernelGeosGeoList *) nil_chk(list)) size]];
  ...
  @try {
    if (self->anovaImpl_ == nil) self->anovaImpl_ = new_OrgApacheCommonsMathStatInferenceOneWayAnovaImpl_init();
