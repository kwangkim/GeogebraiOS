...
void OrgGeogebraCommonKernelStatisticsAlgoTiedRank_compute(OrgGeogebraCommonKernelStatisticsAlgoTiedRank *self) {
  ...
  if (self->rankingAlgorithm_ == nil) self->rankingAlgorithm_ = new_OrgApacheCommonsMathStatRankingNaturalRanking_initWithOrgApacheCommonsMathStatRankingTiesStrategyEnum_(OrgApacheCommonsMathStatRankingTiesStrategyEnum_get_AVERAGE());
  self->outputArray_ = [((OrgApacheCommonsMathStatRankingNaturalRanking *) nil_chk(self->rankingAlgorithm_)) rankWithDoubleArray:self->inputArray_];
  [((OrgGeogebraCommonKernelGeosGeoList *) nil_chk(self->outputList_)) setDefinedWithBoolean:YES];
  [self->outputList_ clear];
  for (jint i = 0; i < self->size_; i++) {
    [self->outputList_ addWithOrgGeogebraCommonKernelGeosGeoElement:new_OrgGeogebraCommonKernelGeosGeoNumeric_initWithOrgGeogebraCommonKernelConstruction_withDouble_(self->cons_, IOSDoubleArray_Get(nil_chk(self->outputArray_), i))];
  }
}
