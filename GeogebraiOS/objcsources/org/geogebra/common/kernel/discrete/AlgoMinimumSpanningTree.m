...
//  source: /Users/jacky/GSoC/GeoGebraiOSARC/GeoGebraiOSARC/javasources/org/geogebra/common/kernel/discrete/AlgoMinimumSpanningTree.java
//
...
- (void)compute {
  ...
  OrgGeogebraCommonKernelDiscreteAldsDsGraphsWeightedGraph *weightedGraph = new_OrgGeogebraCommonKernelDiscreteAldsDsGraphsWeightedGraph_initWithOrgGeogebraCommonKernelDiscreteAldsDsGraphsGraph_TypeEnum_(OrgGeogebraCommonKernelDiscreteAldsDsGraphsGraph_TypeEnum_get_UNDIRECTED());
  IOSObjectArray *vertices = [IOSObjectArray newArrayWithLength:size_ type:OrgGeogebraCommonKernelDiscreteAldsDsGraphsVertex_class_()];
  for (jint i = 0; i < size_; i++) {
    (void) IOSObjectArray_Set(vertices, i, new_OrgGeogebraCommonKernelDiscreteAldsDsGraphsVertex_initWithNSString_withOrgGeogebraCommonKernelKernelNDGeoPointND_(JreStrcat("I", i), (id<OrgGeogebraCommonKernelKernelNDGeoPointND>) check_protocol_cast([inputList_ getWithInt:i], @protocol(OrgGeogebraCommonKernelKernelNDGeoPointND))));
  }
  ...
  for (jint i = 0; i < size_; i++) {
    ...
    if (connectedVertex != nil) {
      ...
      [((JavaUtilArrayList *) nil_chk(al_)) addWithId:new_OrgGeogebraCommonKernelMyPoint_initWithDouble_withDouble_withBoolean_([((OrgGeogebraCommonKernelMatrixCoords *) nil_chk(coords)) getWithInt:1], [coords getWithInt:2], NO)];
      coords = [((id<OrgGeogebraCommonKernelKernelNDGeoPointND>) nil_chk(point2)) getInhomCoordsInD2];
      [al_ addWithId:new_OrgGeogebraCommonKernelMyPoint_initWithDouble_withDouble_withBoolean_([((OrgGeogebraCommonKernelMatrixCoords *) nil_chk(coords)) getWithInt:1], [coords getWithInt:2], YES)];
    }
  }
