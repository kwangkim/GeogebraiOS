...
//  source: /Users/jacky/GSoC/GeoGebraiOSARC/GeoGebraiOSARC/javasources/org/geogebra/common/kernel/discrete/AlgoVoronoi.java
//
...
- (void)compute {
  ...
  JavaUtilArrayList *xcoords = new_JavaUtilArrayList_init();
  JavaUtilArrayList *ycoords = new_JavaUtilArrayList_init();
  jdouble delta = 0.0000001;
  JavaUtilTreeSet *pointTree = new_JavaUtilTreeSet_initWithJavaUtilComparator_(OrgGeogebraCommonKernelDiscreteAlgoVoronoi_getPointComparator());
  for (jint i = 0; i < size_; i++) {
    ...
    if ([((OrgGeogebraCommonKernelGeosGeoElement *) nil_chk(geo)) isDefined] && [geo isGeoPoint]) {
      ...
      [pointTree addWithId:new_OrgGeogebraCommonAwtGPoint2D_Double_initWithDouble_withDouble_(IOSDoubleArray_Get(inhom, 0), IOSDoubleArray_Get(inhom, 1))];
    }
  }
  IOSObjectArray *points = [IOSObjectArray newArrayWithLength:[pointTree size] type:OrgGeogebraCommonKernelDiscreteDelaunayPoint_dt_class_()];
  jint indx = 0;
  ...
  while ([((id<JavaUtilIterator>) nil_chk(it3)) hasNext]) {
    ...
    (void) IOSObjectArray_Set(points, indx++, new_OrgGeogebraCommonKernelDiscreteDelaunayPoint_dt_initWithDouble_withDouble_(x, y));
  }
  OrgGeogebraCommonKernelDiscreteDelaunayDelaunay_Triangulation *dt = new_OrgGeogebraCommonKernelDiscreteDelaunayDelaunay_Triangulation_initWithOrgGeogebraCommonKernelDiscreteDelaunayPoint_dtArray_(points);
  if (dt->allCollinear_) {
    [((OrgGeogebraCommonKernelGeosGeoLocus *) nil_chk(locus_)) setUndefined];
    return;
  }
  ...
  JavaUtilTreeSet *tree = new_JavaUtilTreeSet_initWithJavaUtilComparator_(OrgGeogebraCommonKernelDiscreteAlgoDelauneyTriangulation_getComparator());
  ...
  while ([((id<JavaUtilIterator>) nil_chk(it2)) hasNext]) {
    ...
    [((JavaUtilArrayList *) nil_chk(al_)) addWithId:new_OrgGeogebraCommonKernelMyPoint_initWithDouble_withDouble_withBoolean_([((OrgGeogebraCommonAwtGPoint2D *) nil_chk(((OrgGeogebraCommonKernelDiscreteMyLine *) nil_chk(line))->p1_)) getX], [line->p1_ getY], NO)];
    [al_ addWithId:new_OrgGeogebraCommonKernelMyPoint_initWithDouble_withDouble_withBoolean_([((OrgGeogebraCommonAwtGPoint2D *) nil_chk(line->p2_)) getX], [line->p2_ getY], YES)];
  }
...
id<JavaUtilComparator> OrgGeogebraCommonKernelDiscreteAlgoVoronoi_getPointComparator() {
  ...
  if (OrgGeogebraCommonKernelDiscreteAlgoVoronoi_pointComparator_ == nil) {
    OrgGeogebraCommonKernelDiscreteAlgoVoronoi_pointComparator_ = new_OrgGeogebraCommonKernelDiscreteAlgoVoronoi_$1_init();
  }
