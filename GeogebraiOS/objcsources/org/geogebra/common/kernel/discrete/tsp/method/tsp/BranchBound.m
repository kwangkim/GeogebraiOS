...
- (OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_Circuit *)branchWithId:(id)panel
  ...
  IOSObjectArray *betterEdges = [IOSBooleanArray newArrayWithDimensions:2 lengths:(jint[]){ edges->size_, edges->size_ }];
  do {
    ...
    if (OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_updateMulipliersWithDouble_withBooleanArray2_withDoubleArray_(self, multiplier, edges, multipliers)) {
      ...
      return new_OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_Circuit_initWithOrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_withIntArray_withDouble_(self, route, cost);
    }
    multiplier *= 0.95;
  }
  ...
  for (jint i = 0; i < edges->size_; i++) {
    ...
    if (connection > 2) {
      IOSObjectArray *connects = [IOSBooleanArray newArrayWithDimensions:3 lengths:(jint[]){ 3 - fix, edges->size_, edges->size_ }];
      IOSObjectArray *disconnects = [IOSBooleanArray newArrayWithDimensions:3 lengths:(jint[]){ 3 - fix, edges->size_, edges->size_ }];
      if (fix < 2) {
...
- (IOSIntArray *)methodWithOrgGeogebraCommonKernelDiscreteTspModelNodeArray:(IOSObjectArray *)nodes {
  if (((IOSObjectArray *) nil_chk(nodes))->size_ > 2) {
    IOSObjectArray *edges = [IOSBooleanArray newArrayWithDimensions:2 lengths:(jint[]){ nodes->size_, nodes->size_ }];
    IOSObjectArray *table = [self createTableWithOrgGeogebraCommonKernelDiscreteTspModelNodeArray:nodes];
    IOSDoubleArray *multipliers = [IOSDoubleArray newArrayWithLength:nodes->size_];
    IOSObjectArray *connect = [IOSBooleanArray newArrayWithDimensions:2 lengths:(jint[]){ nodes->size_, nodes->size_ }];
    IOSObjectArray *disconnect = [IOSBooleanArray newArrayWithDimensions:2 lengths:(jint[]){ nodes->size_, nodes->size_ }];
    jint depth = 0;
    jdouble percent = 0;
...
IOSIntArray *OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_compressTableToArrayWithBooleanArray2_(IOSObjectArray *table) {
  ...
  IOSIntArray *route = [IOSIntArray newArrayWithLength:((IOSObjectArray *) nil_chk(table))->size_];
  jint before = -1;
  jint index = 0;
  jint count = 0;
...
void OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_getOneTreeWithBooleanArray2_withDoubleArray2_withDoubleArray_withBooleanArray2_withBooleanArray2_(OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound *self, IOSObjectArray *edges, IOSObjectArray *table, IOSDoubleArray *multipliers, IOSObjectArray *connect, IOSObjectArray *disconnect) {
  ...
  OrgGeogebraCommonKernelDiscreteTspUtilHeap *open = new_OrgGeogebraCommonKernelDiscreteTspUtilHeap_initWithInt_(11);
  OrgGeogebraCommonKernelDiscreteTspUtilIntegerSet *notClose = new_OrgGeogebraCommonKernelDiscreteTspUtilIntegerSet_initWithInt_(edges->size_);
  for (jint i = 0; i < edges->size_; i++) {
  ...
  for (jint i = 0; i < [notClose size]; i++) {
    ...
    if (!IOSBooleanArray_Get(nil_chk(IOSObjectArray_Get(nil_chk(disconnect), index)), a)) {
      [open addWithId:new_OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_Entry_initWithOrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_withInt_withInt_withDouble_(self, index, a, IOSBooleanArray_Get(nil_chk(IOSObjectArray_Get(nil_chk(connect), index)), a) ? JavaLangDouble_NEGATIVE_INFINITY : IOSDoubleArray_Get(nil_chk(IOSObjectArray_Get(nil_chk(table), index)), a) + IOSDoubleArray_Get(nil_chk(multipliers), index) + IOSDoubleArray_Get(multipliers, a))];
    }
  }
  do {
    edge = [open poll];
  ...
  for (jint i = 0; i < edges->size_; i++) {
    if (s != i && !IOSBooleanArray_Get(nil_chk(IOSObjectArray_Get(nil_chk(disconnect), s)), i)) {
      [open addWithId:new_OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_Entry_initWithOrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_withInt_withInt_withDouble_(self, s, i, IOSBooleanArray_Get(nil_chk(IOSObjectArray_Get(nil_chk(connect), s)), i) ? JavaLangDouble_NEGATIVE_INFINITY : IOSDoubleArray_Get(nil_chk(IOSObjectArray_Get(nil_chk(table), s)), i) + IOSDoubleArray_Get(nil_chk(multipliers), s) + IOSDoubleArray_Get(multipliers, i))];
    }
  }
  edge = [open poll];
  *IOSBooleanArray_GetRef(nil_chk(IOSObjectArray_Get(edges, ((OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_Entry *) nil_chk(edge))->s_)), edge->t_) = YES;
...
- (jint)compareToWithId:(OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_Entry *)e {
  (void) check_class_cast(e, [OrgGeogebraCommonKernelDiscreteTspMethodTspBranchBound_Entry class]);
