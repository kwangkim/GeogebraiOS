...
- (OrgGeogebraCommonUtilProver_ProofResultEnum *)proveWithOrgGeogebraCommonUtilProver:(OrgGeogebraCommonUtilProver *)prover {
  ...
  JavaUtilTreeSet *fixedVariables = new_JavaUtilTreeSet_initWithJavaUtilComparator_(new_OrgGeogebraCommonKernelProverAbstractProverReciosMethod_$1_init());
  JavaUtilHashSet *freeVariables = new_JavaUtilHashSet_init();
  ...
  while ([((id<JavaUtilIterator>) nil_chk(it)) hasNext]) {
    ...
    if (nrOfFixedPoints == 0) {
      var = [it next];
      (void) [values putWithId:var withId:JavaMathBigInteger_get_ZERO_()];
      (void) [values putWithId:[it next] withId:JavaMathBigInteger_get_ZERO_()];
      ...
      var = [it next];
      (void) [values putWithId:var withId:JavaMathBigInteger_get_ZERO_()];
      (void) [values putWithId:[it next] withId:JavaMathBigInteger_get_ONE_()];
  ...
  if (nrOfFixedPoints == 1) {
    OrgGeogebraCommonKernelProverAbstractProverReciosMethod_fixedPoints_ = [IOSObjectArray newArrayWithLength:1 type:OrgGeogebraCommonKernelGeosGeoElement_class_()];
    (void) IOSObjectArray_Set(OrgGeogebraCommonKernelProverAbstractProverReciosMethod_fixedPoints_, 0, fixedElement1);
  }
  else if (nrOfFixedPoints == 2) {
    OrgGeogebraCommonKernelProverAbstractProverReciosMethod_fixedPoints_ = [IOSObjectArray newArrayWithLength:2 type:OrgGeogebraCommonKernelGeosGeoElement_class_()];
    (void) IOSObjectArray_Set(OrgGeogebraCommonKernelProverAbstractProverReciosMethod_fixedPoints_, 0, fixedElement1);
    (void) IOSObjectArray_Set(OrgGeogebraCommonKernelProverAbstractProverReciosMethod_fixedPoints_, 1, fixedElement2);
  }
...
OrgGeogebraCommonUtilProver_ProofResultEnum *OrgGeogebraCommonKernelProverAbstractProverReciosMethod_compute1dWithJavaUtilHashSet_withJavaUtilHashMap_withInt_withOrgGeogebraCommonKernelAlgosSymbolicParameters_(JavaUtilHashSet *freeVariables, JavaUtilHashMap *values, jint deg, OrgGeogebraCommonKernelAlgosSymbolicParameters *s) {
  ...
  for (jint i = 1; i <= deg + 2; i++) {
    (void) [((JavaUtilHashMap *) nil_chk(values)) putWithId:variable withId:JavaMathBigInteger_valueOfWithLong_(i)];
    @try {
      IOSObjectArray *exactCoordinates = [((OrgGeogebraCommonKernelAlgosSymbolicParameters *) nil_chk(s)) getExactCoordinatesWithJavaUtilHashMap:values];
      {
        IOSObjectArray *a__ = exactCoordinates;
...
OrgGeogebraCommonUtilProver_ProofResultEnum *OrgGeogebraCommonKernelProverAbstractProverReciosMethod_compute2dWithJavaUtilHashSet_withJavaUtilHashMap_withInt_withOrgGeogebraCommonKernelAlgosSymbolicParameters_(JavaUtilHashSet *freeVariables, JavaUtilHashMap *values, jint deg, OrgGeogebraCommonKernelAlgosSymbolicParameters *s) {
  OrgGeogebraCommonKernelProverAbstractProverReciosMethod_initialize();
  IOSObjectArray *variables = [IOSObjectArray newArrayWithLength:[((JavaUtilHashSet *) nil_chk(freeVariables)) size] type:OrgGeogebraCommonKernelProverPolynomialVariable_class_()];
  id<JavaUtilIterator> it = [freeVariables iterator];
  for (jint i = 0; i < variables->size_; i++) {
    (void) IOSObjectArray_Set(variables, i, [((id<JavaUtilIterator>) nil_chk(it)) next]);
  }
  jint nrOfTests = ((deg + 2) * (deg + 1)) / 2;
  OrgGeogebraCommonMainApp_debugWithNSString_(JreStrcat("$I", @"nr of tests: ", nrOfTests));
  for (jint i = 1; i <= deg + 2; i++) {
    for (jint j = 1; j <= i; j++) {
      (void) [((JavaUtilHashMap *) nil_chk(values)) putWithId:IOSObjectArray_Get(variables, 0) withId:JavaMathBigInteger_valueOfWithLong_((deg + 2 - i) * (deg + 2 - j))];
      (void) [values putWithId:IOSObjectArray_Get(variables, 1) withId:JavaMathBigInteger_valueOfWithLong_(i * j)];
      @try {
        IOSObjectArray *exactCoordinates = [((OrgGeogebraCommonKernelAlgosSymbolicParameters *) nil_chk(s)) getExactCoordinatesWithJavaUtilHashMap:values];
        {
          IOSObjectArray *a__ = exactCoordinates;
