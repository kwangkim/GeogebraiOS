...
- (void)runWithOrgGeogebraCommonPluginEvent:(OrgGeogebraCommonPluginEvent *)evt {
  ...
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(lines))->size_; i++) {
    ...
    @try {
      [((OrgGeogebraCommonKernelCommandsAlgebraProcessor *) nil_chk(proc_)) processAlgebraCommandNoExceptionHandlingWithNSString:line withBoolean:NO withBoolean:NO withBoolean:YES withBoolean:NO];
    }
    @catch (JavaLangThrowable *e) {
      @throw [new_OrgGeogebraCommonPluginScriptError_initWithNSString_(JreStrcat("$C$", [((OrgGeogebraCommonMainLocalization *) nil_chk([((OrgGeogebraCommonMainApp *) nil_chk(app_)) getLocalization])) getPlainWithNSString:@"ErrorInScriptAtLineAFromObjectB" withNSString:JreStrcat("I", (i + 1)) withNSString:[((OrgGeogebraCommonKernelGeosGeoElement *) nil_chk(evt->target_)) getLabelWithOrgGeogebraCommonKernelStringTemplate:OrgGeogebraCommonKernelStringTemplate_get_defaultTemplate_()]], 0x000a, [((JavaLangThrowable *) nil_chk(e)) getLocalizedMessage])) autorelease];
    }
  }
}
...
- (OrgGeogebraCommonPluginScriptScript *)copy__ {
  return [new_OrgGeogebraCommonPluginScriptGgbScript_initWithOrgGeogebraCommonMainApp_withNSString_(app_, text_) autorelease];
}
...
- (jboolean)renameGeoWithNSString:(NSString *)oldLabel
  ...
  OrgGeogebraCommonPluginScriptScript_set_text_(self, OrgGeogebraCommonUtilStringUtil_joinTokensWithJavaLangIterable_withNSString_(work, nil));
  return ret;
}
...
void OrgGeogebraCommonPluginScriptGgbScript_initWithOrgGeogebraCommonMainApp_withNSString_(OrgGeogebraCommonPluginScriptGgbScript *self, OrgGeogebraCommonMainApp *app, NSString *scriptText) {
  OrgGeogebraCommonPluginScriptScript_initWithOrgGeogebraCommonMainApp_withNSString_(self, app, scriptText);
  OrgGeogebraCommonPluginScriptGgbScript_set_proc_(self, [((OrgGeogebraCommonKernelKernel *) nil_chk([((OrgGeogebraCommonMainApp *) nil_chk(app)) getKernel])) getAlgebraProcessor]);
}
...
NSString *OrgGeogebraCommonPluginScriptGgbScript_script2LocalizedScriptWithOrgGeogebraCommonMainApp_withNSString_(OrgGeogebraCommonMainApp *app, NSString *st) {
  ...
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(starr))->size_; i++) {
    if ((i % 2) == 0 || OrgGeogebraCommonPluginScriptGgbScript_isFunctionWithNSStringArray_withInt_withOrgGeogebraCommonMainApp_(starr, i, app)) {
      ...
      [retone appendWithNSString:[((OrgGeogebraCommonMainLocalization *) nil_chk([((OrgGeogebraCommonMainApp *) nil_chk(app)) getLocalization])) getCommandWithNSString:IOSObjectArray_Get(starr, i)]];
    }
  }
