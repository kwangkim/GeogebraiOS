...
- (void)triangularize {
  ...
  @try {
    if ([pt_ updatePoints] > 2) {
      ...
      IOSObjectArray *vertices = [IOSObjectArray newArrayWithLength:length + OrgGeogebraCommonKernelDiscretePolygonTriangulation_EXTRA_POINTS type:OrgGeogebraCommonKernelMatrixCoords_class_()];
      jint j = 0;
      for (jint i = 0; i < [poly_ getPointsLength]; i++) {
        (void) IOSObjectArray_Set(vertices, i, [((id<OrgGeogebraCommonKernelKernelNDGeoPointND>) nil_chk([poly_ getPointNDWithInt:i])) getCoords]);
        j++;
      }
      (void) IOSObjectArray_Set(vertices, j, [((id<OrgGeogebraCommonKernelKernelNDGeoPointND>) nil_chk([poly_ getPointNDWithInt:0])) getCoords]);
      j++;
      for (jint i = 0; i < OrgGeogebraCommonKernelDiscretePolygonTriangulation_CORNERS; i++) {
        (void) IOSObjectArray_Set(vertices, j, IOSObjectArray_Get(nil_chk(extraCoords_), i));
        j++;
      }
      (void) IOSObjectArray_Set(vertices, j, IOSObjectArray_Get(nil_chk(extraCoords_), 0));
      j++;
      for (jint i = 0; i < OrgGeogebraCommonKernelDiscretePolygonTriangulation_CORNERS; i++) {
        (void) IOSObjectArray_Set(vertices, j, IOSObjectArray_Get(extraCoords_, 4 + i));
        j++;
      }
      (void) IOSObjectArray_Set(vertices, j, IOSObjectArray_Get(extraCoords_, 4));
      j++;
      (void) IOSObjectArray_Set(vertices, j, IOSObjectArray_Get(extraCoords_, 0));
      ...
      if (convexity != OrgGeogebraCommonKernelDiscretePolygonTriangulation_ConvexityEnum_get_NOT()) {
        ...
        if (gpTriangularize_ == nil) gpTriangularize_ = new_OrgGeogebraCommonEuclidianGeneralPathClipped_initWithOrgGeogebraCommonEuclidianEuclidianViewInterfaceSlim_(view_);
  ...
  if (isVisible_) {
    ...
    if ([((OrgGeogebraCommonEuclidianEuclidianController *) nil_chk([view_ getEuclidianController])) isAltDown]) {
      ...
      if ([points_ size] > 1) {
        ...
        OrgGeogebraCommonKernelGeosGeoPoint *intersection = new_OrgGeogebraCommonKernelGeosGeoPoint_initWithOrgGeogebraCommonKernelConstruction_(cons);
        OrgGeogebraCommonKernelGeosGeoLine *l = new_OrgGeogebraCommonKernelGeosGeoLine_initWithOrgGeogebraCommonKernelConstruction_(cons);
        OrgGeogebraCommonKernelGeosGeoLine *l2 = new_OrgGeogebraCommonKernelGeosGeoLine_initWithOrgGeogebraCommonKernelConstruction_(cons);
...
void OrgGeogebraCommonEuclidianDrawDrawPolygon_initWithOrgGeogebraCommonEuclidianEuclidianView_withOrgGeogebraCommonKernelGeosGeoPolygon_(OrgGeogebraCommonEuclidianDrawDrawPolygon *self, OrgGeogebraCommonEuclidianEuclidianView *view, OrgGeogebraCommonKernelGeosGeoPolygon *poly) {
  ...
  self->poly_ = poly;
  self->geo_ = poly;
  self->extraCoords_ = [IOSObjectArray newArrayWithLength:8 type:OrgGeogebraCommonKernelMatrixCoords_class_()];
  for (jint i = 0; i < 8; i++) {
    (void) IOSObjectArray_Set(self->extraCoords_, i, new_OrgGeogebraCommonKernelMatrixCoords_initWithDouble_withDouble_(0, 0));
  }
  OrgGeogebraCommonEuclidianDrawDrawPolygon_update(self);
}
...
void OrgGeogebraCommonEuclidianDrawDrawPolygon_drawTriangleFanWithOrgGeogebraCommonKernelMatrixCoords_withOrgGeogebraCommonKernelMatrixCoordsArray_withOrgGeogebraCommonKernelDiscretePolygonTriangulation_TriangleFan_(OrgGeogebraCommonEuclidianDrawDrawPolygon *self, OrgGeogebraCommonKernelMatrixCoords *n, IOSObjectArray *v, OrgGeogebraCommonKernelDiscretePolygonTriangulation_TriangleFan *triFan) {
  ...
  if (self->fanCoords_ == nil || self->fanCoords_->size_ < size) {
    self->fanCoords_ = [IOSObjectArray newArrayWithLength:size type:IOSClass_doubleArray(1)];
  }
  ...
  *IOSDoubleArray_GetRef(self->coords_, 1) = [coordsApex getY];
  ...
  for (jint i = 0; i < size; i++) {
    ...
    if (IOSObjectArray_Get(nil_chk(self->fanCoords_), i) == nil) {
      (void) IOSObjectArray_SetAndConsume(self->fanCoords_, i, [IOSDoubleArray newArrayWithLength:2]);
    }
