...
- (void)reinitializeWithDoubleArray:(IOSDoubleArray *)y
  ...
  currentState_ = y;
  interpolatedState_ = [IOSDoubleArray newArrayWithLength:((IOSDoubleArray *) nil_chk(y))->size_];
  interpolatedDerivatives_ = [IOSDoubleArray newArrayWithLength:y->size_];
  finalized_ = NO;
...
- (jdouble)readBaseExternalWithJavaIoObjectInput:(id<JavaIoObjectInput>)inArg {
  ...
  if (dimension < 0) {
    ...
    currentState_ = [IOSDoubleArray newArrayWithLength:dimension];
    for (jint i = 0; i < currentState_->size_; ++i) {
      *IOSDoubleArray_GetRef(currentState_, i) = [inArg readDouble];
    }
  }
  interpolatedTime_ = JavaLangDouble_NaN;
  interpolatedState_ = (dimension < 0) ? nil : [IOSDoubleArray newArrayWithLength:dimension];
  interpolatedDerivatives_ = (dimension < 0) ? nil : [IOSDoubleArray newArrayWithLength:dimension];
  finalized_ = YES;
...
void OrgApacheCommonsMathOdeSamplingAbstractStepInterpolator_init(OrgApacheCommonsMathOdeSamplingAbstractStepInterpolator *self) {
  ...
  self->currentState_ = nil;
  self->interpolatedState_ = nil;
  self->interpolatedDerivatives_ = nil;
  self->finalized_ = NO;
  self->forward_ = YES;
  self->dirtyState_ = YES;
}
...
void OrgApacheCommonsMathOdeSamplingAbstractStepInterpolator_initWithDoubleArray_withBoolean_(OrgApacheCommonsMathOdeSamplingAbstractStepInterpolator *self, IOSDoubleArray *y, jboolean forward) {
  ...
  self->h_ = JavaLangDouble_NaN;
  self->interpolatedTime_ = JavaLangDouble_NaN;
  self->currentState_ = y;
  self->interpolatedState_ = [IOSDoubleArray newArrayWithLength:((IOSDoubleArray *) nil_chk(y))->size_];
  self->interpolatedDerivatives_ = [IOSDoubleArray newArrayWithLength:y->size_];
  self->finalized_ = NO;
...
void OrgApacheCommonsMathOdeSamplingAbstractStepInterpolator_initWithOrgApacheCommonsMathOdeSamplingAbstractStepInterpolator_(OrgApacheCommonsMathOdeSamplingAbstractStepInterpolator *self, OrgApacheCommonsMathOdeSamplingAbstractStepInterpolator *interpolator) {
  ...
  if (interpolator->currentState_ != nil) {
    self->currentState_ = [interpolator->currentState_ clone];
    self->interpolatedState_ = [((IOSDoubleArray *) nil_chk(interpolator->interpolatedState_)) clone];
    self->interpolatedDerivatives_ = [((IOSDoubleArray *) nil_chk(interpolator->interpolatedDerivatives_)) clone];
  }
  else {
    self->currentState_ = nil;
    self->interpolatedState_ = nil;
