...
- (OrgApacheCommonsMathLinearArray2DRowRealMatrix *)initializeHighOrderDerivativesWithDoubleArray:(IOSDoubleArray *)first
  ...
  return [((OrgApacheCommonsMathLinearArray2DRowRealMatrix *) nil_chk(initialization_)) multiplyWithOrgApacheCommonsMathLinearArray2DRowRealMatrix:new_OrgApacheCommonsMathLinearArray2DRowRealMatrix_initWithDoubleArray2_withBoolean_(multistep, NO)];
}
...
+ (void)initialize {
  if (self == [OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer class]) {
    OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer_CACHE_ = new_JavaUtilHashMap_init();
...
void OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer_initWithInt_(OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer *self, jint nSteps) {
  ...
  id<OrgApacheCommonsMathLinearFieldDecompositionSolver> pSolver = [new_OrgApacheCommonsMathLinearFieldLUDecompositionImpl_initWithOrgApacheCommonsMathLinearFieldMatrix_(bigP) getSolver];
  IOSObjectArray *u = [IOSObjectArray newArrayWithLength:nSteps type:OrgApacheCommonsMathFractionBigFraction_class_()];
  ...
  for (jint i = ((IOSObjectArray *) nil_chk(shiftedP))->size_ - 1; i > 0; --i) {
    (void) IOSObjectArray_Set(shiftedP, i, IOSObjectArray_Get(shiftedP, i - 1));
  }
  (void) IOSObjectArray_SetAndConsume(shiftedP, 0, [IOSObjectArray newArrayWithLength:nSteps type:OrgApacheCommonsMathFractionBigFraction_class_()]);
  ...
  id<OrgApacheCommonsMathLinearFieldMatrix> bigMSupdate = [pSolver solveWithOrgApacheCommonsMathLinearFieldMatrix:new_OrgApacheCommonsMathLinearArray2DRowFieldMatrix_initWithOrgApacheCommonsMathFieldElementArray2_withBoolean_(shiftedP, NO)];
  (void) [bigP walkInOptimizedOrderWithOrgApacheCommonsMathLinearFieldMatrixChangingVisitor:new_OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer_$1_initWithOrgApacheCommonsMathFractionBigFraction_(OrgApacheCommonsMathFractionBigFraction_get_ZERO_())];
  id<OrgApacheCommonsMathLinearFieldMatrix> bigRInverse = [((id<OrgApacheCommonsMathLinearFieldDecompositionSolver>) nil_chk([new_OrgApacheCommonsMathLinearFieldLUDecompositionImpl_initWithOrgApacheCommonsMathLinearFieldMatrix_(bigP) getSolver])) getInverse];
  self->initialization_ = OrgApacheCommonsMathLinearMatrixUtils_bigFractionMatrixToRealMatrixWithOrgApacheCommonsMathLinearFieldMatrix_(bigRInverse);
  self->update_ = OrgApacheCommonsMathLinearMatrixUtils_bigFractionMatrixToRealMatrixWithOrgApacheCommonsMathLinearFieldMatrix_(bigMSupdate);
  self->c1_ = [IOSDoubleArray newArrayWithLength:nSteps];
...
OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer *OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer_getInstanceWithInt_(jint nSteps) {
  ...
  @synchronized(OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer_CACHE_) {
    ...
    if (t == nil) {
      t = new_OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer_initWithInt_(nSteps);
      (void) [OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer_CACHE_ putWithId:JavaLangInteger_valueOfWithInt_(nSteps) withId:t];
    }
    return t;
  }
}
...
id<OrgApacheCommonsMathLinearFieldMatrix> OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer_buildPWithInt_(OrgApacheCommonsMathOdeNonstiffAdamsNordsieckTransformer *self, jint nSteps) {
  IOSObjectArray *pData = [IOSObjectArray newArrayWithDimensions:2 lengths:(jint[]){ nSteps, nSteps } type:OrgApacheCommonsMathFractionBigFraction_class_()];
  for (jint i = 0; i < pData->size_; ++i) {
    IOSObjectArray *pI = IOSObjectArray_Get(pData, i);
    jint factor = -(i + 1);
    jint aj = factor;
    for (jint j = 0; j < ((IOSObjectArray *) nil_chk(pI))->size_; ++j) {
      (void) IOSObjectArray_Set(pI, j, new_OrgApacheCommonsMathFractionBigFraction_initWithInt_(aj * (j + 2)));
      aj *= factor;
    }
  }
  return new_OrgApacheCommonsMathLinearArray2DRowFieldMatrix_initWithOrgApacheCommonsMathFieldElementArray2_withBoolean_(pData, NO);
}
