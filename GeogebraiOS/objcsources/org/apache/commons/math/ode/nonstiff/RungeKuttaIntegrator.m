...
- (jdouble)integrateWithOrgApacheCommonsMathOdeFirstOrderDifferentialEquations:(id<OrgApacheCommonsMathOdeFirstOrderDifferentialEquations>)equations
  ...
  IOSObjectArray *yDotK = [IOSObjectArray newArrayWithLength:stages type:IOSClass_doubleArray(1)];
  for (jint i = 0; i < stages; ++i) {
    (void) IOSObjectArray_SetAndConsume(yDotK, i, [IOSDoubleArray newArrayWithLength:((IOSDoubleArray *) nil_chk(y0))->size_]);
  }
  IOSDoubleArray *yTmp = [IOSDoubleArray newArrayWithLength:((IOSDoubleArray *) nil_chk(y0))->size_];
