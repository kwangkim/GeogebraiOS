...
//  source: /Users/jacky/GSoC/GeoGebraiOSARC/GeoGebraiOSARC/javasources/org/apache/commons/math/ode/AbstractIntegrator.java
//
...
- (void)addEventHandlerWithOrgApacheCommonsMathOdeEventsEventHandler:(id<OrgApacheCommonsMathOdeEventsEventHandler>)handler
  ...
  [((id<JavaUtilCollection>) nil_chk(eventsStates_)) addWithId:new_OrgApacheCommonsMathOdeEventsEventState_initWithOrgApacheCommonsMathOdeEventsEventHandler_withDouble_withDouble_withInt_(handler, maxCheckInterval, convergence, maxIterationCount)];
}
...
- (void)computeDerivativesWithDouble:(jdouble)t
  ...
  if (++evaluations_ > maxEvaluations_) {
    @throw new_OrgApacheCommonsMathOdeDerivativeException_initWithJavaLangThrowable_(new_OrgApacheCommonsMathMaxEvaluationsExceededException_initWithInt_(maxEvaluations_));
  }
...
- (jdouble)acceptStepWithOrgApacheCommonsMathOdeSamplingAbstractStepInterpolator:(OrgApacheCommonsMathOdeSamplingAbstractStepInterpolator *)interpolator
  ...
  @try {
    ...
    id<JavaUtilSortedSet> occuringEvents = new_JavaUtilTreeSet_initWithJavaUtilComparator_(new_OrgApacheCommonsMathOdeAbstractIntegrator_$1_initWithInt_(orderingSign));
    for (OrgApacheCommonsMathOdeEventsEventState * __strong state in nil_chk(eventsStates_)) {
  ...
  @catch (OrgApacheCommonsMathOdeEventsEventException *se) {
    ...
    @throw new_OrgApacheCommonsMathOdeIntegratorException_initWithJavaLangThrowable_(se);
  }
  @catch (OrgApacheCommonsMathConvergenceException *ce) {
    @throw new_OrgApacheCommonsMathOdeIntegratorException_initWithJavaLangThrowable_(ce);
  }
}
...
- (void)sanityChecksWithOrgApacheCommonsMathOdeFirstOrderDifferentialEquations:(id<OrgApacheCommonsMathOdeFirstOrderDifferentialEquations>)ode
  ...
  if ([((id<OrgApacheCommonsMathOdeFirstOrderDifferentialEquations>) nil_chk(ode)) getDimension] != ((IOSDoubleArray *) nil_chk(y0))->size_) {
    @throw new_OrgApacheCommonsMathOdeIntegratorException_initWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_DIMENSIONS_MISMATCH_SIMPLE(), [IOSObjectArray newArrayWithObjects:(id[]){ JavaLangInteger_valueOfWithInt_([ode getDimension]), JavaLangInteger_valueOfWithInt_(y0->size_) } count:2 type:NSObject_class_()]);
  }
...
- (OrgApacheCommonsMathOdeEventsCombinedEventsManager *)addEndTimeCheckerWithDouble:(jdouble)startTime
  ...
  OrgApacheCommonsMathOdeEventsCombinedEventsManager *newManager = new_OrgApacheCommonsMathOdeEventsCombinedEventsManager_init();
...
void OrgApacheCommonsMathOdeAbstractIntegrator_initWithNSString_(OrgApacheCommonsMathOdeAbstractIntegrator *self, NSString *name) {
  ...
  self->name_ = name;
  self->stepHandlers_ = new_JavaUtilArrayList_init();
  self->stepStart_ = JavaLangDouble_NaN;
  self->stepSize_ = JavaLangDouble_NaN;
  self->eventsStates_ = new_JavaUtilArrayList_init();
...
void OrgApacheCommonsMathOdeAbstractIntegrator_init(OrgApacheCommonsMathOdeAbstractIntegrator *self) {
  (void) OrgApacheCommonsMathOdeAbstractIntegrator_initWithNSString_(self, nil);
}
