...
- (OrgApacheCommonsMathGeneticsChromosome *)mutateWithOrgApacheCommonsMathGeneticsChromosome:(OrgApacheCommonsMathGeneticsChromosome *)original {
  if (!([original isKindOfClass:[OrgApacheCommonsMathGeneticsRandomKey class]])) {
    @throw OrgApacheCommonsMathMathRuntimeException_createIllegalArgumentExceptionWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_RANDOMKEY_MUTATION_WRONG_CLASS(), [IOSObjectArray newArrayWithObjects:(id[]){ [[((OrgApacheCommonsMathGeneticsChromosome *) nil_chk(original)) getClass] getSimpleName] } count:1 type:NSObject_class_()]);
  }
  ...
  id<JavaUtilList> newRepr = new_JavaUtilArrayList_initWithJavaUtilCollection_(repr);
  (void) [newRepr setWithInt:rInd withId:JavaLangDouble_valueOfWithDouble_([((id<OrgApacheCommonsMathRandomRandomGenerator>) nil_chk(OrgApacheCommonsMathGeneticsGeneticAlgorithm_getRandomGenerator())) nextDouble])];
