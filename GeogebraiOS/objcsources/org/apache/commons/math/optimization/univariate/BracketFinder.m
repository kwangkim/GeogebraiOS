...
- (void)searchWithOrgApacheCommonsMathAnalysisUnivariateRealFunction:(id<OrgApacheCommonsMathAnalysisUnivariateRealFunction>)func
  ...
  while (isMinim ? fC < fB : fC > fB) {
    if (++iterations_ > maxIterations_) {
      @throw new_OrgApacheCommonsMathMaxIterationsExceededException_initWithInt_(maxIterations_);
    }
...
void OrgApacheCommonsMathOptimizationUnivariateBracketFinder_init(OrgApacheCommonsMathOptimizationUnivariateBracketFinder *self) {
  (void) OrgApacheCommonsMathOptimizationUnivariateBracketFinder_initWithDouble_withInt_(self, 100, 50);
}
...
void OrgApacheCommonsMathOptimizationUnivariateBracketFinder_initWithDouble_withInt_(OrgApacheCommonsMathOptimizationUnivariateBracketFinder *self, jdouble growLimit, jint maxIterations) {
  ...
  if (growLimit <= 0) {
    @throw new_OrgApacheCommonsMathExceptionNotStrictlyPositiveException_initWithNSNumber_(JavaLangDouble_valueOfWithDouble_(growLimit));
  }
  if (maxIterations <= 0) {
    @throw new_OrgApacheCommonsMathExceptionNotStrictlyPositiveException_initWithNSNumber_(JavaLangInteger_valueOfWithInt_(maxIterations));
  }
