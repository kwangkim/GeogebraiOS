...
- (OrgApacheCommonsMathOptimizationVectorialPointValuePair *)doOptimize {
  ...
  IOSDoubleArray *oldObj = [IOSDoubleArray newArrayWithLength:rows_];
  IOSDoubleArray *qtf = [IOSDoubleArray newArrayWithLength:rows_];
  ...
  OrgApacheCommonsMathOptimizationVectorialPointValuePair *current = new_OrgApacheCommonsMathOptimizationVectorialPointValuePair_initWithDoubleArray_withDoubleArray_(point_, objective_);
  while (YES) {
  ...
  for (jint k = 0; k < self->cols_; ++k) {
    ...
    for (jint i = k; i < self->cols_; ++i) {
      ...
      if (JavaLangDouble_isInfiniteWithDouble_(norm2) || JavaLangDouble_isNaNWithDouble_(norm2)) {
        @throw new_OrgApacheCommonsMathOptimizationOptimizationException_initWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_UNABLE_TO_PERFORM_QR_DECOMPOSITION_ON_JACOBIAN(), [IOSObjectArray newArrayWithObjects:(id[]){ JavaLangInteger_valueOfWithInt_(self->rows_), JavaLangInteger_valueOfWithInt_(self->cols_) } count:2 type:NSObject_class_()]);
      }
      if (norm2 > ak2) {
        nextColumn = i;
        ak2 = norm2;
