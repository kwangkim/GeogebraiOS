...
- (jdouble)computeObjectiveValueWithDoubleArray:(IOSDoubleArray *)evaluationPoint {
  if (++evaluations_ > maxEvaluations_) {
    @throw new_OrgApacheCommonsMathFunctionEvaluationException_initWithJavaLangThrowable_withDoubleArray_(new_OrgApacheCommonsMathMaxEvaluationsExceededException_initWithInt_(maxEvaluations_), evaluationPoint);
  }
...
- (OrgApacheCommonsMathOptimizationRealPointValuePair *)optimizeWithOrgApacheCommonsMathAnalysisDifferentiableMultivariateRealFunction:(id<OrgApacheCommonsMathAnalysisDifferentiableMultivariateRealFunction>)f
  ...
  function_ = f;
  gradient_ = [((id<OrgApacheCommonsMathAnalysisDifferentiableMultivariateRealFunction>) nil_chk(f)) gradient];
  goal_ = goalType;
  point_ = [((IOSDoubleArray *) nil_chk(startPoint)) clone];
