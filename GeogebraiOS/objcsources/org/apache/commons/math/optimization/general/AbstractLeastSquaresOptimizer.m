...
- (void)updateResidualsAndCost {
  if (++objectiveEvaluations_ > maxEvaluations_) {
    @throw new_OrgApacheCommonsMathFunctionEvaluationException_initWithJavaLangThrowable_withDoubleArray_(new_OrgApacheCommonsMathMaxEvaluationsExceededException_initWithInt_(maxEvaluations_), point_);
  }
  objective_ = [((id<OrgApacheCommonsMathAnalysisDifferentiableMultivariateVectorialFunction>) nil_chk(function_)) valueWithDoubleArray:point_];
  if (((IOSDoubleArray *) nil_chk(objective_))->size_ != rows_) {
    @throw new_OrgApacheCommonsMathFunctionEvaluationException_initWithDoubleArray_withOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(point_, OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_DIMENSIONS_MISMATCH_SIMPLE(), [IOSObjectArray newArrayWithObjects:(id[]){ JavaLangInteger_valueOfWithInt_(objective_->size_), JavaLangInteger_valueOfWithInt_(rows_) } count:2 type:NSObject_class_()]);
  }
  cost_ = 0;
  for (jint i = 0; i < rows_; i++) {
...
- (IOSObjectArray *)getCovariances {
  [self updateJacobian];
  IOSObjectArray *jTj = [IOSDoubleArray newArrayWithDimensions:2 lengths:(jint[]){ cols_, cols_ }];
  ...
  @try {
    id<OrgApacheCommonsMathLinearRealMatrix> inverse = [((id<OrgApacheCommonsMathLinearDecompositionSolver>) nil_chk([new_OrgApacheCommonsMathLinearLUDecompositionImpl_initWithOrgApacheCommonsMathLinearRealMatrix_(OrgApacheCommonsMathLinearMatrixUtils_createRealMatrixWithDoubleArray2_(jTj)) getSolver])) getInverse];
  ...
  @catch (OrgApacheCommonsMathLinearInvalidMatrixException *ime) {
    @throw new_OrgApacheCommonsMathOptimizationOptimizationException_initWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_UNABLE_TO_COMPUTE_COVARIANCE_SINGULAR_PROBLEM(), [IOSObjectArray newArrayWithLength:0 type:NSObject_class_()]);
  }
}

- (IOSDoubleArray *)guessParametersErrors {
  if (rows_ <= cols_) {
    @throw new_OrgApacheCommonsMathOptimizationOptimizationException_initWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_NO_DEGREES_OF_FREEDOM(), [IOSObjectArray newArrayWithObjects:(id[]){ JavaLangInteger_valueOfWithInt_(rows_), JavaLangInteger_valueOfWithInt_(cols_) } count:2 type:NSObject_class_()]);
  }
  IOSDoubleArray *errors = [IOSDoubleArray newArrayWithLength:cols_];
...
- (OrgApacheCommonsMathOptimizationVectorialPointValuePair *)optimizeWithOrgApacheCommonsMathAnalysisDifferentiableMultivariateVectorialFunction:(id<OrgApacheCommonsMathAnalysisDifferentiableMultivariateVectorialFunction>)f
  ...
  if (((IOSDoubleArray *) nil_chk(target))->size_ != ((IOSDoubleArray *) nil_chk(weights))->size_) {
    @throw new_OrgApacheCommonsMathOptimizationOptimizationException_initWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_DIMENSIONS_MISMATCH_SIMPLE(), [IOSObjectArray newArrayWithObjects:(id[]){ JavaLangInteger_valueOfWithInt_(target->size_), JavaLangInteger_valueOfWithInt_(weights->size_) } count:2 type:NSObject_class_()]);
  }
  iterations_ = 0;
  objectiveEvaluations_ = 0;
  jacobianEvaluations_ = 0;
  function_ = f;
  jF_ = [((id<OrgApacheCommonsMathAnalysisDifferentiableMultivariateVectorialFunction>) nil_chk(f)) jacobian];
  targetValues_ = [target clone];
  residualsWeights_ = [weights clone];
  self->point_ = [((IOSDoubleArray *) nil_chk(startPoint)) clone];
  self->residuals_ = [IOSDoubleArray newArrayWithLength:target->size_];
  rows_ = target->size_;
  cols_ = ((IOSDoubleArray *) nil_chk(point_))->size_;
  jacobian_ = [IOSDoubleArray newArrayWithDimensions:2 lengths:(jint[]){ rows_, cols_ }];
  wjacobian_ = [IOSDoubleArray newArrayWithDimensions:2 lengths:(jint[]){ rows_, cols_ }];
  wresiduals_ = [IOSDoubleArray newArrayWithLength:rows_];
...
void OrgApacheCommonsMathOptimizationGeneralAbstractLeastSquaresOptimizer_init(OrgApacheCommonsMathOptimizationGeneralAbstractLeastSquaresOptimizer *self) {
  ...
  [self setConvergenceCheckerWithOrgApacheCommonsMathOptimizationVectorialConvergenceChecker:new_OrgApacheCommonsMathOptimizationSimpleVectorialValueChecker_init()];
