...
- (OrgApacheCommonsMathOptimizationFittingHarmonicFunction *)fit {
  if (parameters_ == nil) {
    ...
    if (((IOSObjectArray *) nil_chk(observations))->size_ < 4) {
      @throw new_OrgApacheCommonsMathOptimizationOptimizationException_initWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_INSUFFICIENT_OBSERVED_POINTS_IN_SAMPLE(), [IOSObjectArray newArrayWithObjects:(id[]){ JavaLangInteger_valueOfWithInt_(observations->size_), JavaLangInteger_valueOfWithInt_(4) } count:2 type:NSObject_class_()]);
    }
    OrgApacheCommonsMathOptimizationFittingHarmonicCoefficientsGuesser *guesser = new_OrgApacheCommonsMathOptimizationFittingHarmonicCoefficientsGuesser_initWithOrgApacheCommonsMathOptimizationFittingWeightedObservedPointArray_(observations);
    [guesser guess];
    parameters_ = [IOSDoubleArray newArrayWithDoubles:(jdouble[]){ [guesser getGuessedAmplitude], [guesser getGuessedPulsation], [guesser getGuessedPhase] } count:3];
  }
  @try {
    IOSDoubleArray *fitted = [((OrgApacheCommonsMathOptimizationFittingCurveFitter *) nil_chk(fitter_)) fitWithOrgApacheCommonsMathOptimizationFittingParametricRealFunction:new_OrgApacheCommonsMathOptimizationFittingHarmonicFitter_ParametricHarmonicFunction_init() withDoubleArray:parameters_];
    return new_OrgApacheCommonsMathOptimizationFittingHarmonicFunction_initWithDouble_withDouble_withDouble_(IOSDoubleArray_Get(nil_chk(fitted), 0), IOSDoubleArray_Get(fitted, 1), IOSDoubleArray_Get(fitted, 2));
  }
...
void OrgApacheCommonsMathOptimizationFittingHarmonicFitter_initWithOrgApacheCommonsMathOptimizationDifferentiableMultivariateVectorialOptimizer_withDoubleArray_(OrgApacheCommonsMathOptimizationFittingHarmonicFitter *self, id<OrgApacheCommonsMathOptimizationDifferentiableMultivariateVectorialOptimizer> optimizer, IOSDoubleArray *initialGuess) {
  (void) NSObject_init(self);
  self->fitter_ = new_OrgApacheCommonsMathOptimizationFittingCurveFitter_initWithOrgApacheCommonsMathOptimizationDifferentiableMultivariateVectorialOptimizer_(optimizer);
  self->parameters_ = [((IOSDoubleArray *) nil_chk(initialGuess)) clone];
}
...
- (IOSDoubleArray *)gradientWithDouble:(jdouble)x
  ...
  return [IOSDoubleArray newArrayWithDoubles:(jdouble[]){ cosAlpha, -a * x * sinAlpha, -a * sinAlpha } count:3];
}
