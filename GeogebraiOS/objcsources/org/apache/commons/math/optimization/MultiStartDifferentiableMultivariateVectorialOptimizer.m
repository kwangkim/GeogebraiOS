...
- (OrgApacheCommonsMathOptimizationVectorialPointValuePair *)optimizeWithOrgApacheCommonsMathAnalysisDifferentiableMultivariateVectorialFunction:(id<OrgApacheCommonsMathAnalysisDifferentiableMultivariateVectorialFunction>)f
  ...
  optima_ = [IOSObjectArray newArrayWithLength:starts_ type:OrgApacheCommonsMathOptimizationVectorialPointValuePair_class_()];
  totalIterations_ = 0;
  totalEvaluations_ = 0;
  ...
  for (jint i = 0; i < starts_; ++i) {
    @try {
      ...
      (void) IOSObjectArray_Set(optima_, i, [optimizer_ optimizeWithOrgApacheCommonsMathAnalysisDifferentiableMultivariateVectorialFunction:f withDoubleArray:target withDoubleArray:weights withDoubleArray:(i == 0) ? startPoint : [((id<OrgApacheCommonsMathRandomRandomVectorGenerator>) nil_chk(generator_)) nextVector]]);
    }
  ...
  JavaUtilArrays_sortWithNSObjectArray_withJavaUtilComparator_(optima_, new_OrgApacheCommonsMathOptimizationMultiStartDifferentiableMultivariateVectorialOptimizer_$1_initWithDoubleArray_withDoubleArray_(target, weights));
