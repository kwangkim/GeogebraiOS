...
- (void)iterateSimplexWithJavaUtilComparator:(id<JavaUtilComparator>)comparator {
  ...
  IOSDoubleArray *centroid = [IOSDoubleArray newArrayWithLength:n];
  for (jint i = 0; i < n; ++i) {
  ...
  IOSDoubleArray *xR = [IOSDoubleArray newArrayWithLength:n];
  for (jint j = 0; j < n; ++j) {
  ...
  OrgApacheCommonsMathOptimizationRealPointValuePair *reflected = new_OrgApacheCommonsMathOptimizationRealPointValuePair_initWithDoubleArray_withDouble_withBoolean_(xR, [self evaluateWithDoubleArray:xR], NO);
  if (([((id<JavaUtilComparator>) nil_chk(comparator)) compareWithId:best withId:reflected] <= 0) && ([comparator compareWithId:reflected withId:secondBest] < 0)) {
    ...
    IOSDoubleArray *xE = [IOSDoubleArray newArrayWithLength:n];
    for (jint j = 0; j < n; ++j) {
      *IOSDoubleArray_GetRef(xE, j) = IOSDoubleArray_Get(centroid, j) + khi_ * (IOSDoubleArray_Get(xR, j) - IOSDoubleArray_Get(centroid, j));
    }
    OrgApacheCommonsMathOptimizationRealPointValuePair *expanded = new_OrgApacheCommonsMathOptimizationRealPointValuePair_initWithDoubleArray_withDouble_withBoolean_(xE, [self evaluateWithDoubleArray:xE], NO);
    ...
    if ([comparator compareWithId:reflected withId:worst] < 0) {
      IOSDoubleArray *xC = [IOSDoubleArray newArrayWithLength:n];
      for (jint j = 0; j < n; ++j) {
        *IOSDoubleArray_GetRef(xC, j) = IOSDoubleArray_Get(centroid, j) + gamma_ * (IOSDoubleArray_Get(xR, j) - IOSDoubleArray_Get(centroid, j));
      }
      OrgApacheCommonsMathOptimizationRealPointValuePair *outContracted = new_OrgApacheCommonsMathOptimizationRealPointValuePair_initWithDoubleArray_withDouble_withBoolean_(xC, [self evaluateWithDoubleArray:xC], NO);
      ...
      OrgApacheCommonsMathOptimizationRealPointValuePair *inContracted = new_OrgApacheCommonsMathOptimizationRealPointValuePair_initWithDoubleArray_withDouble_withBoolean_(xC, [self evaluateWithDoubleArray:xC], NO);
    ...
    for (jint i = 1; i < simplex_->size_; ++i) {
      ...
      (void) IOSObjectArray_Set(simplex_, i, new_OrgApacheCommonsMathOptimizationRealPointValuePair_initWithDoubleArray_withDouble_withBoolean_(x, JavaLangDouble_NaN, NO));
    }
