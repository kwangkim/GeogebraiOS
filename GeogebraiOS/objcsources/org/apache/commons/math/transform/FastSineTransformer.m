...
- (IOSDoubleArray *)fstWithDoubleArray:(IOSDoubleArray *)f {
  IOSDoubleArray *transformed = [IOSDoubleArray newArrayWithLength:((IOSDoubleArray *) nil_chk(f))->size_];
  OrgApacheCommonsMathTransformFastFourierTransformer_verifyDataSetWithDoubleArray_(f);
  if (IOSDoubleArray_Get(f, 0) != 0.0) {
    @throw OrgApacheCommonsMathMathRuntimeException_createIllegalArgumentExceptionWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_FIRST_ELEMENT_NOT_ZERO(), [IOSObjectArray newArrayWithObjects:(id[]){ JavaLangDouble_valueOfWithDouble_(IOSDoubleArray_Get(f, 0)) } count:1 type:NSObject_class_()]);
  }
  jint n = f->size_;
  if (n == 1) {
  ...
  *IOSDoubleArray_GetRef(x, RShift32(n, 1)) = 2.0 * IOSDoubleArray_Get(f, RShift32(n, 1));
  ...
  OrgApacheCommonsMathTransformFastFourierTransformer *transformer = new_OrgApacheCommonsMathTransformFastFourierTransformer_init();
