...
- (OrgApacheCommonsMathAnalysisPolynomialsPolynomialFunctionNewtonForm *)interpolateWithDoubleArray:(IOSDoubleArray *)x
  ...
  IOSDoubleArray *c = [IOSDoubleArray newArrayWithLength:((IOSDoubleArray *) nil_chk(x))->size_ - 1];
  ...
  return new_OrgApacheCommonsMathAnalysisPolynomialsPolynomialFunctionNewtonForm_initWithDoubleArray_withDoubleArray_(a, c);
}
...
IOSDoubleArray *OrgApacheCommonsMathAnalysisInterpolationDividedDifferenceInterpolator_computeDividedDifferenceWithDoubleArray_withDoubleArray_(IOSDoubleArray *x, IOSDoubleArray *y) {
  ...
  IOSDoubleArray *a = [IOSDoubleArray newArrayWithLength:n];
  *IOSDoubleArray_GetRef(a, 0) = IOSDoubleArray_Get(nil_chk(divdiff), 0);
  for (jint i = 1; i < n; i++) {
    for (jint j = 0; j < n - i; j++) {
      ...
      if (denominator == 0.0) {
        @throw new_OrgApacheCommonsMathDuplicateSampleAbscissaException_initWithDouble_withInt_withInt_(IOSDoubleArray_Get(x, j), j, j + i);
      }
