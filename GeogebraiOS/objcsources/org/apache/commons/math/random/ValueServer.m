...
- (IOSDoubleArray *)fillWithInt:(jint)length {
  IOSDoubleArray *out = [IOSDoubleArray newArrayWithLength:length];
...
- (void)computeDistribution {
  empiricalDistribution_ = new_OrgApacheCommonsMathRandomEmpiricalDistributionImpl_init();
  [empiricalDistribution_ load__WithJavaNetURL:valuesFileURL_];
}

- (void)computeDistributionWithInt:(jint)binCount {
  empiricalDistribution_ = new_OrgApacheCommonsMathRandomEmpiricalDistributionImpl_initWithInt_(binCount);
  [empiricalDistribution_ load__WithJavaNetURL:valuesFileURL_];
...
- (void)setValuesFileURLWithNSString:(NSString *)url {
  self->valuesFileURL_ = new_JavaNetURL_initWithNSString_(url);
}

- (void)setValuesFileURLWithJavaNetURL:(JavaNetURL *)url {
  self->valuesFileURL_ = url;
}
...
- (void)resetReplayFile {
  ...
  filePointer_ = new_JavaIoBufferedReader_initWithJavaIoReader_(new_JavaIoInputStreamReader_initWithJavaIoInputStream_([((JavaNetURL *) nil_chk(valuesFileURL_)) openStream]));
}
...
void OrgApacheCommonsMathRandomValueServer_init(OrgApacheCommonsMathRandomValueServer *self) {
  ...
  self->mode_ = 5;
  self->valuesFileURL_ = nil;
  self->mu_ = 0.0;
  self->sigma_ = 0.0;
  self->empiricalDistribution_ = nil;
  self->filePointer_ = nil;
...
jdouble OrgApacheCommonsMathRandomValueServer_getNextDigest(OrgApacheCommonsMathRandomValueServer *self) {
  if ((self->empiricalDistribution_ == nil) || ([((id<JavaUtilList>) nil_chk([self->empiricalDistribution_ getBinStats])) size] == 0)) {
    @throw OrgApacheCommonsMathMathRuntimeException_createIllegalStateExceptionWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_DIGEST_NOT_INITIALIZED(), [IOSObjectArray newArrayWithLength:0 type:NSObject_class_()]);
  }
...
jdouble OrgApacheCommonsMathRandomValueServer_getNextReplay(OrgApacheCommonsMathRandomValueServer *self) {
  ...
  if ((str = [((JavaIoBufferedReader *) nil_chk(self->filePointer_)) readLine]) == nil) {
    ...
    if ((str = [self->filePointer_ readLine]) == nil) {
      @throw OrgApacheCommonsMathMathRuntimeException_createEOFExceptionWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_URL_CONTAINS_NO_DATA(), [IOSObjectArray newArrayWithObjects:(id[]){ self->valuesFileURL_ } count:1 type:NSObject_class_()]);
    }
  }
