...
- (OrgApacheCommonsMathStatClusteringEuclideanIntegerPoint *)centroidOfWithJavaUtilCollection:(id<JavaUtilCollection>)points {
  IOSIntArray *centroid = [IOSIntArray newArrayWithLength:((IOSIntArray *) nil_chk([self getPoint]))->size_];
  ...
  return new_OrgApacheCommonsMathStatClusteringEuclideanIntegerPoint_initWithIntArray_(centroid);
}
...
- (NSString *)description {
  ...
  for (jint i = 0; i < ((IOSIntArray *) nil_chk(coordinates))->size_; i++) {
    (void) [buff appendWithInt:IOSIntArray_Get(coordinates, i)];
    if (i < coordinates->size_ - 1) {
      (void) [buff appendWithNSString:@","];
    }
  }
  (void) [buff appendWithNSString:@")"];
