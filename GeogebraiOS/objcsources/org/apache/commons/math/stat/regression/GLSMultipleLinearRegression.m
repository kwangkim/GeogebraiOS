...
- (void)newCovarianceDataWithDoubleArray2:(IOSObjectArray *)omega {
  self->Omega_ = new_OrgApacheCommonsMathLinearArray2DRowRealMatrix_initWithDoubleArray2_(omega);
  self->OmegaInverse_ = nil;
}

- (id<OrgApacheCommonsMathLinearRealMatrix>)getOmegaInverse {
  if (OmegaInverse_ == nil) {
    OmegaInverse_ = [((id<OrgApacheCommonsMathLinearDecompositionSolver>) nil_chk([new_OrgApacheCommonsMathLinearLUDecompositionImpl_initWithOrgApacheCommonsMathLinearRealMatrix_(Omega_) getSolver])) getInverse];
  }
...
- (id<OrgApacheCommonsMathLinearRealVector>)calculateBeta {
  ...
  id<OrgApacheCommonsMathLinearRealMatrix> inverse = [((id<OrgApacheCommonsMathLinearDecompositionSolver>) nil_chk([new_OrgApacheCommonsMathLinearLUDecompositionImpl_initWithOrgApacheCommonsMathLinearRealMatrix_(XTOIX) getSolver])) getInverse];
...
- (id<OrgApacheCommonsMathLinearRealMatrix>)calculateBetaVariance {
  ...
  return [((id<OrgApacheCommonsMathLinearDecompositionSolver>) nil_chk([new_OrgApacheCommonsMathLinearLUDecompositionImpl_initWithOrgApacheCommonsMathLinearRealMatrix_(XTOIX) getSolver])) getInverse];
}
...
void OrgApacheCommonsMathStatRegressionGLSMultipleLinearRegression_init(OrgApacheCommonsMathStatRegressionGLSMultipleLinearRegression *self) {
  (void) OrgApacheCommonsMathStatRegressionAbstractMultipleLinearRegression_init(self);
}
