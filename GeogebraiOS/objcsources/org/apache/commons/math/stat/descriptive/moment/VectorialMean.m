...
- (void)incrementWithDoubleArray:(IOSDoubleArray *)v {
  if (((IOSDoubleArray *) nil_chk(v))->size_ != ((IOSObjectArray *) nil_chk(means_))->size_) {
    @throw new_OrgApacheCommonsMathDimensionMismatchException_initWithInt_withInt_(v->size_, means_->size_);
  }
...
- (IOSDoubleArray *)getResult {
  IOSDoubleArray *result = [IOSDoubleArray newArrayWithLength:((IOSObjectArray *) nil_chk(means_))->size_];
...
void OrgApacheCommonsMathStatDescriptiveMomentVectorialMean_initWithInt_(OrgApacheCommonsMathStatDescriptiveMomentVectorialMean *self, jint dimension) {
  ...
  self->means_ = [IOSObjectArray newArrayWithLength:dimension type:OrgApacheCommonsMathStatDescriptiveMomentMean_class_()];
  for (jint i = 0; i < dimension; ++i) {
    (void) IOSObjectArray_Set(self->means_, i, new_OrgApacheCommonsMathStatDescriptiveMomentMean_init());
  }
}
