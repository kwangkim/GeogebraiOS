...
- (id<OrgApacheCommonsMathLinearRealMatrix>)getV {
  if (cachedV_ == nil) {
    ...
    cachedV_ = OrgApacheCommonsMathLinearMatrixUtils_createRealMatrixWithInt_withInt_(m, m);
    for (jint k = 0; k < m; ++k) {
...
- (id<OrgApacheCommonsMathLinearRealMatrix>)getD {
  if (cachedD_ == nil) {
    cachedD_ = OrgApacheCommonsMathLinearMatrixUtils_createRealDiagonalMatrixWithDoubleArray_(realEigenvalues_);
  }
  return cachedD_;
}
...
- (id<OrgApacheCommonsMathLinearRealMatrix>)getVT {
  if (cachedVt_ == nil) {
    jint m = ((IOSObjectArray *) nil_chk(eigenvectors_))->size_;
    cachedVt_ = OrgApacheCommonsMathLinearMatrixUtils_createRealMatrixWithInt_withInt_(m, m);
    for (jint k = 0; k < m; ++k) {
...
- (id<OrgApacheCommonsMathLinearDecompositionSolver>)getSolver {
  return new_OrgApacheCommonsMathLinearEigenDecompositionImpl_Solver_initWithDoubleArray_withDoubleArray_withOrgApacheCommonsMathLinearArrayRealVectorArray_(realEigenvalues_, imagEigenvalues_, eigenvectors_);
}
...
void OrgApacheCommonsMathLinearEigenDecompositionImpl_initWithOrgApacheCommonsMathLinearRealMatrix_withDouble_(OrgApacheCommonsMathLinearEigenDecompositionImpl *self, id<OrgApacheCommonsMathLinearRealMatrix> matrix, jdouble splitTolerance) {
  ...
  if (OrgApacheCommonsMathLinearEigenDecompositionImpl_isSymmetricWithOrgApacheCommonsMathLinearRealMatrix_(self, matrix)) {
    ...
    @throw new_OrgApacheCommonsMathLinearInvalidMatrixException_initWithOrgApacheCommonsMathExceptionUtilLocalizable_withNSObjectArray_(OrgApacheCommonsMathExceptionUtilLocalizedFormatsEnum_get_ASSYMETRIC_EIGEN_NOT_SUPPORTED(), [IOSObjectArray newArrayWithLength:0 type:NSObject_class_()]);
  }
}
...
void OrgApacheCommonsMathLinearEigenDecompositionImpl_initWithDoubleArray_withDoubleArray_withDouble_(OrgApacheCommonsMathLinearEigenDecompositionImpl *self, IOSDoubleArray *main, IOSDoubleArray *secondary, jdouble splitTolerance) {
  (void) NSObject_init(self);
  self->maxIter_ = 30;
  self->main_ = [((IOSDoubleArray *) nil_chk(main)) clone];
  self->secondary_ = [((IOSDoubleArray *) nil_chk(secondary)) clone];
  self->transformer_ = nil;
  jint size = main->size_;
  IOSObjectArray *z = [IOSDoubleArray newArrayWithDimensions:2 lengths:(jint[]){ size, size }];
...
void OrgApacheCommonsMathLinearEigenDecompositionImpl_transformToTridiagonalWithOrgApacheCommonsMathLinearRealMatrix_(OrgApacheCommonsMathLinearEigenDecompositionImpl *self, id<OrgApacheCommonsMathLinearRealMatrix> matrix) {
  self->transformer_ = new_OrgApacheCommonsMathLinearTriDiagonalTransformer_initWithOrgApacheCommonsMathLinearRealMatrix_(matrix);
  self->main_ = [self->transformer_ getMainDiagonalRef];
  self->secondary_ = [self->transformer_ getSecondaryDiagonalRef];
}
...
void OrgApacheCommonsMathLinearEigenDecompositionImpl_findEigenVectorsWithDoubleArray2_(OrgApacheCommonsMathLinearEigenDecompositionImpl *self, IOSObjectArray *householderMatrix) {
  ...
  self->realEigenvalues_ = [IOSDoubleArray newArrayWithLength:n];
  self->imagEigenvalues_ = [IOSDoubleArray newArrayWithLength:n];
  IOSDoubleArray *e = [IOSDoubleArray newArrayWithLength:n];
  ...
  *IOSDoubleArray_GetRef(e, n - 1) = 0.0;
  ...
  for (jint j = 0; j < n; j++) {
    ...
    do {
      ...
      if (m != j) {
        if (its == self->maxIter_) @throw new_OrgApacheCommonsMathLinearInvalidMatrixException_initWithJavaLangThrowable_(new_OrgApacheCommonsMathMaxIterationsExceededException_initWithInt_(self->maxIter_));
        its++;
  ...
  self->eigenvectors_ = [IOSObjectArray newArrayWithLength:n type:OrgApacheCommonsMathLinearArrayRealVector_class_()];
  IOSDoubleArray *tmp = [IOSDoubleArray newArrayWithLength:n];
  for (jint i = 0; i < n; i++) {
    for (jint j = 0; j < n; j++) {
      *IOSDoubleArray_GetRef(tmp, j) = IOSDoubleArray_Get(nil_chk(IOSObjectArray_Get(nil_chk(z), j)), i);
    }
    (void) IOSObjectArray_Set(self->eigenvectors_, i, new_OrgApacheCommonsMathLinearArrayRealVector_initWithDoubleArray_(tmp));
  }
}
...
- (id<OrgApacheCommonsMathLinearRealMatrix>)getInverse {
  ...
  jint m = ((IOSDoubleArray *) nil_chk(realEigenvalues_))->size_;
  IOSObjectArray *invData = [IOSDoubleArray newArrayWithDimensions:2 lengths:(jint[]){ m, m }];
  for (jint i = 0; i < m; ++i) {
...
void OrgApacheCommonsMathLinearEigenDecompositionImpl_Solver_initWithDoubleArray_withDoubleArray_withOrgApacheCommonsMathLinearArrayRealVectorArray_(OrgApacheCommonsMathLinearEigenDecompositionImpl_Solver *self, IOSDoubleArray *realEigenvalues, IOSDoubleArray *imagEigenvalues, IOSObjectArray *eigenvectors) {
  (void) NSObject_init(self);
  self->realEigenvalues_ = realEigenvalues;
  self->imagEigenvalues_ = imagEigenvalues;
