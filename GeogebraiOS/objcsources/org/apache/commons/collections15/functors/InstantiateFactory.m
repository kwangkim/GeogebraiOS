...
- (id)create {
  ...
  @catch (JavaLangInstantiationException *ex) {
    @throw new_OrgApacheCommonsCollections15FunctorException_initWithNSString_withJavaLangThrowable_(@"InstantiateFactory: InstantiationException", ex);
  }
  @catch (JavaLangIllegalAccessException *ex) {
    @throw new_OrgApacheCommonsCollections15FunctorException_initWithNSString_withJavaLangThrowable_(@"InstantiateFactory: Constructor must be public", ex);
  }
  @catch (JavaLangReflectInvocationTargetException *ex) {
    @throw new_OrgApacheCommonsCollections15FunctorException_initWithNSString_withJavaLangThrowable_(@"InstantiateFactory: Constructor threw an exception", ex);
  }
}
...
id<OrgApacheCommonsCollections15Factory> OrgApacheCommonsCollections15FunctorsInstantiateFactory_getInstanceWithIOSClass_withIOSClassArray_withNSObjectArray_(IOSClass *classToInstantiate, IOSObjectArray *paramTypes, IOSObjectArray *args) {
  ...
  if (classToInstantiate == nil) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Class to instantiate must not be null");
  }
  ...
  if (paramTypes == nil || paramTypes->size_ == 0) {
    return new_OrgApacheCommonsCollections15FunctorsInstantiateFactory_initWithIOSClass_(classToInstantiate);
  }
  else {
    paramTypes = [paramTypes clone];
    args = [((IOSObjectArray *) nil_chk(args)) clone];
    return new_OrgApacheCommonsCollections15FunctorsInstantiateFactory_initWithIOSClass_withIOSClassArray_withNSObjectArray_(classToInstantiate, paramTypes, args);
  }
}
...
void OrgApacheCommonsCollections15FunctorsInstantiateFactory_initWithIOSClass_(OrgApacheCommonsCollections15FunctorsInstantiateFactory *self, IOSClass *classToInstantiate) {
  ...
  self->iConstructor_ = nil;
  self->iClassToInstantiate_ = classToInstantiate;
...
void OrgApacheCommonsCollections15FunctorsInstantiateFactory_findConstructor(OrgApacheCommonsCollections15FunctorsInstantiateFactory *self) {
  @try {
    self->iConstructor_ = [((IOSClass *) nil_chk(self->iClassToInstantiate_)) getConstructor:self->iParamTypes_];
  }
  @catch (JavaLangNoSuchMethodException *ex) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"InstantiateFactory: The constructor must exist and be public ");
  }
}
