...
- (id)transformWithId:(id)input {
  ...
  @catch (JavaLangNoSuchMethodException *ex) {
    @throw new_OrgApacheCommonsCollections15FunctorException_initWithNSString_(JreStrcat("$$$@$", @"InvokerTransformer: The method '", iMethodName_, @"' on '", [nil_chk(input) getClass], @"' does not exist"));
  }
  @catch (JavaLangIllegalAccessException *ex) {
    @throw new_OrgApacheCommonsCollections15FunctorException_initWithNSString_(JreStrcat("$$$@$", @"InvokerTransformer: The method '", iMethodName_, @"' on '", [nil_chk(input) getClass], @"' cannot be accessed"));
  }
  @catch (JavaLangReflectInvocationTargetException *ex) {
    @throw new_OrgApacheCommonsCollections15FunctorException_initWithNSString_withJavaLangThrowable_(JreStrcat("$$$@$", @"InvokerTransformer: The method '", iMethodName_, @"' on '", [nil_chk(input) getClass], @"' threw an exception"), ex);
  }
}
...
id<OrgApacheCommonsCollections15Transformer> OrgApacheCommonsCollections15FunctorsInvokerTransformer_getInstanceWithNSString_(NSString *methodName) {
  ...
  if (methodName == nil) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"The method to invoke must not be null");
  }
  return new_OrgApacheCommonsCollections15FunctorsInvokerTransformer_initWithNSString_(methodName);
}
...
id<OrgApacheCommonsCollections15Transformer> OrgApacheCommonsCollections15FunctorsInvokerTransformer_getInstanceWithNSString_withIOSClassArray_withNSObjectArray_(NSString *methodName, IOSObjectArray *paramTypes, IOSObjectArray *args) {
  ...
  if (((paramTypes == nil) && (args != nil)) || ((paramTypes != nil) && (args == nil)) || ((paramTypes != nil) && (args != nil) && (paramTypes->size_ != args->size_))) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"The parameter types must match the arguments");
  }
  if (paramTypes == nil || paramTypes->size_ == 0) {
    ...
    return new_OrgApacheCommonsCollections15FunctorsInvokerTransformer_initWithNSString_withIOSClassArray_withNSObjectArray_(methodName, paramTypes, args);
  }
}
