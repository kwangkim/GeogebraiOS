...
- (void)checkBoundWithInt:(jint)bound
  ...
  if (bound > self->endIndex_) {
    @throw new_JavaLangArrayIndexOutOfBoundsException_initWithNSString_(JreStrcat("$$$", @"Attempt to make an ArrayIterator that ", type, @"s beyond the end of the array. "));
  }
  if (bound < 0) {
    @throw new_JavaLangArrayIndexOutOfBoundsException_initWithNSString_(JreStrcat("$$$", @"Attempt to make an ArrayIterator that ", type, @"s before the start of the array. "));
  }
}
...
- (void)remove {
  @throw new_JavaLangUnsupportedOperationException_initWithNSString_(@"remove() method is not supported");
}
