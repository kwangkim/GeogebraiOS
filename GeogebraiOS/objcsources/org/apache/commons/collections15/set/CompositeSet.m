...
- (void)addCompositedWithJavaUtilCollection:(id<JavaUtilCollection>)c {
  @synchronized(self) {
    if (!([JavaUtilSet_class_() isInstance:c])) {
      @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Collections added must implement java.util.Set");
    }
    for (id<JavaUtilIterator> i = [((id<JavaUtilCollection>) nil_chk([self getCollections])) iterator]; [((id<JavaUtilIterator>) nil_chk(i)) hasNext]; ) {
      ...
      if ([((id<JavaUtilCollection>) nil_chk(intersects)) size] > 0) {
        if (self->mutator_ == nil) {
          @throw new_JavaLangUnsupportedOperationException_initWithNSString_(@"Collision adding composited collection with no SetMutator set");
        }
        else if (!([OrgApacheCommonsCollections15SetCompositeSet_SetMutator_class_() isInstance:self->mutator_])) {
          @throw new_JavaLangUnsupportedOperationException_initWithNSString_(@"Collision adding composited collection to a CompositeSet with a CollectionMutator instead of a SetMutator");
        }
        ...
        if ([((id<JavaUtilCollection>) nil_chk(OrgApacheCommonsCollections15CollectionUtils_intersectionWithJavaUtilCollection_withJavaUtilCollection_(set, c))) size] > 0) {
          @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Attempt to add illegal entry unresolved by SetMutator.resolveCollision()");
        }
      }
    }
    [super addCompositedWithJavaUtilCollectionArray:[IOSObjectArray newArrayWithObjects:(id[]){ c } count:1 type:JavaUtilCollection_class_()]];
  }
}
...
- (void)addCompositedWithJavaUtilCollection:(id<JavaUtilCollection>)c
  ...
  @synchronized(self) {
    if (!([JavaUtilSet_class_() isInstance:c])) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Argument must implement java.util.Set");
    if (!([JavaUtilSet_class_() isInstance:d])) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Argument must implement java.util.Set");
    [self addCompositedWithJavaUtilCollectionArray:[IOSObjectArray newArrayWithObjects:(id[]){ (id<JavaUtilSet>) check_protocol_cast(c, @protocol(JavaUtilSet)), (id<JavaUtilSet>) check_protocol_cast(d, @protocol(JavaUtilSet)) } count:2 type:JavaUtilSet_class_()]];
  }
}
...
void OrgApacheCommonsCollections15SetCompositeSet_initWithJavaUtilSet_(OrgApacheCommonsCollections15SetCompositeSet *self, id<JavaUtilSet> set) {
  (void) OrgApacheCommonsCollections15CollectionCompositeCollection_initWithJavaUtilCollection_(self, set);
}
...
void OrgApacheCommonsCollections15SetCompositeSet_initWithJavaUtilSetArray_(OrgApacheCommonsCollections15SetCompositeSet *self, IOSObjectArray *sets) {
  (void) OrgApacheCommonsCollections15CollectionCompositeCollection_initWithJavaUtilCollectionArray_(self, sets);
}
