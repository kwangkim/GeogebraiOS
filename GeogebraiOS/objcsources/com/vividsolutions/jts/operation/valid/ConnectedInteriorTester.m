...
- (jboolean)isInteriorsConnected {
  ...
  ComVividsolutionsJtsGeomgraphPlanarGraph *graph = [new_ComVividsolutionsJtsGeomgraphPlanarGraph_initWithComVividsolutionsJtsGeomgraphNodeFactory_([new_ComVividsolutionsJtsOperationOverlayOverlayNodeFactory_init() autorelease]) autorelease];
...
- (void)dealloc {
  ...
  RELEASE_(geomGraph_);
  RELEASE_(disconnectedRingcoord_);
  [super dealloc];
}
...
void ComVividsolutionsJtsOperationValidConnectedInteriorTester_initWithComVividsolutionsJtsGeomgraphGeometryGraph_(ComVividsolutionsJtsOperationValidConnectedInteriorTester *self, ComVividsolutionsJtsGeomgraphGeometryGraph *geomGraph) {
  NSObject_init(self);
  ComVividsolutionsJtsOperationValidConnectedInteriorTester_setAndConsume_geometryFactory_(self, new_ComVividsolutionsJtsGeomGeometryFactory_init());
  ComVividsolutionsJtsOperationValidConnectedInteriorTester_set_geomGraph_(self, geomGraph);
}
  ...
  for (jint i = 0; i < [((id<JavaUtilList>) nil_chk(edgeRings)) size]; i++) {
    ...
    for (jint j = 0; j < [edges size]; j++) {
      ...
      if (![((ComVividsolutionsJtsGeomgraphDirectedEdge *) nil_chk(de)) isVisited]) {
        ComVividsolutionsJtsOperationValidConnectedInteriorTester_set_disconnectedRingcoord_(self, [de getCoordinate]);
        return YES;
      }
    }
  }
