...
- (ComVividsolutionsJtsGeomPolygon *)createRectangle {
  ...
  IOSObjectArray *pts = [IOSObjectArray newArrayWithLength:4 * nSide + 1 type:ComVividsolutionsJtsGeomCoordinate_class_()];
...
- (ComVividsolutionsJtsGeomLineString *)createArcWithDouble:(jdouble)startAng
  ...
  IOSObjectArray *pts = [IOSObjectArray newArrayWithLength:nPts_ type:ComVividsolutionsJtsGeomCoordinate_class_()];
  jint iPt = 0;
  for (jint i = 0; i < nPts_; i++) {
...
- (ComVividsolutionsJtsGeomPolygon *)createArcPolygonWithDouble:(jdouble)startAng
  ...
  if (angSize <= 0.0 || angSize > 2 * JavaLangMath_PI) angSize = 2 * JavaLangMath_PI;
  jdouble angInc = angSize / (nPts_ - 1);
  IOSObjectArray *pts = [IOSObjectArray newArrayWithLength:nPts_ + 2 type:ComVividsolutionsJtsGeomCoordinate_class_()];
  jint iPt = 0;
  (void) IOSObjectArray_Set(pts, iPt++, [self createCoordWithDouble:centreX withDouble:centreY]);
  for (jint i = 0; i < nPts_; i++) {
...
void ComVividsolutionsJtsUtilGeometricShapeFactory_init(ComVividsolutionsJtsUtilGeometricShapeFactory *self) {
  (void) ComVividsolutionsJtsUtilGeometricShapeFactory_initWithComVividsolutionsJtsGeomGeometryFactory_(self, new_ComVividsolutionsJtsGeomGeometryFactory_init());
}
...
void ComVividsolutionsJtsUtilGeometricShapeFactory_initWithComVividsolutionsJtsGeomGeometryFactory_(ComVividsolutionsJtsUtilGeometricShapeFactory *self, ComVividsolutionsJtsGeomGeometryFactory *geomFact) {
  ...
  self->dim_ = new_ComVividsolutionsJtsUtilGeometricShapeFactory_Dimensions_initWithComVividsolutionsJtsUtilGeometricShapeFactory_(self);
  self->nPts_ = 100;
  self->geomFact_ = geomFact;
}
...
- (ComVividsolutionsJtsGeomEnvelope *)getEnvelope {
  if (base_ != nil) {
    return new_ComVividsolutionsJtsGeomEnvelope_initWithDouble_withDouble_withDouble_withDouble_(base_->x_, base_->x_ + width_, base_->y_, base_->y_ + height_);
  }
  if (centre_ != nil) {
    return new_ComVividsolutionsJtsGeomEnvelope_initWithDouble_withDouble_withDouble_withDouble_(centre_->x_ - width_ / 2, centre_->x_ + width_ / 2, centre_->y_ - height_ / 2, centre_->y_ + height_ / 2);
  }
  return new_ComVividsolutionsJtsGeomEnvelope_initWithDouble_withDouble_withDouble_withDouble_(0, width_, 0, height_);
}
