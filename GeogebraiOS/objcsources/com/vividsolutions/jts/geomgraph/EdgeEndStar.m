...
- (void)insertEdgeEndWithComVividsolutionsJtsGeomgraphEdgeEnd:(ComVividsolutionsJtsGeomgraphEdgeEnd *)e
                                                       withId:(id)obj {
  (void) [((id<JavaUtilMap>) nil_chk(edgeMap_)) putWithId:e withId:obj];
  edgeList_ = nil;
}
...
- (id<JavaUtilList>)getEdges {
  if (edgeList_ == nil) {
    edgeList_ = new_JavaUtilArrayList_initWithJavaUtilCollection_([((id<JavaUtilMap>) nil_chk(edgeMap_)) values]);
  }
  return edgeList_;
}
...
- (void)computeLabellingWithComVividsolutionsJtsGeomgraphGeometryGraphArray:(IOSObjectArray *)geomGraph {
  ...
  IOSBooleanArray *hasDimensionalCollapseEdge = [IOSBooleanArray newArrayWithBooleans:(jboolean[]){ NO, NO } count:2];
  ...
  for (id<JavaUtilIterator> it = [self iterator]; [((id<JavaUtilIterator>) nil_chk(it)) hasNext]; ) {
    ...
    if ([label isAreaWithInt:geomIndex]) {
      ...
      if (rightLoc != ComVividsolutionsJtsGeomLocation_NONE) {
        if (rightLoc != currLoc) @throw new_ComVividsolutionsJtsGeomTopologyException_initWithNSString_withComVividsolutionsJtsGeomCoordinate_(@"side location conflict", [e getCoordinate]);
...
void ComVividsolutionsJtsGeomgraphEdgeEndStar_init(ComVividsolutionsJtsGeomgraphEdgeEndStar *self) {
  ...
  self->edgeMap_ = new_JavaUtilTreeMap_init();
  self->ptInAreaLocation_ = [IOSIntArray newArrayWithInts:(jint[]){ ComVividsolutionsJtsGeomLocation_NONE, ComVividsolutionsJtsGeomLocation_NONE } count:2];
}
