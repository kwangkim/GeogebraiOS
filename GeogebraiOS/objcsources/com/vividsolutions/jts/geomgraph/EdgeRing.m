...
- (ComVividsolutionsJtsGeomPolygon *)toPolygonWithComVividsolutionsJtsGeomGeometryFactory:(ComVividsolutionsJtsGeomGeometryFactory *)geometryFactory {
  IOSObjectArray *holeLR = [IOSObjectArray newArrayWithLength:[((JavaUtilArrayList *) nil_chk(holes_)) size] type:ComVividsolutionsJtsGeomLinearRing_class_()];
  for (jint i = 0; i < [holes_ size]; i++) {
    (void) IOSObjectArray_Set(holeLR, i, [((ComVividsolutionsJtsGeomgraphEdgeRing *) nil_chk(((ComVividsolutionsJtsGeomgraphEdgeRing *) check_class_cast([holes_ getWithInt:i], [ComVividsolutionsJtsGeomgraphEdgeRing class])))) getLinearRing]);
  }
...
- (void)computeRing {
  if (ring_ != nil) return;
  IOSObjectArray *coord = [IOSObjectArray newArrayWithLength:[((id<JavaUtilList>) nil_chk(pts_)) size] type:ComVividsolutionsJtsGeomCoordinate_class_()];
  for (jint i = 0; i < [pts_ size]; i++) {
    (void) IOSObjectArray_Set(coord, i, (ComVividsolutionsJtsGeomCoordinate *) check_class_cast([pts_ getWithInt:i], [ComVividsolutionsJtsGeomCoordinate class]));
  }
  ring_ = [((ComVividsolutionsJtsGeomGeometryFactory *) nil_chk(geometryFactory_)) createLinearRingWithComVividsolutionsJtsGeomCoordinateArray:coord];
...
- (void)computePointsWithComVividsolutionsJtsGeomgraphDirectedEdge:(ComVividsolutionsJtsGeomgraphDirectedEdge *)start {
  ...
  do {
    if (de == nil) @throw new_ComVividsolutionsJtsGeomTopologyException_initWithNSString_(@"Found null DirectedEdge");
    if ([((ComVividsolutionsJtsGeomgraphDirectedEdge *) nil_chk(de)) getEdgeRing] == self) @throw new_ComVividsolutionsJtsGeomTopologyException_initWithNSString_(JreStrcat("$@", @"Directed Edge visited twice during ring-building at ", [de getCoordinate]));
...
void ComVividsolutionsJtsGeomgraphEdgeRing_initWithComVividsolutionsJtsGeomgraphDirectedEdge_withComVividsolutionsJtsGeomGeometryFactory_(ComVividsolutionsJtsGeomgraphEdgeRing *self, ComVividsolutionsJtsGeomgraphDirectedEdge *start, ComVividsolutionsJtsGeomGeometryFactory *geometryFactory) {
  ...
  self->label_ = new_ComVividsolutionsJtsGeomgraphLabel_initWithInt_(ComVividsolutionsJtsGeomLocation_NONE);
  self->holes_ = new_JavaUtilArrayList_init();
