...
- (jboolean)removeWithComVividsolutionsJtsGeomEnvelope:(ComVividsolutionsJtsGeomEnvelope *)itemEnv
  ...
  for (jint i = 0; i < 4; i++) {
    if (IOSObjectArray_Get(nil_chk(subnode_), i) != nil) {
      ...
      if (found) {
        if ([((ComVividsolutionsJtsIndexQuadtreeNode *) nil_chk(IOSObjectArray_Get(subnode_, i))) isPrunable]) (void) IOSObjectArray_Set(subnode_, i, nil);
        break;
      }
    }
  }
...
- (id<JavaUtilList>)addAllItemsWithJavaUtilList:(id<JavaUtilList>)resultItems {
  ...
  for (jint i = 0; i < 4; i++) {
    if (IOSObjectArray_Get(nil_chk(subnode_), i) != nil) {
      (void) [((ComVividsolutionsJtsIndexQuadtreeNode *) nil_chk(IOSObjectArray_Get(subnode_, i))) addAllItemsWithJavaUtilList:resultItems];
    }
  }
  return resultItems;
}
...
void ComVividsolutionsJtsIndexQuadtreeNodeBase_init(ComVividsolutionsJtsIndexQuadtreeNodeBase *self) {
  ...
  self->subnode_ = [IOSObjectArray newArrayWithLength:4 type:ComVividsolutionsJtsIndexQuadtreeNode_class_()];
}
