...
NSString *ComVividsolutionsJtsIoWKTWriter_toLineStringWithComVividsolutionsJtsGeomCoordinateSequence_(id<ComVividsolutionsJtsGeomCoordinateSequence> seq) {
  ...
  (void) [buf appendWithNSString:@"LINESTRING "];
  if ([((id<ComVividsolutionsJtsGeomCoordinateSequence>) nil_chk(seq)) size] == 0) (void) [buf appendWithNSString:@" EMPTY"];
  else {
    (void) [buf appendWithNSString:@"("];
    for (jint i = 0; i < [seq size]; i++) {
      if (i > 0) (void) [buf appendWithNSString:@", "];
      (void) [buf appendWithNSString:JreStrcat("DCD", [seq getXWithInt:i], ' ', [seq getYWithInt:i])];
    }
    (void) [buf appendWithNSString:@")"];
  }
...
void ComVividsolutionsJtsIoWKTWriter_initWithInt_(ComVividsolutionsJtsIoWKTWriter *self, jint outputDimension) {
  ...
  if (outputDimension < 2 || outputDimension > 3) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Invalid output dimension (must be 2 or 3)");
}
