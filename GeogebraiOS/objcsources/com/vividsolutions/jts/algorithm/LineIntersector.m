...
- (void)computeIntersectionWithComVividsolutionsJtsGeomCoordinate:(ComVividsolutionsJtsGeomCoordinate *)p1
  ...
  (void) IOSObjectArray_Set(nil_chk(IOSObjectArray_Get(nil_chk(inputLines_), 0)), 0, p1);
  (void) IOSObjectArray_Set(nil_chk(IOSObjectArray_Get(inputLines_, 0)), 1, p2);
  (void) IOSObjectArray_Set(nil_chk(IOSObjectArray_Get(inputLines_, 1)), 0, p3);
  (void) IOSObjectArray_Set(nil_chk(IOSObjectArray_Get(inputLines_, 1)), 1, p4);
...
- (void)computeIntLineIndex {
  if (intLineIndex_ == nil) {
    intLineIndex_ = [IOSIntArray newArrayWithDimensions:2 lengths:(jint[]){ 2, 2 }];
...
void ComVividsolutionsJtsAlgorithmLineIntersector_init(ComVividsolutionsJtsAlgorithmLineIntersector *self) {
  ...
  self->inputLines_ = [IOSObjectArray newArrayWithDimensions:2 lengths:(jint[]){ 2, 2 } type:ComVividsolutionsJtsGeomCoordinate_class_()];
  self->intPt_ = [IOSObjectArray newArrayWithLength:2 type:ComVividsolutionsJtsGeomCoordinate_class_()];
  self->precisionModel_ = nil;
  (void) IOSObjectArray_Set(self->intPt_, 0, new_ComVividsolutionsJtsGeomCoordinate_init());
  (void) IOSObjectArray_Set(self->intPt_, 1, new_ComVividsolutionsJtsGeomCoordinate_init());
  self->pa_ = IOSObjectArray_Get(self->intPt_, 0);
  self->pb_ = IOSObjectArray_Get(self->intPt_, 1);
  self->result_ = 0;
}
...
NSString *ComVividsolutionsJtsAlgorithmLineIntersector_getTopologySummary(ComVividsolutionsJtsAlgorithmLineIntersector *self) {
  JavaLangStringBuffer *catBuf = new_JavaLangStringBuffer_init();
  if ([self isEndPoint]) (void) [catBuf appendWithNSString:@" endpoint"];
  if (self->isProper__) (void) [catBuf appendWithNSString:@" proper"];
  if ([self isCollinear]) (void) [catBuf appendWithNSString:@" collinear"];
