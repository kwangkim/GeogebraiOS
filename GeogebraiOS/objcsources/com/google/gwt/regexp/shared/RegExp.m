...
//  source: /Users/jacky/GSoC/GeoGebraiOSARC/GeoGebraiOSARC/javasources/com/google/gwt/regexp/shared/RegExp.java
//
...
- (ComGoogleGwtRegexpSharedMatchResult *)execWithNSString:(NSString *)input {
  ...
  if (matcher != nil) {
    ...
    id<JavaUtilList> groups = new_JavaUtilArrayList_initWithInt_(1 + groupCount);
    ...
    return new_ComGoogleGwtRegexpSharedMatchResult_initWithInt_withNSString_withJavaUtilList_([matcher start], input, groups);
  }
  else {
    if (globalFlag_) {
      lastIndex_ = 0;
...
- (NSString *)replaceWithNSString:(NSString *)input
  ...
  if ([((JavaUtilRegexMatcher *) nil_chk([((JavaUtilRegexPattern *) nil_chk(ComGoogleGwtRegexpSharedRegExp_REPLACEMENT_DOLLAR_APOSTROPHE_)) matcherWithJavaLangCharSequence:replacement])) find]) {
    @throw new_JavaLangUnsupportedOperationException_initWithNSString_(@"$` and $' replacements are not supported");
  }
...
- (ComGoogleGwtRegexpSharedSplitResult *)splitWithNSString:(NSString *)input
  ...
  if (((jint) [((NSString *) nil_chk(source_)) length]) == 0) {
    ...
    result = [IOSObjectArray newArrayWithLength:resultLength type:NSString_class_()];
    for (jint i = 0; i < resultLength; i++) {
      (void) IOSObjectArray_Set(result, i, [input substring:i endIndex:i + 1]);
    }
  }
  else {
    ...
    if (((IOSObjectArray *) nil_chk(result))->size_ > limit && limit >= 0) {
      IOSObjectArray *realResult = [IOSObjectArray newArrayWithLength:limit type:NSString_class_()];
      for (jint i = 0; i < limit; i++) {
        (void) IOSObjectArray_Set(realResult, i, IOSObjectArray_Get(result, i));
      }
      result = realResult;
    }
  }
  return new_ComGoogleGwtRegexpSharedSplitResult_initWithNSStringArray_(result);
}
...
+ (void)initialize {
  if (self == [ComGoogleGwtRegexpSharedRegExp class]) {
    ComGoogleGwtRegexpSharedRegExp_REPLACEMENT_BACKSLASH_ = JavaUtilRegexPattern_compileWithNSString_(@"\\\\");
    ComGoogleGwtRegexpSharedRegExp_REPLACEMENT_DOLLAR_AMPERSAND_ = JavaUtilRegexPattern_compileWithNSString_(@"((?:^|\\G|[^$])(?:\\$\\$)*)\\$&");
    ComGoogleGwtRegexpSharedRegExp_REPLACEMENT_DOLLAR_APOSTROPHE_ = JavaUtilRegexPattern_compileWithNSString_(@"(?:^|[^$])(?:\\$\\$)*\\$[`']");
    ComGoogleGwtRegexpSharedRegExp_REPLACEMENT_DOLLAR_DOLLAR_ = JavaUtilRegexPattern_compileWithNSString_(@"\\$\\$");
...
ComGoogleGwtRegexpSharedRegExp *ComGoogleGwtRegexpSharedRegExp_compileWithNSString_withNSString_(NSString *pattern, NSString *flags) {
  ...
  for (JavaLangCharacter *boxed__ in nil_chk(ComGoogleGwtRegexpSharedRegExp_parseFlagsWithNSString_(flags))) {
    ...
    switch (flag) {
      ...
      break;
      default:
      @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$CC", @"Unknown regexp flag: '", flag, '\''));
    }
  }
  JavaUtilRegexPattern *javaPattern = JavaUtilRegexPattern_compileWithNSString_withInt_(pattern, javaPatternFlags);
  return new_ComGoogleGwtRegexpSharedRegExp_initWithNSString_withJavaUtilRegexPattern_withBoolean_(pattern, javaPattern, globalFlag);
}
...
id<JavaUtilSet> ComGoogleGwtRegexpSharedRegExp_parseFlagsWithNSString_(NSString *flags) {
  ComGoogleGwtRegexpSharedRegExp_initialize();
  id<JavaUtilSet> flagsSet = new_JavaUtilHashSet_initWithInt_(((jint) [((NSString *) nil_chk(flags)) length]));
  for (jint flagIndex = 0; flagIndex < ((jint) [flags length]); flagIndex++) {
    ...
    if (![flagsSet addWithId:JavaLangCharacter_valueOfWithChar_(flag)]) {
      @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$CC", @"Flag cannot be specified twice: '", flag, '\''));
    }
  }
  return flagsSet;
}
