...
- (id<JavaUtilSet>)transformWithId:(id<EduUciIcsJungGraphGraph>)graph {
  if (mNumEdgesToRemove_ < 0 || mNumEdgesToRemove_ > [((id<EduUciIcsJungGraphGraph>) nil_chk(graph)) getEdgeCount]) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Invalid number of edges passed in.");
  }
  [((id<JavaUtilMap>) nil_chk(edges_removed_)) clear];
  for (jint k = 0; k < mNumEdgesToRemove_; k++) {
    EduUciIcsJungAlgorithmsScoringBetweennessCentrality *bc = new_EduUciIcsJungAlgorithmsScoringBetweennessCentrality_initWithEduUciIcsJungGraphGraph_(graph);
    id to_remove = nil;
    jdouble score = 0;
    ...
    (void) [edges_removed_ putWithId:to_remove withId:[graph getEndpointsWithId:to_remove]];
    [graph removeEdgeWithId:to_remove];
  }
  EduUciIcsJungAlgorithmsClusterWeakComponentClusterer *wcSearch = new_EduUciIcsJungAlgorithmsClusterWeakComponentClusterer_init();
...
- (id<JavaUtilList>)getEdgesRemoved {
  return new_JavaUtilArrayList_initWithJavaUtilCollection_([((id<JavaUtilMap>) nil_chk(edges_removed_)) keySet]);
}
...
void EduUciIcsJungAlgorithmsClusterEdgeBetweennessClusterer_initWithInt_(EduUciIcsJungAlgorithmsClusterEdgeBetweennessClusterer *self, jint numEdgesToRemove) {
  ...
  self->edges_removed_ = new_JavaUtilLinkedHashMap_init();
}
