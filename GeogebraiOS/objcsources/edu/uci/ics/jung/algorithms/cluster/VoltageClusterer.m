...
- (void)setRandomSeedWithInt:(jint)random_seed {
  rand_ = new_JavaUtilRandom_initWithLong_(random_seed);
}
...
- (id<JavaUtilCollection>)cluster_internalWithId:(id)origin
  ...
  JavaUtilArrayList *v_array = new_JavaUtilArrayList_initWithJavaUtilCollection_([((id<EduUciIcsJungGraphGraph>) nil_chk(g_)) getVertices]);
  JavaUtilLinkedList *candidates = new_JavaUtilLinkedList_init();
  for (jint j = 0; j < num_candidates_; j++) {
    ...
    while (source == target);
    EduUciIcsJungAlgorithmsScoringVoltageScorer *vs = new_EduUciIcsJungAlgorithmsScoringVoltageScorer_initWithEduUciIcsJungGraphHypergraph_withId_withId_(g_, source, target);
    [vs evaluate];
    id<JavaUtilMap> voltage_ranks = new_JavaUtilHashMap_init();
    for (id __strong v in nil_chk([g_ getVertices])) (void) [voltage_ranks putWithId:v withId:[IOSDoubleArray newArrayWithDoubles:(jdouble[]){ [((JavaLangDouble *) nil_chk([vs getVertexScoreWithId:v])) doubleValue] } count:1]];
    [self addTwoCandidateClustersWithJavaUtilLinkedList:candidates withJavaUtilMap:voltage_ranks];
  }
  id<JavaUtilCollection> clusters = new_JavaUtilLinkedList_init();
  id<JavaUtilSet> remaining = new_JavaUtilHashSet_initWithJavaUtilCollection_([g_ getVertices]);
...
- (void)addTwoCandidateClustersWithJavaUtilLinkedList:(JavaUtilLinkedList *)candidates
  ...
  @try {
    id<JavaUtilList> clusters = new_JavaUtilArrayList_initWithJavaUtilCollection_([((EduUciIcsJungAlgorithmsUtilKMeansClusterer *) nil_chk(kmc_)) clusterWithJavaUtilMap:voltage_ranks withInt:3]);
...
- (void)addOneCandidateClusterWithJavaUtilLinkedList:(JavaUtilLinkedList *)candidates
  ...
  @try {
    ...
    clusters = new_JavaUtilArrayList_initWithJavaUtilCollection_([((EduUciIcsJungAlgorithmsUtilKMeansClusterer *) nil_chk(kmc_)) clusterWithJavaUtilMap:voltage_ranks withInt:2]);
...
- (id<JavaUtilList>)getSeedCandidatesWithJavaUtilCollection:(id<JavaUtilCollection>)candidates {
  ...
  JavaUtilArrayList *occurrences = new_JavaUtilArrayList_initWithJavaUtilCollection_([((id<JavaUtilMap>) nil_chk(occur_counts)) keySet]);
  JavaUtilCollections_sortWithJavaUtilList_withJavaUtilComparator_(occurrences, new_EduUciIcsJungAlgorithmsClusterVoltageClusterer_MapValueArrayComparator_initWithEduUciIcsJungAlgorithmsClusterVoltageClusterer_withJavaUtilMap_(self, occur_counts));
...
- (id<JavaUtilMap>)getObjectCountsWithJavaUtilCollection:(id<JavaUtilCollection>)candidates
                                                  withId:(id)seed {
  id<JavaUtilMap> occur_counts = new_JavaUtilHashMap_init();
  for (id __strong v in nil_chk([((id<EduUciIcsJungGraphGraph>) nil_chk(g_)) getVertices])) (void) [occur_counts putWithId:v withId:[IOSDoubleArray newArrayWithDoubles:(jdouble[]){ 0 } count:1]];
...
void EduUciIcsJungAlgorithmsClusterVoltageClusterer_initWithEduUciIcsJungGraphGraph_withInt_(EduUciIcsJungAlgorithmsClusterVoltageClusterer *self, id<EduUciIcsJungGraphGraph> g, jint num_candidates) {
  ...
  if (num_candidates < 1) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"must generate >=1 candidates");
  self->num_candidates_ = num_candidates;
  self->kmc_ = new_EduUciIcsJungAlgorithmsUtilKMeansClusterer_init();
  self->rand_ = new_JavaUtilRandom_init();
  self->g_ = g;
}
