...
- (id<JavaUtilSet>)transformWithId:(id<EduUciIcsJungGraphUndirectedGraph>)theGraph {
  id<JavaUtilSet> bicomponents = new_JavaUtilLinkedHashSet_init();
  ...
  dfs_num_ = new_JavaUtilHashMap_init();
  for (id __strong v in nil_chk([theGraph getVertices])) {
    (void) [dfs_num_ putWithId:v withId:JavaLangInteger_valueOfWithInt_(0)];
  }
  for (id __strong v in nil_chk([theGraph getVertices])) {
    if ([((NSNumber *) nil_chk([dfs_num_ getWithId:v])) intValue] == 0) {
      high_ = new_JavaUtilHashMap_init();
      stack_ = new_JavaUtilStack_init();
      parents_ = new_JavaUtilHashMap_init();
...
- (void)findBiconnectedComponentsWithEduUciIcsJungGraphUndirectedGraph:(id<EduUciIcsJungGraphUndirectedGraph>)g
  ...
  (void) [((id<JavaUtilMap>) nil_chk(dfs_num_)) putWithId:v withId:JavaLangInteger_valueOfWithInt_(v_dfs_num)];
  converse_depth_--;
  (void) [((id<JavaUtilMap>) nil_chk(high_)) putWithId:v withId:JavaLangInteger_valueOfWithInt_(v_dfs_num)];
  for (id __strong w in nil_chk([((id<EduUciIcsJungGraphUndirectedGraph>) nil_chk(g)) getNeighborsWithId:v])) {
    ...
    if (w_dfs_num == 0) {
      (void) [((id<JavaUtilMap>) nil_chk(parents_)) putWithId:w withId:v];
      (void) [((JavaUtilStack *) nil_chk(stack_)) pushWithId:vw];
      ...
      if (w_high <= v_dfs_num) {
        id<JavaUtilSet> bicomponent = new_JavaUtilHashSet_init();
        id e;
        do {
          e = [stack_ pop];
      ...
      (void) [high_ putWithId:v withId:JavaLangInteger_valueOfWithInt_(JavaLangMath_maxWithInt_withInt_(w_high, [((NSNumber *) nil_chk([high_ getWithId:v])) intValue]))];
    }
    else if (w != [((id<JavaUtilMap>) nil_chk(parents_)) getWithId:v]) (void) [high_ putWithId:v withId:JavaLangInteger_valueOfWithInt_(JavaLangMath_maxWithInt_withInt_(w_dfs_num, [((NSNumber *) nil_chk([high_ getWithId:v])) intValue]))];
  }
}
