...
//  source: /Users/jacky/GSoC/GeoGebraiOSARC/GeoGebraiOSARC/javasources/edu/uci/ics/jung/algorithms/blockmodel/StructurallyEquivalent.java
//
...
- (EduUciIcsJungAlgorithmsBlockmodelVertexPartition *)transformWithId:(id<EduUciIcsJungGraphGraph>)g {
  ...
  id<JavaUtilSet> rv = new_JavaUtilHashSet_init();
  id<JavaUtilMap> intermediate = new_JavaUtilHashMap_init();
  for (EduUciIcsJungGraphUtilPair * __strong p in nil_chk(vertex_pairs)) {
    ...
    if (res == nil) res = new_JavaUtilHashSet_init();
    [((id<JavaUtilSet>) nil_chk(res)) addWithId:[p getFirst]];
    [res addWithId:[p getSecond]];
    (void) [intermediate putWithId:[p getFirst] withId:res];
    (void) [intermediate putWithId:[p getSecond] withId:res];
  }
  ...
  for (id __strong v in nil_chk(singletons)) {
    ...
    (void) [intermediate putWithId:v withId:v_set];
    [rv addWithId:v_set];
  }
  return new_EduUciIcsJungAlgorithmsBlockmodelVertexPartition_initWithEduUciIcsJungGraphGraph_withJavaUtilMap_withJavaUtilCollection_(g, intermediate, rv);
}

- (id<JavaUtilSet>)getEquivalentPairsWithEduUciIcsJungGraphGraph:(id<EduUciIcsJungGraphGraph>)g {
  id<JavaUtilSet> rv = new_JavaUtilHashSet_init();
  id<JavaUtilSet> alreadyEquivalent = new_JavaUtilHashSet_init();
  id<JavaUtilList> l = new_JavaUtilArrayList_initWithJavaUtilCollection_([((id<EduUciIcsJungGraphGraph>) nil_chk(g)) getVertices]);
  for (id __strong v1 in l) {
    ...
    for (id<JavaUtilIterator> iterator = [l listIteratorWithInt:[l indexOfWithId:v1] + 1]; [((id<JavaUtilIterator>) nil_chk(iterator)) hasNext]; ) {
      ...
      if ([self isStructurallyEquivalentWithEduUciIcsJungGraphGraph:g withId:v1 withId:v2]) {
        EduUciIcsJungGraphUtilPair *p = new_EduUciIcsJungGraphUtilPair_initWithId_withId_(v1, v2);
...
- (jboolean)isStructurallyEquivalentWithEduUciIcsJungGraphGraph:(id<EduUciIcsJungGraphGraph>)g
  ...
  id<JavaUtilSet> n1 = new_JavaUtilHashSet_initWithJavaUtilCollection_([g getPredecessorsWithId:v1]);
  [n1 removeWithId:v2];
  [n1 removeWithId:v1];
  id<JavaUtilSet> n2 = new_JavaUtilHashSet_initWithJavaUtilCollection_([g getPredecessorsWithId:v2]);
  [n2 removeWithId:v1];
  [n2 removeWithId:v2];
  id<JavaUtilSet> o1 = new_JavaUtilHashSet_initWithJavaUtilCollection_([g getSuccessorsWithId:v1]);
  id<JavaUtilSet> o2 = new_JavaUtilHashSet_initWithJavaUtilCollection_([g getSuccessorsWithId:v2]);
  [o1 removeWithId:v1];
  [o1 removeWithId:v2];
  [o2 removeWithId:v1];
  [o2 removeWithId:v2];
