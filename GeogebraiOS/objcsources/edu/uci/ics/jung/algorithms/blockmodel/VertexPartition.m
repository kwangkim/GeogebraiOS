...
- (id<JavaUtilMap>)getVertexToPartitionMap {
  if (vertex_partition_map_ == nil) {
    self->vertex_partition_map_ = new_JavaUtilHashMap_init();
    for (id<JavaUtilSet> __strong set in nil_chk(self->vertex_sets_)) for (id __strong v in nil_chk(set)) (void) [self->vertex_partition_map_ putWithId:v withId:set];
  }
...
- (id<JavaUtilCollection>)getVertexPartitions {
  if (vertex_sets_ == nil) {
    self->vertex_sets_ = new_JavaUtilHashSet_init();
...
void EduUciIcsJungAlgorithmsBlockmodelVertexPartition_initWithEduUciIcsJungGraphGraph_withJavaUtilMap_(EduUciIcsJungAlgorithmsBlockmodelVertexPartition *self, id<EduUciIcsJungGraphGraph> g, id<JavaUtilMap> partition_map) {
  ...
  self->vertex_partition_map_ = JavaUtilCollections_unmodifiableMapWithJavaUtilMap_(partition_map);
  self->graph_ = g;
}
