...
//  source: /Users/jacky/GSoC/GeoGebraiOSARC/GeoGebraiOSARC/javasources/edu/uci/ics/jung/algorithms/flows/EdmondsKarpMaxFlow.java
//
...
- (jboolean)hasAugmentingPath {
  ...
  id<JavaUtilSet> visitedEdgesMap = new_JavaUtilHashSet_init();
  ...
  while (![queue isEmpty]) {
    ...
    for (id __strong neighboringEdge in nil_chk(neighboringEdges)) {
      ...
      if ((neighborsParent == nil) || newCapacity > [((NSNumber *) nil_chk(neighborCapacity)) intValue]) {
        (void) [parentMap_ putWithId:neighboringVertex withId:currentVertex];
        (void) [parentCapacityMap_ putWithId:neighboringVertex withId:new_JavaLangInteger_initWithInt_(newCapacity)];
...
- (void)initializeIterations {
  (void) [((id<JavaUtilMap>) nil_chk(parentCapacityMap_)) putWithId:source_ withId:JavaLangInteger_valueOfWithInt_(JavaLangInteger_MAX_VALUE)];
  (void) [((id<JavaUtilMap>) nil_chk(parentMap_)) putWithId:source_ withId:source_];
  id<JavaUtilList> edgeList = new_JavaUtilArrayList_initWithJavaUtilCollection_([((id<EduUciIcsJungGraphDirectedGraph>) nil_chk(mFlowGraph_)) getEdges]);
  for (jint eIdx = 0; eIdx < [edgeList size]; eIdx++) {
    ...
    if (capacity == nil) {
      @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Edge capacities must be provided in Transformer passed to constructor");
    }
    (void) [((id<JavaUtilMap>) nil_chk(residualCapacityMap_)) putWithId:edge withId:capacity];
    ...
    if ([mFlowGraph_ isPredecessorWithId:source withId:destination] == NO) {
      ...
      (void) [residualCapacityMap_ putWithId:backEdge withId:JavaLangInteger_valueOfWithInt_(0)];
    }
  }
}
...
- (void)finalizeIterations {
  for (id __strong currentEdge in nil_chk([((id<EduUciIcsJungGraphDirectedGraph>) nil_chk(mFlowGraph_)) getEdges])) {
    ...
    if (capacity != nil) {
      JavaLangInteger *flowValue = new_JavaLangInteger_initWithInt_([capacity intValue] - [((NSNumber *) nil_chk(residualCapacity)) intValue]);
      (void) [((id<JavaUtilMap>) nil_chk(self->edgeFlowMap_)) putWithId:currentEdge withId:flowValue];
    }
  }
  id<JavaUtilSet> backEdges = new_JavaUtilHashSet_init();
  for (id __strong currentEdge in nil_chk([mFlowGraph_ getEdges])) {
    if ([((id<OrgApacheCommonsCollections15Transformer>) nil_chk(edgeCapacityTransformer_)) transformWithId:currentEdge] == nil) {
      ...
      (void) [((id<JavaUtilMap>) nil_chk(residualCapacityMap_)) removeWithId:currentEdge];
    }
  }
...
void EduUciIcsJungAlgorithmsFlowsEdmondsKarpMaxFlow_initWithEduUciIcsJungGraphDirectedGraph_withId_withId_withOrgApacheCommonsCollections15Transformer_withJavaUtilMap_withOrgApacheCommonsCollections15Factory_(EduUciIcsJungAlgorithmsFlowsEdmondsKarpMaxFlow *self, id<EduUciIcsJungGraphDirectedGraph> directedGraph, id source, id sink, id<OrgApacheCommonsCollections15Transformer> edgeCapacityTransformer, id<JavaUtilMap> edgeFlowMap, id<OrgApacheCommonsCollections15Factory> edgeFactory) {
  (void) EduUciIcsJungAlgorithmsUtilIterativeProcess_init(self);
  self->residualCapacityMap_ = new_JavaUtilHashMap_init();
  self->parentMap_ = new_JavaUtilHashMap_init();
  self->parentCapacityMap_ = new_JavaUtilHashMap_init();
  if ([((id<JavaUtilCollection>) nil_chk([((id<EduUciIcsJungGraphDirectedGraph>) nil_chk(directedGraph)) getVertices])) containsWithId:source] == NO || [((id<JavaUtilCollection>) nil_chk([directedGraph getVertices])) containsWithId:sink] == NO) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"source and sink vertices must be elements of the specified graph");
  }
  if ([nil_chk(source) isEqual:sink]) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"source and sink vertices must be distinct");
  }
  self->mOriginalGraph_ = directedGraph;
  self->source_ = source;
  self->target_ = sink;
  self->edgeFlowMap_ = edgeFlowMap;
  self->edgeCapacityTransformer_ = edgeCapacityTransformer;
  self->edgeFactory_ = edgeFactory;
  @try {
    self->mFlowGraph_ = (id<EduUciIcsJungGraphDirectedGraph>) check_protocol_cast([directedGraph newInstance], @protocol(EduUciIcsJungGraphDirectedGraph));
  ...
  self->mMaxFlow_ = 0;
  self->mSinkPartitionNodes_ = new_JavaUtilHashSet_init();
  self->mSourcePartitionNodes_ = new_JavaUtilHashSet_init();
  self->mMinCutEdges_ = new_JavaUtilHashSet_init();
}
...
void EduUciIcsJungAlgorithmsFlowsEdmondsKarpMaxFlow_clearParentValues(EduUciIcsJungAlgorithmsFlowsEdmondsKarpMaxFlow *self) {
  ...
  (void) [self->parentCapacityMap_ putWithId:self->source_ withId:JavaLangInteger_valueOfWithInt_(JavaLangInteger_MAX_VALUE)];
  (void) [self->parentMap_ putWithId:self->source_ withId:self->source_];
}
...
void EduUciIcsJungAlgorithmsFlowsEdmondsKarpMaxFlow_updateResidualCapacities(EduUciIcsJungAlgorithmsFlowsEdmondsKarpMaxFlow *self) {
  ...
  while ((parentVertex = [((id<JavaUtilMap>) nil_chk(self->parentMap_)) getWithId:currentVertex]) != currentVertex) {
    ...
    (void) [self->residualCapacityMap_ putWithId:currentEdge withId:residualCapacity];
    ...
    residualCapacity = JavaLangInteger_valueOfWithInt_([((NSNumber *) nil_chk(residualCapacity)) intValue] + [augmentingPathCapacity intValue]);
    (void) [self->residualCapacityMap_ putWithId:backEdge withId:residualCapacity];
