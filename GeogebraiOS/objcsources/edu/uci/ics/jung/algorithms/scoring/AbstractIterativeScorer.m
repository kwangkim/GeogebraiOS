...
- (void)setOutputValueWithId:(id)v
                      withId:(id)value {
  (void) [((id<JavaUtilMap>) nil_chk(output_)) putWithId:v withId:value];
}
...
- (void)setCurrentValueWithId:(id)v
                       withId:(id)value {
  (void) [((id<JavaUtilMap>) nil_chk(current_values_)) putWithId:v withId:value];
}
...
- (void)initialize__ {
  ...
  self->current_values_ = new_JavaUtilHashMap_init();
...
- (id)getVertexScoreWithId:(id)v {
  if (![((id<EduUciIcsJungGraphHypergraph>) nil_chk(graph_)) containsVertexWithId:v]) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@$", @"Vertex ", v, @" not an element of this graph"));
  return [((id<JavaUtilMap>) nil_chk(output_)) getWithId:v];
}
...
- (void)setEdgeWeightsWithOrgApacheCommonsCollections15Transformer:(id<OrgApacheCommonsCollections15Transformer>)edge_weights {
  self->edge_weights_ = new_EduUciIcsJungAlgorithmsScoringUtilDelegateToEdgeTransformer_initWithOrgApacheCommonsCollections15Transformer_(edge_weights);
}

- (NSNumber *)getEdgeWeightWithId:(id)v
                           withId:(id)e {
  return [((id<OrgApacheCommonsCollections15Transformer>) nil_chk(edge_weights_)) transformWithId:new_EduUciIcsJungAlgorithmsScoringUtilVEPair_initWithId_withId_(v, e)];
}
