...
- (void)setMaxIterationsWithInt:(jint)max_iterations {
  if (max_iterations < 0) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"max iterations must be >= 0");
...
- (void)setConvergenceThresholdWithDouble:(jdouble)convergence_threshold {
  if (convergence_threshold <= 0) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"convergence threshold must be > 0");
...
- (id<JavaUtilCollection>)clusterWithJavaUtilMap:(id<JavaUtilMap>)object_locations
  ...
  if (object_locations == nil || [object_locations isEmpty]) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"'objects' must be non-empty");
  if (num_clusters < 2 || num_clusters > [((id<JavaUtilMap>) nil_chk(object_locations)) size]) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$IC", @"number of clusters must be >= 2 and <= number of objects (", [((id<JavaUtilMap>) nil_chk(object_locations)) size], ')'));
  id<JavaUtilSet> centroids = new_JavaUtilHashSet_init();
  IOSObjectArray *obj_array = [((id<JavaUtilSet>) nil_chk([((id<JavaUtilMap>) nil_chk(object_locations)) keySet])) toArray];
  id<JavaUtilSet> tried = new_JavaUtilHashSet_init();
  ...
  if ([tried size] >= [object_locations size]) @throw new_EduUciIcsJungAlgorithmsUtilKMeansClusterer_NotEnoughClustersException_init();
  ...
  while (iterations++ < max_iterations_ && max_movement > convergence_threshold_) {
    max_movement = 0;
    id<JavaUtilSet> new_centroids = new_JavaUtilHashSet_init();
    for (id<JavaUtilMap_Entry> __strong entry_ in nil_chk([((id<JavaUtilMap>) nil_chk(clusterMap)) entrySet])) {
      ...
      JavaUtilArrayList *locations = new_JavaUtilArrayList_initWithJavaUtilCollection_([((id<JavaUtilMap>) nil_chk(elements)) values]);
...
- (id<JavaUtilMap>)assignToClustersWithJavaUtilMap:(id<JavaUtilMap>)object_locations
  ...
  id<JavaUtilMap> clusterMap = new_JavaUtilHashMap_init();
  for (IOSDoubleArray * __strong centroid in nil_chk(centroids)) (void) [clusterMap putWithId:centroid withId:new_JavaUtilHashMap_init()];
  for (id<JavaUtilMap_Entry> __strong object_location in nil_chk([((id<JavaUtilMap>) nil_chk(object_locations)) entrySet])) {
    ...
    (void) [((id<JavaUtilMap>) nil_chk([clusterMap getWithId:closest])) putWithId:object withId:location];
  }
  return clusterMap;
}

- (void)setSeedWithInt:(jint)random_seed {
  self->rand_ = new_JavaUtilRandom_initWithLong_(random_seed);
}
...
void EduUciIcsJungAlgorithmsUtilKMeansClusterer_init(EduUciIcsJungAlgorithmsUtilKMeansClusterer *self) {
  (void) EduUciIcsJungAlgorithmsUtilKMeansClusterer_initWithInt_withDouble_(self, 100, 0.001);
}
