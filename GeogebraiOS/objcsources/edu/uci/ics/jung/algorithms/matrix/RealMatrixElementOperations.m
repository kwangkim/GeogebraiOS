...
- (void)mergePathsWithId:(id)e
  ...
  if (ed == nil) {
    (void) [edgeData_ putWithId:e withId:pd];
  }
  else {
    (void) [edgeData_ putWithId:e withId:JavaLangDouble_valueOfWithDouble_([ed doubleValue] + [((NSNumber *) nil_chk(pd)) doubleValue])];
  }
}
...
void EduUciIcsJungAlgorithmsMatrixRealMatrixElementOperations_initWithJavaUtilMap_(EduUciIcsJungAlgorithmsMatrixRealMatrixElementOperations *self, id<JavaUtilMap> edgeData) {
  ...
  self->edgeData_ = new_JavaUtilHashMap_init();
  self->edgeData_ = edgeData;
}
