...
id<JavaUtilMap> EduUciIcsJungAlgorithmsMetricsMetrics_clusteringCoefficientsWithEduUciIcsJungGraphGraph_(id<EduUciIcsJungGraphGraph> graph) {
  ...
  id<JavaUtilMap> coefficients = new_JavaUtilHashMap_init();
  for (id __strong v in nil_chk([((id<EduUciIcsJungGraphGraph>) nil_chk(graph)) getVertices])) {
    jint n = [graph getNeighborCountWithId:v];
    if (n < 2) (void) [coefficients putWithId:v withId:new_JavaLangDouble_initWithDouble_(0)];
    else {
      JavaUtilArrayList *neighbors = new_JavaUtilArrayList_initWithJavaUtilCollection_([graph getNeighborsWithId:v]);
      ...
      (void) [coefficients putWithId:v withId:new_JavaLangDouble_initWithDouble_(edge_count / possible_edges)];
    }
  }
  return coefficients;
}
