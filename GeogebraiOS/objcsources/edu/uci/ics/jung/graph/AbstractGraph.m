...
withEduUciIcsJungGraphUtilEdgeTypeEnum:(EduUciIcsJungGraphUtilEdgeTypeEnum *)edgeType {
  if (vertices == nil) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"'vertices' parameter must not be null");
  if ([((id<JavaUtilCollection>) nil_chk(vertices)) size] == 2) return [self addEdgeWithId:edge withEduUciIcsJungGraphUtilPair:[vertices isKindOfClass:[EduUciIcsJungGraphUtilPair class]] ? (EduUciIcsJungGraphUtilPair *) check_class_cast(vertices, [EduUciIcsJungGraphUtilPair class]) : new_EduUciIcsJungGraphUtilPair_initWithJavaUtilCollection_(vertices) withEduUciIcsJungGraphUtilEdgeTypeEnum:edgeType];
  else if ([vertices size] == 1) {
    id vertex = [((id<JavaUtilIterator>) nil_chk([vertices iterator])) next];
    return [self addEdgeWithId:edge withEduUciIcsJungGraphUtilPair:new_EduUciIcsJungGraphUtilPair_initWithId_withId_(vertex, vertex) withEduUciIcsJungGraphUtilEdgeTypeEnum:edgeType];
  }
  else @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$I", @"Graph objects connect 1 or 2 vertices; vertices arg has ", [vertices size]));
}
...
withEduUciIcsJungGraphUtilEdgeTypeEnum:(EduUciIcsJungGraphUtilEdgeTypeEnum *)edge_type {
  return [self addEdgeWithId:e withEduUciIcsJungGraphUtilPair:new_EduUciIcsJungGraphUtilPair_initWithId_withId_(v1, v2) withEduUciIcsJungGraphUtilEdgeTypeEnum:edge_type];
}
...
- (EduUciIcsJungGraphUtilPair *)getValidatedEndpointsWithId:(id)edge
                             withEduUciIcsJungGraphUtilPair:(EduUciIcsJungGraphUtilPair *)endpoints {
  if (edge == nil) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"input edge may not be null");
  if (endpoints == nil) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"endpoints may not be null");
  EduUciIcsJungGraphUtilPair *new_endpoints = new_EduUciIcsJungGraphUtilPair_initWithId_withId_([((EduUciIcsJungGraphUtilPair *) nil_chk(endpoints)) getFirst], [endpoints getSecond]);
  if ([self containsEdgeWithId:edge]) {
    ...
    if (![((EduUciIcsJungGraphUtilPair *) nil_chk(existing_endpoints)) isEqual:new_endpoints]) {
      @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@$@$@", @"edge ", edge, @" already exists in this graph with endpoints ", existing_endpoints, @" and cannot be added with endpoints ", endpoints));
    }
    else {
      return nil;
    }
...
- (jboolean)isNeighborWithId:(id)v1
                      withId:(id)v2 {
  if (![self containsVertexWithId:v1] || ![self containsVertexWithId:v2]) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@$@", @"At least one of these not in this graph: ", v1, @", ", v2));
  return [((id<JavaUtilCollection>) nil_chk([self getNeighborsWithId:v1])) containsWithId:v2];
}

- (jboolean)isIncidentWithId:(id)vertex
                      withId:(id)edge {
  if (![self containsVertexWithId:vertex] || ![self containsEdgeWithId:edge]) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@$@", @"At least one of these not in this graph: ", vertex, @", ", edge));
...
- (jint)getNeighborCountWithId:(id)vertex {
  if (![self containsVertexWithId:vertex]) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("@$", vertex, @" is not a vertex in this graph"));
...
- (id)getOppositeWithId:(id)vertex
  ...
  else @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("@$@$", vertex, @" is not incident to ", edge, @" in this graph"));
}
...
- (id<JavaUtilCollection>)findEdgeSetWithId:(id)v1
                                     withId:(id)v2 {
  if (![((id<JavaUtilCollection>) nil_chk([self getVertices])) containsWithId:v1]) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("@$", v1, @" is not an element of this graph"));
  if (![((id<JavaUtilCollection>) nil_chk([self getVertices])) containsWithId:v2]) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("@$", v2, @" is not an element of this graph"));
...
- (id<JavaUtilCollection>)getIncidentVerticesWithId:(id)edge {
  ...
  id<JavaUtilCollection> incident = new_JavaUtilArrayList_init();
...
- (NSString *)description {
  JavaLangStringBuffer *sb = new_JavaLangStringBuffer_initWithNSString_(@"Vertices:");
  for (id __strong v in nil_chk([self getVertices])) {
    (void) [sb appendWithNSString:JreStrcat("@C", v, ',')];
  }
  ...
  for (id __strong e in nil_chk([self getEdges])) {
    ...
    (void) [sb appendWithNSString:JreStrcat("@C@C@$", e, '[', [((EduUciIcsJungGraphUtilPair *) nil_chk(ep)) getFirst], ',', [ep getSecond], @"] ")];
  }
