...
withEduUciIcsJungGraphUtilEdgeTypeEnum:(EduUciIcsJungGraphUtilEdgeTypeEnum *)edgeType {
  if ([((id<JavaUtilCollection>) nil_chk([((id<EduUciIcsJungGraphGraph>) nil_chk(delegate_)) getVertices])) containsWithId:v1] == NO) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@", @"Tree must already contain ", v1));
  }
  if ([((id<JavaUtilCollection>) nil_chk([delegate_ getVertices])) containsWithId:v2]) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@", @"Tree must not already contain ", v2));
  }
...
- (jboolean)removeVertexWithId:(id)vertex
  ...
  if (remove_subtrees) for (id __strong v in new_JavaUtilArrayList_initWithJavaUtilCollection_([delegate_ getSuccessorsWithId:vertex])) [self removeVertexWithId:v withBoolean:YES];
...
- (id<JavaUtilCollection>)getRoots {
  id<JavaUtilCollection> roots = new_JavaUtilHashSet_init();
...
- (id<JavaUtilCollection>)getTrees {
  id<JavaUtilCollection> trees = new_JavaUtilHashSet_init();
  for (id __strong v in nil_chk([self getRoots])) {
    id<EduUciIcsJungGraphTree> tree = new_EduUciIcsJungGraphDelegateTree_init();
...
void EduUciIcsJungGraphDelegateForest_init(EduUciIcsJungGraphDelegateForest *self) {
  (void) EduUciIcsJungGraphDelegateForest_initWithEduUciIcsJungGraphDirectedGraph_(self, new_EduUciIcsJungGraphDirectedSparseGraph_init());
}
...
void EduUciIcsJungGraphDelegateForest_initWithEduUciIcsJungGraphDirectedGraph_(EduUciIcsJungGraphDelegateForest *self, id<EduUciIcsJungGraphDirectedGraph> delegate) {
  (void) EduUciIcsJungGraphGraphDecorator_initWithEduUciIcsJungGraphGraph_(self, delegate);
}
