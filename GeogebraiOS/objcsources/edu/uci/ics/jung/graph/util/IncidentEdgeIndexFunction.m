...
//  source: /Users/jacky/GSoC/GeoGebraiOSARC/GeoGebraiOSARC/javasources/edu/uci/ics/jung/graph/util/IncidentEdgeIndexFunction.java
//
...
- (jint)getIndexWithEduUciIcsJungGraphGraph:(id<EduUciIcsJungGraphGraph>)graph
  ...
  for (id __strong other in commonEdgeSet) {
    if ([nil_chk(e) isEqual:other] == NO) {
      (void) [((id<JavaUtilMap>) nil_chk(edge_index_)) putWithId:other withId:JavaLangInteger_valueOfWithInt_(count)];
      count++;
    }
  }
  (void) [((id<JavaUtilMap>) nil_chk(edge_index_)) putWithId:e withId:JavaLangInteger_valueOfWithInt_(count)];
  return count;
}
...
EduUciIcsJungGraphUtilIncidentEdgeIndexFunction *EduUciIcsJungGraphUtilIncidentEdgeIndexFunction_getInstance() {
  ...
  return new_EduUciIcsJungGraphUtilIncidentEdgeIndexFunction_init();
}
