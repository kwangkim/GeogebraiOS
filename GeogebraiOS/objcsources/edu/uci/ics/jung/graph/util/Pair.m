...
- (jboolean)addWithId:(id)o {
  @throw new_JavaLangUnsupportedOperationException_initWithNSString_(@"Pairs cannot be mutated");
}
...
- (id<JavaUtilIterator>)iterator {
  return new_EduUciIcsJungGraphUtilPair_PairIterator_initWithEduUciIcsJungGraphUtilPair_(self);
}
...
- (IOSObjectArray *)toArray {
  IOSObjectArray *to_return = [IOSObjectArray newArrayWithLength:2 type:NSObject_class_()];
  (void) IOSObjectArray_Set(to_return, 0, first_);
  (void) IOSObjectArray_Set(to_return, 1, second_);
  return to_return;
}
...
- (IOSObjectArray *)toArrayWithNSObjectArray:(IOSObjectArray *)a {
  ...
  if (((IOSObjectArray *) nil_chk(a))->size_ < 2) {
    IOSObjectArray *arr = [IOSObjectArray newArrayWithLength:2 type:NSObject_class_()];
    to_return = (IOSObjectArray *) check_class_cast(arr, [IOSObjectArray class]);
  }
  (void) IOSObjectArray_Set(nil_chk(to_return), 0, (id) first_);
  (void) IOSObjectArray_Set(to_return, 1, (id) second_);
  if (to_return->size_ > 2) (void) IOSObjectArray_Set(to_return, 2, nil);
  return to_return;
}
...
void EduUciIcsJungGraphUtilPair_initWithId_withId_(EduUciIcsJungGraphUtilPair *self, id value1, id value2) {
  ...
  if (value1 == nil || value2 == nil) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Pair cannot contain null values");
  self->first_ = value1;
  self->second_ = value2;
}
...
void EduUciIcsJungGraphUtilPair_initWithJavaUtilCollection_(EduUciIcsJungGraphUtilPair *self, id<JavaUtilCollection> values) {
  (void) NSObject_init(self);
  if (values == nil) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Input collection cannot be null");
  if ([((id<JavaUtilCollection>) nil_chk(values)) size] == 2) {
    if ([values containsWithId:nil]) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Pair cannot contain null values");
    id<JavaUtilIterator> iter = [values iterator];
    self->first_ = [((id<JavaUtilIterator>) nil_chk(iter)) next];
    self->second_ = [iter next];
  }
  else @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Pair may only be created from a Collection of exactly 2 elements");
}
...
void EduUciIcsJungGraphUtilPair_initWithNSObjectArray_(EduUciIcsJungGraphUtilPair *self, IOSObjectArray *values) {
  (void) NSObject_init(self);
  if (values == nil) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Input array cannot be null");
  if (((IOSObjectArray *) nil_chk(values))->size_ == 2) {
    if (IOSObjectArray_Get(values, 0) == nil || IOSObjectArray_Get(values, 1) == nil) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Pair cannot contain null values");
    self->first_ = IOSObjectArray_Get(values, 0);
    self->second_ = IOSObjectArray_Get(values, 1);
  }
  else @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Pair may only be created from an array of 2 elements");
}
