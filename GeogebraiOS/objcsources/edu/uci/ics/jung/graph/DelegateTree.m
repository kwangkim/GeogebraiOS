...
//  source: /Users/jacky/GSoC/GeogebraiOS/GeogebraiOS/sources/edu/uci/ics/jung/graph/DelegateTree.java
//
...
- (jboolean)addVertexWithId:(id)vertex {
  if (root_ == nil) {
    EduUciIcsJungGraphDelegateTree_set_root_(self, vertex);
    [((id<JavaUtilMap>) nil_chk(vertex_depths_)) putWithId:vertex withId:JavaLangInteger_valueOfWithInt_(0)];
    return [((id<EduUciIcsJungGraphGraph>) nil_chk(delegate_)) addVertexWithId:vertex];
  }
  else {
    @throw [new_JavaLangUnsupportedOperationException_initWithNSString_(@"Unless you are setting the root, use addChild()") autorelease];
  }
}
...
- (jboolean)removeVertexWithId:(id)vertex {
  ...
  for (id __strong v in nil_chk([self getChildrenWithId:vertex])) {
    ...
    [((id<JavaUtilMap>) nil_chk(vertex_depths_)) removeWithId:v];
  }
  [((id<JavaUtilMap>) nil_chk(vertex_depths_)) removeWithId:vertex];
...
withEduUciIcsJungGraphUtilEdgeTypeEnum:(EduUciIcsJungGraphUtilEdgeTypeEnum *)edgeType {
  ...
  if ([((id<JavaUtilCollection>) nil_chk(vertices)) containsWithId:parent] == NO) {
    @throw [new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@", @"Tree must already contain parent ", parent)) autorelease];
  }
  if ([vertices containsWithId:child]) {
    @throw [new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@", @"Tree must not already contain child ", child)) autorelease];
  }
  [vertex_depths_ putWithId:child withId:JavaLangInteger_valueOfWithInt_([((JavaLangInteger *) nil_chk([((id<JavaUtilMap>) nil_chk(vertex_depths_)) getWithId:parent])) intValue] + 1)];
...
- (id<JavaUtilList>)getPathWithId:(id)vertex {
  ...
  id<JavaUtilList> vertex_to_root = [new_JavaUtilArrayList_init() autorelease];
  ...
  id<JavaUtilList> root_to_vertex = [new_JavaUtilArrayList_initWithInt_([vertex_to_root size]) autorelease];
...
id<OrgApacheCommonsCollections15Factory> EduUciIcsJungGraphDelegateTree_getFactory() {
  ...
  return [new_EduUciIcsJungGraphDelegateTree_$1_init() autorelease];
}

void EduUciIcsJungGraphDelegateTree_init(EduUciIcsJungGraphDelegateTree *self) {
  EduUciIcsJungGraphDelegateTree_initWithOrgApacheCommonsCollections15Factory_(self, EduUciIcsJungGraphDirectedSparseMultigraph_getFactory());
}
...
void EduUciIcsJungGraphDelegateTree_initWithOrgApacheCommonsCollections15Factory_(EduUciIcsJungGraphDelegateTree *self, id<OrgApacheCommonsCollections15Factory> graphFactory) {
  EduUciIcsJungGraphGraphDecorator_initWithEduUciIcsJungGraphGraph_(self, [((id<OrgApacheCommonsCollections15Factory>) nil_chk(graphFactory)) create]);
  EduUciIcsJungGraphDelegateTree_setAndConsume_vertex_depths_(self, new_JavaUtilHashMap_init());
}
...
void EduUciIcsJungGraphDelegateTree_initWithEduUciIcsJungGraphDirectedGraph_(EduUciIcsJungGraphDelegateTree *self, id<EduUciIcsJungGraphDirectedGraph> graph) {
  EduUciIcsJungGraphGraphDecorator_initWithEduUciIcsJungGraphGraph_(self, graph);
  EduUciIcsJungGraphDelegateTree_setAndConsume_vertex_depths_(self, new_JavaUtilHashMap_init());
}
...
- (id<EduUciIcsJungGraphTree>)create {
  return [new_EduUciIcsJungGraphDelegateTree_initWithEduUciIcsJungGraphDirectedGraph_([new_EduUciIcsJungGraphDirectedSparseMultigraph_init() autorelease]) autorelease];
}
