...
public class GBasicStrokeI implements org.geogebra.common.awt.GBasicStroke {
	...
	public GBasicStrokeI() {
		this(1.0f, CAP_SQUARE, JOIN_MITER, 10.0f, null, 0.0f);
	}

	public GBasicStrokeI(float width, int cap, int join, float miterLimit,
	...
	public GBasicStrokeI(float width, int cap, int join) {
		this(width, cap, join, 10.0f, null, 0.0f);
	}

	public GBasicStrokeI(float width) {
	...
	void createSolidShape(org.geogebra.common.awt.GPathIterator pathIterator) {
		...
		while (!pathIterator.isDone()) {
			...
			case GPathIteratorI.SEG_MOVETO:
				if (!isClosed) {
					closeSolidShape();
				}
				rp.clean();
	...
	static class BufferedPath {
		...
		void moveTo(double x, double y) {
			checkBuf(1, 2);
			types[typeSize++] = GPathIteratorI.SEG_MOVETO;
		...
		void lineTo(double x, double y) {
			checkBuf(1, 2);
			types[typeSize++] = GPathIteratorI.SEG_LINETO;
		...
		void quadTo(double x1, double y1, double x2, double y2) {
			checkBuf(1, 4);
			types[typeSize++] = GPathIteratorI.SEG_QUADTO;
		...
		void cubicTo(double x1, double y1, double x2, double y2, double x3,
		        double y3) {
			checkBuf(1, 6);
			types[typeSize++] = GPathIteratorI.SEG_CUBICTO;
			points[pointSize++] = (float) x1;
			points[pointSize++] = (float) y1;
		...
		void closePath() {
			checkBuf(1, 0);
			types[typeSize++] = GPathIteratorI.SEG_CLOSE;
		}
		...
		void appendReverse(BufferedPath p) {
			...
			for (int i = p.typeSize - 1; i >= 0; i--) {
				byte type = p.types[i];
				if (type == GPathIteratorI.SEG_MOVETO) {
					types[closeIndex] = GPathIteratorI.SEG_MOVETO;
					types[typeSize++] = GPathIteratorI.SEG_CLOSE;
				} else {
					if (type == GPathIteratorI.SEG_CLOSE) {
						closeIndex = typeSize;
					}
					types[typeSize++] = type;
				}
	...
	public GBasicStrokeI(org.geogebra.common.awt.GBasicStroke objStroke) {
		this.lineWidth = ((GBasicStrokeI) objStroke).getLineWidth();
		this.lineCap = ((GBasicStrokeI) objStroke).getLineCap();
		this.lineJoin = ((GBasicStrokeI) objStroke).getLineJoin();
		this.dasharray = ((GBasicStrokeI) objStroke).getDashArray();
	}
