 ...
 */
    final public ValidExpression buildCASExpression(GeoCasCell cell) throws ParseException {ValidExpression ve;
    ExpressionNode lhs, rhs;
    Token l = new Token();
    Token assign = null;
    Token c= null;
 ...
 */
    final public ValidExpression buildMathPiperExpression() throws ParseException {ValidExpression ve;
    ExpressionNode lhs, rhs;
    Token l = new Token();
    Vector labels = new Vector();
 ...
 */
    final public ValidExpression buildMaximaExpression() throws ParseException {ValidExpression ve;
    ExpressionNode lhs, rhs;
 ...
 */
    final public Vector labellist() throws ParseException {Token t;
    Vector labels;
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case 73:{
      jj_consume_token(73);
 ...
 */
    final public Equation equation() throws ParseException {ExpressionNode lhs, rhs;
    lhs = expressionnode();
    jj_consume_token(EQUAL);
    rhs = expressionnode();
{if ("" != null) return new Equation(kernel, lhs, rhs);}
    throw new Error("Missing return statement in function");
  }
 ...
 */
    final public ExpressionNode expression() throws ParseException {ExpressionNode rhs;
    rhs = expressionnode();
 ...
 */
    final public ExpressionNode cmdexpression() throws ParseException {ExpressionNode en;
...
// allow f(y) in CAS but not in GeoGebra
    //  if (!ExternalCASParsing && !GeoGebraCASParsing && "y".equals(varName.image)) 
      ...
      if(op!=null)
      {
        ...
        {if ("" != null) return new Equation(kernel,vars.wrap(),rhs).wrap();}
      }
 ...
 */
    final public FunctionNVar casFunction(GeoCasCell cell) throws ParseException {ExpressionNode rhs;
    Token funName = null;
    Token varName = null;
    Token assign = null;
    ArrayList<String> localVars;
 ...
 */
    final public ExpressionNode expressionOrEquation() throws ParseException {ExpressionNode en, rhs;
    en = expressionnode();
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case EQUAL:{
      jj_consume_token(EQUAL);
 ...
 */
    ...
    case MINUS:{
      ...
      e = enpower();
// -e  = -1*e
          {if ("" != null) return new ExpressionNode(kernel, new MyDouble(kernel, -1.0d), Operation.MULTIPLY, e);}
      break;
      }
    case PLUS:{
...
// remove " (first and last character)
    ...
    if (jj_2_28(5)) {
      jj_consume_token(73);
      jj_consume_token(74);
{if ("" != null) return new MyList(kernel);}
    } else {
      switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
      case 73:{
        jj_consume_token(73);
...
{if ("" != null) return myList;}
    ...
    final public ExpressionValue numVecVariable() throws ParseException {GeoElement geo;
    Token c;
    ExpressionValue ev;
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case VARX:{
...
// should we read e for Euler constant and i for imaginary unit?
    ...
    if (lookforEulerImaginary)
    {
      ...
      {
        {if ("" != null) return MySpecialDouble.getEulerConstant(kernel);}
      }
...
if (ExternalCASParsing || GeoGebraCASParsing) {
        ...
        // precision than double
           {if ("" != null) return new MySpecialDouble(kernel, val, c.image);}
      }
      else {
          {if ("" != null) return new MyDouble(kernel, val);}
      }
      break;
      }
    case DEGREE:{
...
{if ("" != null) return new MySpecialDouble(kernel, 0.57721566490153286, Unicode.EULER_GAMMA_STRING);}
      break;
      }
    case E:{
      jj_consume_token(E);
{if ("" != null) return new MySpecialDouble(kernel, Math.E, Unicode.EULER_STRING);}
      break;
      }
    case IMAGINARY:{
 ...
 */
    final public ExpressionNode numberFunction() throws ParseException {ExpressionNode en, en2, en3,en4;
    ExpressionValue ev;
    MyList myList;
...
// remove the opening parenthesis
      ...
      if(!forceCommand){
            ...
            if (cell==null && (geo == null || !(geo.isGeoFunction() || geo.isGeoCurveCartesian())))  {
                if(label.startsWith("log_"))
                {
                        ...
                        double indexVal = MyDouble.parseDouble(loc, logIndex);
                                        ...
                                        {if ("" != null) return new ExpressionNode(kernel, new MyDouble(kernel, indexVal), Operation.LOGB, myList.getListElement(0));}
                }
          ...
          if (forceCommand || (geo == null && cell==null)) {
                ...
                if(op!=null)
                  {if ("" != null) return buildOpNode(op,myList);}
              ...
              {if ("" != null) return new ExpressionNode(kernel, cmd);}
...
//                throw new MyParseError(loc, str);    
      ...
      if(cell!=null && order > 0)  {
            ...
            {if ("" != null) return new ExpressionNode(kernel, derivative, Operation.FUNCTION, myList.getListElement(0));}
        ...
        if(cell!=null && (geo==null || geo.isGeoLine() || geo.isGeoConic()))  {
          if(((GeoCasCell)cell).getFunctionVariables().length <2)
          {
            ...
            {if ("" != null) return new ExpressionNode(kernel, cell, Operation.FUNCTION, myList.getListElement(0));}
        }else
        {
        {if ("" != null) return new ExpressionNode(kernel, cell, Operation.FUNCTION_NVAR, myList);}
        }
      ...
      int argNumber = myList.size();
          ...
          if (order > 0) { // derivative
            ...
            if (geo.isGeoFunction()) {// function 
              ...
              {if ("" != null) return new ExpressionNode(kernel, derivative, Operation.VEC_FUNCTION, myList.getListElement(0));}
            }
            else {
            ...
            if (geo instanceof Evaluatable) // function 
              {if ("" != null) return new ExpressionNode(kernel, geoExp, Operation.FUNCTION, myList.getListElement(0));}
        else if (geo instanceof GeoFunctionNVar) {
          {if ("" != null) return new ExpressionNode(kernel, geoExp, Operation.FUNCTION_NVAR, myList);}
        }
            else if (geo.isGeoCurveCartesian()) // vector function
            // at this point we have eg myList={{1,2}}, so we need first element of myList                    
              {if ("" != null) return new ExpressionNode(kernel, geoExp, Operation.VEC_FUNCTION, myList.getListElement(0));}
 ...
 */
    final public VectorValue cartesianvector() throws ParseException {ExpressionValue x, y;
...
{if ("" != null) return new MyVecNode(kernel, x, y);}
    throw new Error("Missing return statement in function");
  }
 ...
 */
    final public Vector3DValue cartesianvector3D() throws ParseException {ExpressionValue x, y, z;
...
{if ("" != null) return new MyVec3DNode(kernel, x, y, z);}
    throw new Error("Missing return statement in function");
  }
 ...
 */
    final public VectorValue polarvector() throws ParseException {ExpressionValue r, phi;
    MyVecNode v;
    jj_consume_token(75);
    r = expression();
    jj_consume_token(POLAR_SEPARATOR);
 ...
 */
    final public Vector3DValue sphericalvector() throws ParseException {ExpressionValue r, phi, theta;
    MyVec3DNode v;
    jj_consume_token(75);
    r = expression();
    jj_consume_token(POLAR_SEPARATOR);
...
MyVecNode v = new MyVecNode(kernel);
    ...
    private boolean jj_3R_100()
 {
  ...
  public Parser(java.io.Reader stream) {
    if (jj_initialized_once) {
      System.out.println("ERROR: Second call to constructor of   parser. ");
      System.out.println("       You must either use ReInit() or set the JavaCC option STATIC to false");
      System.out.println("       during parser generation.");
      throw new Error();
    }
